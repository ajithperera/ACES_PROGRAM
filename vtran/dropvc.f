      ...
      SUBROUTINE DROPVC(NBAS,MAXSIZE,NDROP,IUHF,EVEC,IDROP,
     &                  IPNTRA,IPNTRB,IRPSVA,IRPSVB,EDROP)
C
...
C
      ...
      LOGICAL FNO_EXIST 
C
      DIMENSION EVEC(NBAS,NBAS),IDROP(MAXSIZE),IPNTRA(NBAS)
...
c
ckjw 1/28/00 extra logic to support frozen natural orbitals because the
c            size of a record (modropa) can't change
c
      inquire(file='FNO',exist=FNO_EXIST)
      IF(NDROP.NE.0)then
        if (FNO_EXIST)then
               CALL GETREC(20,'JOBARC','FNODROPA',NDROP,IDROP)
        else
...
C
       IF(   (IFLAGS(47).NE.5 .AND.
     &        IFLAGS(47).NE.6 .AND.
     &        IFLAGS2(138).NE.1
     &       ) .OR.
     &        IFLAGS(18).EQ.1                          .OR.
     &      ((IFLAGS(54).EQ.2  .OR. IFLAGS(54).EQ.3)   .AND.
     &       (IFLAGS(81).EQ.0  .OR. IFLAGS(81).EQ.2)) )THEN
C
...
10    CONTINUE
      IF(NDROP.NE.0)then
        if(FNO_EXIST)then
          CALL PUTREC(20,'JOBARC','FNODROPA',NDROP,IDROP)
          iken=ishell('rm FNO')
        else
          CALL PUTREC(20,'JOBARC','MODROPA',NDROP,IDROP)
        endif
      endif
...
304   CONTINUE
       CALL PIKSR2(NBAS,EVEC,IDROP(NBAS+1))
       CALL GETREC(20,'JOBARC','NUMDROPB',IONE,NDROP)
C
       IF (NDROP .NE. 0) THEN 
          IF (FNO_EXIST) THEN
             CALL GETREC(20,'JOBARC','FNODROPB',NDROP,IDROP)
          ELSE
...
20     CONTINUE
       IF(NDROP.NE.0)then 
         if(FNO_EXIST) then
           CALL PUTREC(20,'JOBARC','FNODROPB',NDROP,IDROP)
         else
           CALL PUTREC(20,'JOBARC','MODROPB',NDROP,IDROP)
         endif
       endif
...
c Nevin changed occupyb0 to occupyb for twodet to work
       ...
       DO 51 I=1,NBAS
        ...
        ENDIF
51     CONTINUE
C Certianly very strange! WHy this has not cause any problems for
C Brueckner, stability follow is very starnge since this is changing
C the beta occuptions even when there are no drop orbitals, 10/2022
CSSS       CALL PUTREC(20,'JOBARC','OCCUPYB ',NIRREP,IDROP)

       If (NDROP .NE. 0) CALL PUTREC(20,'JOBARC','OCCUPYB ',
     +                               NIRREP,IDROP)
     ...
     &            EVEC)
      ...
      DO 15 I=1,NBAS
       IF(IPNTRA(I).EQ.0)THEN
        ITHRU=ITHRU+1
cSSS    CALL ICOPY(EVEC(1,I),EVEC(1,ITHRU),NBAS*IINTFP)
...
c     CALL PUTREC(20,'JOBARC','IRREPAL0',NBAS,IDROP)
 ...
 3210 CONTINUE
      IF(IFLAGS(1).GE.5)THEN
       IF (IFLAGS(77) .GT.0) Then
          Write(6,*)
          Write(6,"(a,a)")" Caution: For QRHF the eigenvalues printed",
     +                    " here are for the parent state"
          Write(6,"(a)") " and not for the reference state."
       ENDIF 
...
C
C Save the full space post-SCF ordering for future reference
C (See below for the UHF/ROHF case). Full space post-SCF ordering
C may not be something that we often use, but just leave it
C in JOBARC. A. Perera, 07/2005.
C
      I8=8
      CALL IZERO(IDROP,2*NBAS) 
 ...
 2011  CONTINUE
 2010 CONTINUE
cSSS  CALL ICOPY(IDROP(NBAS+1),IDROP,NBAS)
...
c
      IF(IUHF.EQ.1) THEN
        ...
        CALL PUTREC(20,'JOBARC','REORDEB0',NBAS,IDROP)
        NUMDRP=0
        DO 2020 I=1,NBAS
          DO 2021 J=1,NBAS
            IF(IDROP(J).EQ.I) THEN
