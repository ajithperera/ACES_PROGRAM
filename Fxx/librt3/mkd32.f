      ...
      SUBROUTINE MKD32(D3,EVAL,IADBLK,IRPIJK,
...
C
      DO 1000 C=1,VRT(IRPC,2)
      DO  990 B=1,VRT(IRPB,1)
      DO  980 A=1,VRT(IRPA,1)
C
      D3(IOFFD3 + (C-1)*IRPDPD(IRPAB,1) + (B-1)*VRT(IRPA,1) + A) =
     1 EVAL(NOCA+ABSVRT(A,IRPA,1),1)    + 
     1 EVAL(NOCA+ABSVRT(B,IRPB,1),1)    +
     1 EVAL(NOCB+ABSVRT(C,IRPC,2),2)
  980 CONTINUE
  990 CONTINUE
 1000 CONTINUE
C
...
C
      ...
      IOFFD3 = IADBLK(IRPC)  - 1 + IBLKAB
C
      DO 2000 C=1,VRT(IRPC,2)
      DO 1990 B=2,VRT(IRPB,1)
      DO 1980 A=1,B-1
C
      D3(IOFFD3 + (C-1)*IRPDPD(IRPAB,1) + INDEX(B-1) + A) =
     1 EVAL(NOCA+ABSVRT(A,IRPA,1),1)    + 
     1 EVAL(NOCA+ABSVRT(B,IRPB,1),1)    +
     1 EVAL(NOCB+ABSVRT(C,IRPC,2),2)
 1980 CONTINUE
