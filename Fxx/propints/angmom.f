...
C
      SUBROUTINE AngMom(VLIST,NOC,CENTR1,EXPS,CONT,LABEL,NPRIM,NCONTR,
     &                  ITRM,ITRN,CTRN,NCONT,BUF,IBUF,AOINT,LBUF,INDX,
     &                  NGROUP,VS,SCR,MXPRM2)
C
C &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
C &  Evaluates angular momentum AO integrals                         &
C &  Ajith Perera, 12/09                                             &
C &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER AND
      CHARACTER*8 DUML1,LABELD(3),LABELJA(3,25),JUNKLAB
      CHARACTER*8 TITLE
C
...
C
 ...
 25              CONTINUE
C
C Built the l_x = yPz - zpy, First ypz
C
...
C
     ...
     &                                 DISTQD)
C
                       TEMP(1, 1) = EFACTP*DD*V(2) + TEMP(1, 1)
C
  212                  CONTINUE
                    ENDDO
  211               CONTINUE
...
C
     ...
     &                                 DISTQD)
C
                       TEMP(2, 1) = EFACTP*DD*V(2) + TEMP(2, 1)
C
  214                  CONTINUE
                     ENDDO
  213                CONTINUE
...
C
     ...
     &                                 DISTQD)
C
                       TEMP(3, 1) = EFACTP*DD*V(3) + TEMP(3, 1)
C
  216                  CONTINUE
                    ENDDO
  215               CONTINUE
...
C
     ...
     &                                 DISTQD)
C
                       TEMP(4, 1) = EFACTP*DD*V(3) + TEMP(4, 1)
C
  218                  CONTINUE
                     ENDDO
  217                CONTINUE
...
C Built the l_y = -(xPz - zpx), First xpz
C
                 CALL GETTYPE (LB, MB, NB - 1, JDTYP)
                 CALL RHFTCE (BBB, B, EP, JDTYP, JTM, ONE, JMN)
C
...
C Now l_y = -(xpz - zpx)
C
                 DV(2) = -((NB*TEMP(1, 1) - TWO*EXPS(JEX)*TEMP(2, 1))
     &                   -(LB*TEMP(3, 1) - TWO*EXPS(JEX)*TEMP(4, 1)))
C
C  l_z =  xPy - ypx, First xpy
C
                 CALL GETTYPE (LB, MB - 1, NB, JDTYP)
                 CALL RHFTCE (BBB, B, EP, JDTYP, JTM, ONE, JMN)
C
...
C     
     ...
     &                                 DISTQD)
C     
                       TEMP(3, 1) = EFACTP*DD*V(2) + TEMP(3, 1)
C                          
  232                  CONTINUE
                    ENDDO
  231               CONTINUE
...
C
     ...
     &                                 DISTQD)
C     
                       TEMP(4, 1) = EFACTP*DD*V(2) + TEMP(4, 1)
C     
  234                  CONTINUE
                     ENDDO
  233                CONTINUE
...
C Now l_z = xpy - ypx
C
                 DV(3) = (MB*TEMP(1, 1) - TWO*EXPS(JEX)*TEMP(2, 1)) - 
     &                   (LB*TEMP(3, 1) - TWO*EXPS(JEX)*TEMP(4, 1))
C
C
                 DO 30 MM = 1, 3
