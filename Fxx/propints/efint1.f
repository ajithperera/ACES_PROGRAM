C
      SUBROUTINE EFINT1 (L, M, N, GA, V, NT, D, lmn)
...
C
      ...
      ITOT = L + M + N
      ITOTH = ITOT/2
      if (lmn .eq. 1)  PRE = 4.00D+00*PI*FN(L)*FN(M+1)*FN(N+1)
      if (lmn .eq. 2)  PRE = 4.00D+00*PI*FN(L+1)*FN(M)*FN(N+1)
      if (lmn .eq. 3)  PRE = 4.00D+00*PI*FN(L+1)*FN(M+1)*FN(N)
C
