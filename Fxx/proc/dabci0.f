...
C DRIVER FOR SORTING <AB||CI> INTEGRALS.

      SUBROUTINE DABCI0(ICORE,MEMSIZ,NUMIRW,ISYM,IPW,IPDIS,
     &                  NWDIS,ISPIN,NMO,IRRVEC)
...
C
C  SUBTRACT AWAY SPACE FOR SCRATCH ARRAY IN THE ANTISYMMETRIZATION PART
C
...
4000   FORMAT(T3,'@DABCI0-F, Not enough core to hold one integral ',
     &        'distribution.',/,
...
CSSS      WRITE(6,*)' I100 Is ',I100,'  MEMSIZ IS ',MEMSIZ
      IF(I100.GT.MEMSZ)THEN
       WRITE(6,5000)
5000   FORMAT(T3,'@DABCI0-F, Insufficient core to perform sort.')
       CALL INSMEM('DABCI0',I100,MEMSZ)
       CALL ERREX
      ENDIF
      WRITE(6,1000)MEMSZ,MEMSIZ,DISSIZ*LENINT,NUMDIS,NINCOR,NBUCK,
     &        NDBCK,NBKINT,IRECL,I100
1000   FORMAT(T3,'@DABCI0-I, Information for AO integral sort',/,
     ...
     &   '        Total utilized memory (words)          ',I10,'.')
      CALL SABCI0(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
     &           ICORE(I040),ICORE(I050),NBUCK,NDBCK,
     &           NBKINT,NREC,IRECL,ILNBUF,ISPIN,ISYM,
...
C
      ...
      I025=I020+DISSIZ*IINTFP
      I030=I025+NBKINT*IINTFP
      I040=I030+NBKINT
      WRITE(6,200)I040,MEMSZ
200   FORMAT(T3,'@DABCI0-I, Sort chain processing uses ',I10,
     &          ' out of ',I10,' words.')
      IF(I040.GT.MEMSZ)THEN
       WRITE(6,300)I040,MEMSZ
       CALL ERREX
300    FORMAT(T3,'@DABCI0-F, Core screwup. ',I10,' words needed.',
     &           I10,' words available.')
      ENDIF
      CALL CABCI0(ICORE(I010),ICORE(I020),ICORE(I025),ICORE(I030),
     &            ICORE(I000),DISSIZ,NDBCK,NBKINT,NBUCK,
     &            IRECL,ISPIN,NUMIRW,ISYM,IPW,IPDIS,NWDIS,NMO,
     &            IRRVEC)
      RETURN
      END
