...
C DRIVER FOR SORTING <AB|IC> (SPIN CASE ALPHA-BETA) INTEGRALS.

      SUBROUTINE DABCI2(ICORE,MEMSIZ,NUMIRW,ISYM,IPW,IPDIS,
...
4000   FORMAT(T3,'@DABCI2-F, Not enough core to hold one integral ',
     &        'distribution.',/,
...
CSSS      WRITE(6,*)' I100 Is ',I100,'  MEMSIZ IS ',MEMSIZ
      IF(I100.GT.MEMSIZ)THEN
       WRITE(6,5000)
5000   FORMAT(T3,'@DABCI2-F, Insufficient core to perform sort.')
       CALL INSMEM('DABCI2',I100,MEMSZ)
       CALL ERREX
      ENDIF
      WRITE(6,1000)MEMSZ,MEMSIZ,DISSIZ*LENINT,NUMDIS,NINCOR,NBUCK,
     &        NDBCK,NBKINT,IRECL,I100
1000   FORMAT(T3,'@DABCI2-I, Information for AO integral sort',/,
     ...
     &   '        Total utilized memory (words)          ',I10,'.')
      CALL SABCI2(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
...
200   FORMAT(T3,'@DABCI2-I, Sort chain processing uses ',I10,
     &          ' out of ',I10,' words.')
      IF(I040.GT.MEMSIZ)THEN
       WRITE(6,300)I060,MEMSIZ
       CALL ERREX
300    FORMAT(T3,'@DABCI2-F, Core screwup. ',I10,' words needed.',
     &           I10,' words available.')
      ENDIF
      CALL CABCI2(ICORE(I010),ICORE(I020),ICORE(I030),
