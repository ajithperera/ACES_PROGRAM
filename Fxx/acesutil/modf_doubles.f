...
C
      ...
      NNM1O2(IX)=(IX*(IX-1))/2
C
      IF (ACT .EQ. "SAVE") WRITE(IUNIT, "(I10)") NT2SIZ
...
C
            DO J_OCC = 1, POP(JIRREP, BSPIN)
               DO I_OCC = 1, POP(IIRREP, ASPIN)

                  DO BIRREP = 1, NIRREP
                     AIRREP = DIRPRD(BIRREP, IRREPL)

                      DO B_VRT = 1, VRT(BIRREP, BSPIN)
                         DO A_VRT = 1, VRT(AIRREP, ASPIN)
C
                           If (ORDERED(I_OCC, J_OCC, A_VRT, B_VRT,
     &                                 IIRREP, JIRREP, AIRREP, 
     &                                 BIRREP, ICASE)) THEN
                           IOFF  = IOFF + 1
...
C
                              ...
                              I_IND = I(Ioff) + NDRPOP(I_IRREP) +
     &                                          IPOP_OFF(I_IRREP)
                              J_IND = J(Ioff) + NDRPOP(J_IRREP) + 
     &                                          IPOP_OFF(J_IRREP)
                              A_IND = A(Ioff) + IVRT_OFF(A_IRREP)
                              B_IND = B(Ioff) + IVRT_OFF(B_IRREP)
