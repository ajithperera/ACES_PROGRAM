      SUBROUTINE OST2I2(ICORE,MAXCOR,IRREPM,IRREPN2,IRREPM2,IRREPN1,
     $   LIST1,ISPINT1,LIST2,
     $   POPII,POPC,POPD,STR2,STRTAR,LISTAR,FACT,FACT2,FTAU2,IEX)
C
C     THIS ROUTINE CALCULATE THE OUTER PRODUCT OF LIST1 AND LIST2:
C
C     T2(CI,DM)=FACT*T2(N2C,M2I)*T1(D,N1)
C
C     INPUT:
C             IRREPM:         IRREP FOR M
...
C             POPII,
C             POPC,POPD:      POPULATION VECTORS FOR I,C AND D
...
C
      ...
      CALL FSGETT1(ICORE(I000),ISPINT1,LIST1,'IA',8+ISPINT1)
C
      IRREPD=IRREPN1
      IRREP=DIRPRD(IRREPD,IRREPM)
C
      IF(STRTAR.EQ.'IAII') THEN
         NUMSYT=FSDPDII(IRREP,ISYTYP(2,LISTAR))
         DISSYT=FSDPDIA(IRREP,ISYTYP(1,LISTAR))
      ELSE IF(STRTAR.EQ.'IIIA') THEN
         NUMSYT=FSDPDIA(IRREP,ISYTYP(2,LISTAR))
         DISSYT=FSDPDII(IRREP,ISYTYP(1,LISTAR))
      ELSE
         WRITE(6,*)'@OST2I2-E FATAL ERROR.',STRTAR,
...
C
         ...
         ELSEIF(STR2.EQ.'AIIA') THEN
            ...
            STRING='AIIA'
         ELSE
            WRITE(6,*)'@OST2I2-E FATAL ERROR.',STR2,
...
C
         ...
         ENDIF
C
         IF(STRTAR.EQ.'IIIA') THEN 
            IF(NUMSYT.NE.NSIZT1) THEN
               WRITE(6,*)'@OST2I2-E FATAL ERROR,',
