      SUBROUTINE PDCC_H4ALL(ICORE,MAXCOR,SPCASE,IUHF)
C
...
C AAAA: W(BM,EJ) = T2(FB,NJ)*L2(MN,EF) + T2(fB,nJ)*L2(Mn,Ef) 
C BBBB: W(bm,ej) = T2(fb,nj)*L2(mn,ef) + T2(Fb,Nj)*L2(mN,eF) 
C With a minus sign to account for (BM,EJ) storage. 
C
      IF(SPCASE.EQ.'AAAA'.OR.SPCASE.EQ.'BBBB')THEN
       ...
       ELSE IF(CC) THEN
        LIST1A=133+ISPIN
CSSS        IF (ISPAR .AND. COULOMB) LIST1A=138-ISPIN
        LIST2A=33+ISPIN
CSSS        IF (ISPAR .AND. COULOMB) LIST2A=38-ISPIN
        LIST1B=138-ISPIN
        LIST2B=38-ISPIN
        ISAME=.FALSE.
        FACT=ONEM
...
C
CSSS        CALL DSCAL(NUMSYG*DISSYG,-1.0D0,ICORE(I000),1)
        CALL PUTLST(ICORE(I000),1,NUMSYG,2,IRREP,LISTG)
        IF (IUHF .EQ. 0) 
     &   CALL PUTLST(ICORE(I000),1,NUMSYG,2,IRREP,254)
        if (ispin.eq.1) CALL Checksum("List-254:",ICORE(I000),
     &                                 NUMSYG*DISSYG)
        if (ispin.eq.2) CALL Checksum("List-255:",ICORE(I000),
     &                                 NUMSYG*DISSYG)
100    CONTINUE

C ABAB: W(Mb,EJ) = T2(Fb,Nj)*L2(MN,EF) + T2(fb,nj)*L2(Mn,Ef) 
C BBBB: W(mB,eJ) = T2(fB,nJ)*L2(mn,ef) + T2(FB,NJ)*L2(mN,eF) 
C With a minus sign to account for (BM,EJ) storage. I believe 
C This is a misnomer to call these ABBA and BAAB
      ...
      ELSEIF(SPCASE.EQ.'ABBA'.OR.SPCASE.EQ.'BAAB')THEN
       ...
       IF(SPCASE.EQ.'ABBA')THEN
        ...
        ELSE IF(CC) THEN
         FACT=ONEM
         LIST1A=134
CSSS         IF (ISPAR .AND. COULOMB) LIST1A=137
         LIST2A=37
         LIST1B=137
         LIST2B=34+IUHF
CSSS         IF (ISPAR .AND. COULOMB) LIST2B=37
        ENDIF
       ENDIF
       IF(SPCASE.EQ.'ABBA'.AND.IUHF.EQ.1.AND.CC)THEN
        ...
        FACT=ONEM
        LIST3A=135
CSSS        IF (ISPAR .AND. COULOMB) LIST3A=139
        LIST4A=36
        LIST3B=136
        LIST4B=34
CSSS        IF (ISPAR .AND. COULOMB) LIST4B=39
       ENDIF
...
C  SAVE LIST 56
C
CSSS        CALL DSCAL(NUMSYG*DISSYG,0.0D0,ICORE(I000),1)
        CALL PUTLST(ICORE(I000),1,NUMSYG,2,IRREP,LISTG)
          CALL Checksum("List-256:",ICORE(I000), NUMSYG*DISSYG)
C
...
C  SAVE 57 ON DISK
C
CSSS          CALL DSCAL(NUMSYG2*DISSYG2,0.0D0,ICORE(I000),1)
          CALL PUTLST(ICORE(I000),1,NUMSYG2,2,IRREP,LISTG2)
          CALL Checksum("List-257:",ICORE(I000), NUMSYG2*DISSYG2)
         ENDIF
        ENDIF
200    CONTINUE
...
C
C ABBA: (Mb,eJ) = T2(bF,nJ)*L2(nM,eF)  Stored as (eM,bJ)
C BAAB: (mB,Ej) = T2(Bf,Nj)*L2(Nm,Ef)  stored as (Em,Bj)
C Storage pattern introdue a negative sign.
...
C SAVE H4 INTERMEDIATE ON DISK
C
CSSS        IF (ISPAR .AND. COULOMB) CALL DSCAL(NUMSYG*DISSYG,-1.0D0,
CSSS     &                                      ICORE(I000),1)
C
        If (SPCASE.EQ."ABAB") CALL Checksum("List-258:",ICORE(I000),
     &                                     NUMSYG*DISSYG)
        If (SPCASE.EQ."BABA") CALL Checksum("List-259:",ICORE(I000),
     &                                     NUMSYG*DISSYG)
        CALL PUTLST(ICORE(I000),1,NUMSYG,2,IRREP,LISTG)
300    CONTINUE
      ENDIF
