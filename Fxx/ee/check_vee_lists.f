...
c syminf.com : end
      ...
      Call checksum("list-493-AA      :",Work,Pha_length)
      If (Iuhf .Ne. 0) Then
         Call Getlst(Work,1,1,1,4,493)
         Call checksum("list-493-BB      :",Work,Phb_length)
      Endif 
      Else 
      Call Getlst(Work,1,1,1,3,490)
      Call checksum("list-490-AA      :",Work,Pha_length)
      ...
      call checksum("List-463;Z(Ab,Ij):",Work(1),length)
      Else
      Length=idsymsz(Irrepx,isytyp(1,463),isytyp(2,463))
      call getall(Work(1),length,irrepx,463)
      ...
      call checksum("List-467;Z(AB,IJ):",Work(1),length)
      Length=idsymsz(Irrepx,isytyp(1,468),isytyp(2,468))
      call getall(Work(1),length,irrepx,468)
      call checksum("List-468;Z(ab,ij):",Work(1),length)
      Length=idsymsz(Irrepx,isytyp(1,469),isytyp(2,469))
      call getall(Work(1),length,irrepx,469)
      call checksum("List-469;Z(Ab,Ij):",Work(1),length)
      Else
      Length=idsymsz(Irrepx,isytyp(1,469),isytyp(2,469))
      call getall(Work(1),length,irrepx,469)
      call checksum("List-469;Z(Ai,Bj):",Work(1),length)
      Endif 
      Else
      If (iuhf .ne. 0) Then
      Length=idsymsz(Irrepx,isytyp(1,461),isytyp(2,461))
