...
C     
            DO 30 IPERT = 1, 6
C     
               CALL SEEKLB_EE (LABELSD(IPERT), IERR, IRWND, 30)
               IF (IERR .NE. 0) CALL ERREX
               CALL LOADINT (SCR(I000), NATOM, NSIZE, NAO, IUHF)
C     
               IF (IFLAGS(1) .GT. 40) THEN
                  CALL HEADER_EE ('PROPERTY INTEGRALS (SD)', 1, 6)
                  CALL TAB (LUOUT, SCR(I000), NAO, NAO, NAO, NAO)
                  CALL HEADER_EE ('TRANSITION DENSITY', 1, 6)
                  CALL TAB (LUOUT, TRNDEN, NAO, NAO, NAO, NAO)
               ENDIF
...
C     
            DO 50 IPERT = 1, 3
C     
               CALL SEEKLB_EE (LABELPSO(IPERT), IERR, IRWND, 30)
               IF (IERR .NE. 0) CALL ERREX
               CALL LOADINT (SCR(I000), NATOM, NSIZE, NAO, IUHF)
C
               IF (IFLAGS(1) .GT. 40) THEN
                  CALL HEADER_EE ('PROPERTY INTEGRALS (PSO)', 1, 6)
                  CALL TAB (LUOUT, SCR(I000), NAO, NAO, NAO, NAO)
                  CALL HEADER_EE ('TRANSITION DENSITY', 1, 6)
                  CALL TAB (LUOUT, TRNDEN, NAO, NAO, NAO, NAO)
               ENDIF
