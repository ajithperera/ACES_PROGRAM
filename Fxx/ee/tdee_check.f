...
C Source and target singels
      ...
      If (Iside .EQ.1) Then
         ...
         Call checksum("list-390-AA      :",Work,Pha_length)
         Call Putlst(Work,1,1,1,1,490)
         If (Iuhf .EQ. 1) Then
            Call Getlst(Work,1,1,1,2,390)
            Call checksum("list-390-BB      :",Work,Pha_length)
            Call Putlst(Work,1,1,1,2,490)
         Endif 
      Else if (Iside .EQ. 2) Then
         Call Getlst(Work,1,1,1,1,392)
         Call checksum("list-392-AA      :",Work,Pha_length)
         Call Putlst(Work,1,1,1,1,490)
         If (Iuhf .EQ. 1) Then
            Call Getlst(Work,1,1,1,2,392)
            Call checksum("list-392-BB      :",Work,Pha_length)
            Call Putlst(Work,1,1,1,2,490)
         Endif
      Endif 
      ...
      If (Iuhf .NE. 0) Then
         ...
         Call checksum("List-461;Z(AB,IJ):",Work,Length)
        
         Length = idsymsz(Irrepx,isytyp(1,462),isytyp(2,462))
         Call Getall(Work,Length,Irrepx,462)
         Call checksum("List-462;Z(ab,ij):",Work,Length)

         Length = idsymsz(Irrepx,isytyp(1,463),isytyp(2,463))
         Call Getall(Work,Length,Irrepx,463)
         Call checksum("List-463;Z(ab,ij):",Work,Length)
      Else
         Length = idsymsz(Irrepx,isytyp(1,463),isytyp(2,463))
         Call Getall(Work,Length,Irrepx,463)
         Call checksum("List-463;Z(Ai,Bj):",Work,Length)
      Endif 
...
C#ifdef _NOSKIP
      ...
      If (Iuhf .EQ. 0) Then
         ...
         Call checksum("List-446;Z(Ai,Bj):",Work,Length)
      Else
         Length = idsymsz(Irrepx,isytyp(1,444),isytyp(2,444))
         Call Getall(Work,Length,Irrepx,444)
         Call checksum("List-444;Z(AB,IJ):",Work,Length)

         Length = idsymsz(Irrepx,isytyp(1,445),isytyp(2,445))
         Call Getall(Work,Length,Irrepx,445)
         Call checksum("List-445;Z(ab,ij):",Work,Length)

         Length = idsymsz(Irrepx,isytyp(1,446),isytyp(2,446))
         Call Getall(Work,Length,Irrepx,446)
         Call checksum("List-446;Z(Ab,Ij):",Work,Length)
      Endif
C#endif 
