...
C    **** experimental hacked up version ****

      SUBROUTINE MIRREPS(ICORE,MAXCOR,NBAS,IUHF,EVAL,MOSYMOK)
...
C IS SMALLER THAN THE CARTESIAN SET.  READ THAT VALUE NOW.
     ...
     &                 '{MIRREPS} <-- VSCF',I110-1,MAXCOR)
         ELSE
            CALL PUTREC(20,'JOBARC','EVALORDR',NBAS*IINTFP,
     &                  EVAL(1+(ISPIN-1)*NBAS))
c TRANSFORM THE EIGENVECTORS TO THE SIMPLE AO CARTESIAN BASIS
c       CALL T2FULAO(ICORE(I000),ICORE(I010),ICORE(I020),NBAS,NBAS,
c     &              NBASX,'SCFEVC'//SP(ISPIN)//'0',
c     &              'EVECAO_'//SP(ISPIN),
c     &              'SCFEVL'//SP(ISPIN)//'0',
c     &              'EVALORDR',1,0)
            CALL IZERO(ICORE,I110)
            CALL MIRRORB(ICORE(I000),ICORE(I010),
     &                   ICORE(I020),ICORE(I030),
     &                   ICORE(I040),NBAS,IRRSYM,ICORE(I060),
     &                   ICORE(I070),ICORE(I080),
     &                   ICORE(I090),ICORE(I100),
     &                   NATOMS,ISPIN,NBASX,MOSYMOK,IUHF)
         END IF

c     END DO ISPIN = 1, (IUHF+1)
      END DO

      CALL CHKMOSYM(IRRSYM,NBAS,IUHF,MOSYMOK)
