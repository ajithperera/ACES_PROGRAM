      ...
      subroutine uno_ref2(oneh,dens,fock,eval,evec,icore,maxmem,ldim1,
     &     ldim2,nbas,mxlssz1,repuls,etot,iuhf, xform,
     $     densnew, densscr, scra, scrb)
c     
...
c     
      ...
      IF(I030-I000.GT.MAXMEM) THEN
        CALL NOMEM('Make Fock matrix','{MKFOCK2} <-- UNO_REF2 <-- '//
...
c     
      ...
      if(i030-i000.gt.maxmem) then
         call nomem('qrhf energy','{mkener} <-- uno_ref2 <-- oofcor '//
