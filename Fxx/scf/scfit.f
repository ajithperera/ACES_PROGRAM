...
C
      IF(ISPIN.EQ.1)THEN
       CALL GETREC(20,'JOBARC','SCFEVCA0',NBAS*NBAS*IINTFP,COLD)
C     WRITE(6,1020)
C1020 FORMAT(' @SCFIT-I, Previous alpha MOs in SO basis. ')
C     CALL OUTPUT(COLD,1,NBAS,1,NBAS,NBAS,NBAS,1)
      ELSE
       CALL GETREC(20,'JOBARC','SCFEVCB0',NBAS*NBAS*IINTFP,COLD)
C     WRITE(6,1030)
...
C
      WRITE(6,1040) ISPIN,I
1040  FORMAT(' @SCFIT-I, COLD(T)*S*CNEW for spin ',I2,' symmetry ',I3)
...
CC
      if (scfks) then
       ...
       call mat_trans(1,0,evec((ispin-1)*ldim3+1),
     &                scrsqr,0)
...
C
      ...
      ENDIF
C
      IF(GSSLOK.OR.IFLAGS(10).EQ.2)THEN
C
