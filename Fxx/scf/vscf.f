      ...
      SUBROUTINE VSCF(ICORE,ICRSIZ,IUhf)
...
c            o open the 2-electron integral file
               itmp=-1
               iopt=0
CSSS               call opnord(itmp,iopt,fnord,luord)
               if (itmp.ne.0) then
...
C
      ...
      IF (IMEMGET .GT. ICRSIZ) CALL INSMEM("scf", IMEMGET,ICRSIZ)
      I0 = 1 
C
...
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS         if (imemget .lt. iflags (36)) then
CSSS            call oep_mem_init(MAXMEM, IRPPMEM, IMEMGET)
CSSS         endif
CSSS      endif
C
C      CALL ACES_MALLOC(IMEMGET,ICORE,I0)
C      IF (ICORE(1).EQ.0) THEN
C         WRITE(*,*) "@VSCF: Unable to allocate ",IMEMGET,
C     &              " words of memory for AO integral sorting."
C         CALL ERREX
C      END IF

c   o initialize the I/O subsystem for PK chaining
CSSS      if (.not. bioup) call aces_io_init(icore,i0,imemget,.true.)
CSSS      call aces_io_init(icore,i0,imemget,.true.)
...
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS         call oep_init(petite_list,iuhf,icore(i010),
CSSS     &                 (maxmem-i010+i000),loslater)
CSSS      endif
C
C PICK UP THE CORE HAMILTONIAN AND OVERLAP MATRICES
C
      I020=I010+ITRILN(NIRREP+1)*IINTFP
      I025=I020+ITRILN(NIRREP+1)*IINTFP
...
c
      dkhess  = (iflags2(167) .gt. 0)
      if (dkhess) then
         dkhorder=iflags2(167)
         ...
         call init_ks
         ipnull = setptr(1, 0, 1)
         ip010 = setptr(1, 0, ibufln)

         iznull = setptr(1, 1, 1)
         iz010 = setptr(1, 1, ibufln)
         iz020 = setptr(1, 1, itriln(nirrep+1))
         iz030 = setptr(1, 1, itriln(nirrep+1))
         iz040 = setptr(1, 1, itriln(nirrep+1))
         iz050 = setptr(1, 1, itriln(nirrep+1))
         iz060 = setptr(1, 1, mxirr2)
         iz070 = setptr(1, 1, mxirr2)
         iz080 = setptr(1, 1, mxirr2)
         iz090 = setptr(1, 1, mxirr2)
         iz100 = setptr(1, 1, isize)
         iz110 = setptr(1, 1, DKHZMAX)
     ...
     +                                              icore(i010) )
         ...
         call relptr(1, 1, iznull)
         call relptr(1, 0, ipnull)
      end if
c
...
c the indexing math must be changed to doubles.
     ...
     &                ICORE(I210),(I000+MAXMEM-I210)/IINTFP)
      END IF
C
c-------------------------------------------------------
      IF (IFLAGS2(153) .eq.1)  scfks = .TRUE.
      IF (IFLAGS2(153) .eq.2)  hfdft = .TRUE.
      IF (IFLAGS2(153) .ge.3)  oepks = .TRUE.
      IF (IFLAGS2(138).eq.1)  agrad = .TRUE.
      IF (IFLAGS(18).eq.1)        props = .TRUE.
...
c 07/02 Ajith Perera,
           ...
           izvalao = setptr(1,1,nbasx*ncount*intnumradpts
     &                                   *max_angpts)
           izvalgradao = setptr(1,1,nbasx*ncount*
     &                          intnumradpts*max_angpts*3)

           iztotwt = setptr(1,1,ncount*intnumradpts 
     &                      *max_angpts)    

           irrtritot= itriln(nirrep+1)*iintfp
           izkspot  = setptr(1, 1,nbasx*nbasx*(iuhf+1))
           iz1      = setptr(1, 0,          nbasx*2)
           izscrexc = setptr(1, 1,       nbasx*nbasx*3)
           izscr    = setptr(1,1,nbasx*nbasx*(iuhf+1))
           izksa    = setptr(1, 1,           irrtritot)
           izksb    = setptr(1, 1,           irrtritot)
...
c scfexcat in not working has not being incorporated  
   ...
   13       format(i5)
C
            izscr2   = setptr(1, 1, nbas*nbas)     
            iz2      = setptr(1, 1,nbasx*nbasx*numauxbas)
            izresp   = setptr(1,1,3*numauxbas*numauxbas)
          end if 
           else
           izvalao = znull                                     
           izvalgradao = znull                         
           ...
           iscre=setptr(1, 1, nbas*nbas)
           iscrq=setptr(1, 1, nbasx*nbas)
           iscrv=setptr(1, 1, nbas*(iuhf+1))
           iscrc=setptr(1,1, nbas*nbasx*2)
           iscra=setptr(1,1, nbasx*nbasx)
           iscrocc=setptr(1,0, nbas*2) 
           izend =setptr(1,1,1)
           ipend =setptr(1,0,1)
      end if
C
...
C  We also iterate over this section.  This is an SCF cycle.
      ...
      DMAX=0.D0
      ITER=0
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4)
CSSS     &    call oeptimer ('VSCF: PRE-SCF', 'OFF')
C
 2000 CONTINUE
       ITER=ITER+1
c
...
C
       transfer = .false.
CSSS       if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS           i070 = i060 + nbas*nbas*iintfp
CSSS           i080 = i070 + (iuhf+1)*isqrln(nirrep+1)*iintfp
CSSS           if (maxmem-i070+i000 .le. 0) call nomem("do oep iters.",
CSSS     &         " oep_dowork <-- vscf", i070-i000, maxmem)
CSSS           CALL PUTREC(20,'JOBARC','OCCUPYA0',NIRREP,NOCC(1))
CSSS           call oep_dowork(iter, iuhf, nbas, nocc, icore(i070), 
CSSS     &                     maxmem-i070+i000, i070-i000,
CSSS     &                     icore (i010), icore (i050), 
CSSS     &                     icore (i060), icore (i030), icore (i040), 
CSSS     &                     etot, MAX_ERROR, itertype, transfer,
CSSS     &                     slater,loslater,iSlaterON)
CSSS#ifdef _BUILD_KSPOT
CSSS        if(transfer) then 
CSSS         Call buildKSpot(dcore(izkspot),kscore(iz1),
CSSS     &                   dcore(izscrexc),dcore(izscr),
CSSS     &                   dcore(izvalao),dcore(izvalgradao),
CSSS     &                   dcore(iztotwt),
CSSS     &                   intnumradpts,max_angpts,natoms,
CSSS     &                   dcore(izksa),ICORE(I040),
CSSS     &                   nbasx,0,ITRILN(NIRREP+1))
CSSS       end if
CSSS#endif
CSSS       endif
...
c
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4)
CSSS     &    call oeptimer ('SCFITER: REST', 'ON')
c
...
C
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS         call oeptimer ('SCFITER: REST', 'OFF')
CSSS         call oeptimer ('VSCF: SCFITER', 'OFF')
CSSS      endif
c
       if (scfks) then
          call relptr(1,1,izend)
          call relptr(1,0,ipend)
       end if
C
      IF (NOCONV.AND.ITER.LT.MAXIT) GOTO 2000
C
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) 
CSSS     &    call oeptimer ('VSCF: POSTSCF', 'ON')
c
...
C
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS         call oep_post(iuhf, nbas, ibufln,luint,nocc, icore(i070),
CSSS     &                 maxmem-i070+i000, icore (i050), 
CSSS     &                 icore (i030), etot, dmax, itertype)
CSSS         call putrec(20, 'JOBARC', 'SCFEVCA0', nbas*nbas*iintfp, 
CSSS     &               Icore(i060))
CSSS         CALL MKRHFF(ICORE(I040),
CSSS     &               ICORE(I030),ICORE(I010),
CSSS     &               ICORE(I055),ICORE(I056),
CSSS     &               ITRILN(NIRREP+1),NBAS,NBFIRR,ICORE(I057),
CSSS     &               ITMP,LUINT,.TRUE.,
CSSS     &               nbasx,0,scfksiter,scfksexact,scfkslastiter,
CSSS     &               iter,
CSSS     &               dcore(izkspot),kscore(iz1),
CSSS     &               dcore(izksa),dcore(izscrexc),
CSSS     &               dcore(izscr),dcore(izscr2),dcore(izscreval),
CSSS     &               dcore(izvalao),dcore(izvalgradao),
CSSS     &               dcore(iztotwt),max_angpts,natoms,
CSSS     &               intnumradpts,ncount,kshf)
CSSS         call getrec(20, 'JOBARC', 'SCFEVLA0', nbas*iintfp, 
CSSS     &               icore(i050))
CSSS         call getrec(20, 'JOBARC', 'SCFEVCA0', nbas*nbas*iintfp, 
CSSS     &               Icore(i060))
CSSS         goto 5999
CSSS      endif
C
...
C (just like standard QRHF). 09/2015, Ajith perera. 
            ...
            If (IFLAGS(77).EQ.0) THEN
               CALL GETREC(-1,'JOBARC','OSCALC  ',1,IOS)
               CALL TEMPLATE_ORBS(ICORE(I010),ICORE(I030),ICORE(I040),
     &                           ICORE(I050),ICORE(I060),
     &                           ITRILN(NIRREP+1),
     &                           ISQRLN(NIRREP+1),NBAS,IUHF,IOS)
...
C
CSSS      if (iflags2 (153).eq.3 .or. iflags2 (153).eq.4) then
CSSS         call oeptimer ('VSCF: POSTSCF', 'OFF')
CSSS         call oeptimer ('VSCF', 'OFF')
CSSS      call oeptimer ('', 'REPORT')
CSSS      endif
c
