      ...
      subroutine uno_makerhf(pk,oneh,dens,fock,eval,evec,icore,maxmem,
...
c     
            ...
            enddo
            info = 0
c      call output(scra, 1, nsize, 1, nsize, nsize, nsize, 1)
...
c     
     ...
     $           nsize, scrb, nsize, 0.0d0, densscr, nsize)
            ...
            enddo
       write(6,*) ' @uno_makerhf: trace of transformed dens ' ,
     $           trace
         endif
...
c     
      ...
      enddo
c     
c     determine number of nominally closed and open shell electrons
c     
      do irp = 1 , nirrep
         iclosed(irp) =  min(nocc(irp,1), nocc(irp,2))
         iactive(irp) = abs(nocc(irp,1) - nocc(irp,2))
      enddo
c     
c     determine # of doubly occupied and open_shell electrons in uno_ref
c     
      nclosed = 0
      nopen = 0
      do irp = 1, nirrep
         nclosed = nclosed + iclosed(irp)
         nopen = nopen + iactive(irp)
      enddo
c     
c     adjust nocc:
c     
      do irp = 1, nirrep
         nocc(irp,1) = iclosed(irp)+iactive(irp)
         nocc(irp,2) = iclosed(irp)+iactive(irp)
         nvrt(irp,1) = nbfirr(irp) - nocc(irp,1)
         nvrt(irp,2) = nbfirr(irp) - nocc(irp,2)
      enddo
c     
...
c     
      ...
      nelec = nelec + icharge
c
c adjust nelec using value of uno_charge: The charge of ultimate reference system
c
      nelec = nelec - uno_charge
c
c     nelec denotes number of electrons in neutral system (i.e. same as nuclear charge)
c     This is only true for non_qrhf reoccupation ...  Perhaps adjust charge in QRHF.
c     
      n0_open = nelec - 2*nclosed ! number of fractional electrons for neutral system.
      If (nopen .NE. 0) Then
         fraction = n0_open / (2.0d0 * nopen)
      Else
         fraction = 1.0D0
      Endif
c     
      write(6,*) ' UNO_MAKERHF orbital information '
      write(6,660) nclosed
 660  format( '   # of closed shell orbitals : ', i6)
      write(6,662) nopen
 662  format( '   # of active orbials : ', i6)
      write(6,661) n0_open
 661  format( '   # of active electrons in neutral system: ', i6)
      write(6,663) n0_open, 2*nopen, fraction
 663  format( '   Fractional occupation neutral system: ',
     $     i6, '/', i6, ' = ', F14.6)
      write(6,*)
c     
      write(6,*) '  Populations closed shell orbitals '
      write(6,664) (iclosed(irp), irp=1,nirrep)
      write(6,*) '  Populations open shell orbitals '
      write(6,664) (iactive(irp), irp=1,nirrep)
      write(6,*) '  Populations occupied orbitals '
      write(6,664) (nocc(irp,1), irp=1,nirrep)
      write(6,*) '  Symmetries virtual orbitals '
      write(6,664) (nvrt(irp,1), irp=1,nirrep)
c     
 664  format(8x, 10I6)
c     
...
c     
            ...
            enddo
c     
c     set eval such that it is not found again.
            eval(icount0) = -2.0d0
c     
...
c     
c     next find open shell orbitals
c     
      ioff = 0
      do irp = 1, nirrep
         do iorb = 1, iactive(irp)
c     
c     find maximum occupation number
c     
            amax = -2.0d0 *fraction +1.0d-5
            icount = ioff
            do i = 1, nbfirr(irp)
               icount = icount + 1
               if (eval(icount) .gt. amax) then
...
c     
     ...
     $           1.0d0, densscr(isqrof(irp0)), nsize)
c     
c     also add to densfrac, multiplying by fraction
c     
...
c     
      ...
      enddo
c     
c     copy alpha to to beta part of density
c     
      do i = 1, ldim2
         densscr(ldim2+i) = densscr(i)
      ...
      enddo
c     
c     set remaining eval components that are larger than zero to zero
c     
      nsize = 0
      do irp = 1, nirrep
         nsize = nsize + nbfirr(irp)
      enddo
c     
      do i = 1, nsize
         if (eval(i) .ge. (-2.0d0*fraction+1.0d-4)) eval(i) = 0.0d0
      enddo
c     
      ...
      if (.true.) then
         ...
         do irp = 1, nirrep
            ...
            if (nsize .gt. 0) then
               ...
               do i = 1, nsize
                  tracea = tracea +
     $                 densfrac(isqrof(irp)+(i-1)*nsize+i-1)
               enddo
               do i = 1, nsize
                  traceb = traceb + densfrac(ldim2+
     $                 isqrof(irp)+(i-1)*nsize+i-1)
               enddo
               write(6,*) ' partial traces fractional density matrix ',
     $              tracea, traceb
               trace = trace + tracea + traceb
            endif
         enddo
         write(6,*) ' Total trace of fractional density ', trace
      endif
c     
c     new density matrix was formed in orthogonal basis. transform to ao basis
c     
      aotran = .true.
      if (.not. aotran)
...
c     
c     we have overwritten densfrac: Now: neutral fractional density in AO basis.
c     
...
c     
     ...
     $           irp, ioccb, ldim2+isqrof(irp)
         ...
         iacta = iocca + iclosed(irp) * nsize
         iactb = ioccb + iclosed(irp) * nsize
         ivrta = iocca + nocc(irp, 1) * nsize
         ivrtb = ioccb + nocc(irp, 2) * nsize
         do i =1, nsize
            if (abs(eval(icount) +2.0d0) .lt. 1.0d-5) then
c     
...
c     
               ...
               iocca = iocca + nsize
               ioccb = ioccb + nsize
            elseif (abs(eval(icount) + 2.0d0*fraction) .lt. 1.0d-5) then
c     
c     active occupied orbital
c     
               call scopy(nsize, scra(ivec), 1, evec(iacta), 1)
               call scopy(nsize, scra(ivec), 1, evec(iactb), 1)
               iacta = iacta + nsize
               iactb = iactb + nsize
            elseif (abs(eval(icount)) .lt. 1.0d-5) then
c     
...
c     
               ...
               write(6,*) ' @uno_makerhf: problem assinging evec '
               write(6,*) ' eval(icount), fraction ',
     $              eval(icount), fraction
               call errex
            endif
            ivec = ivec + nsize
            icount = icount + 1
...
c     
      ...
      write(6,5304) (iclosed(i), i=1,nirrep)
      write(6,5305) (iactive(i), i=1,nirrep)
...
c     
      ...
      ENDIF
      call putrec(20,'JOBARC','ICLOSED', nirrep, iclosed)
      call putrec(20,'JOBARC','IACTIVE', nirrep, iactive)
      call putrec(20,'JOBARC','IVIRTUAL', nirrep, nvrt(1,1))
c     
 5302 format(T8,'  Total Occupied Populations by irrep: ', 8(I3,2X))
 5304 format(T8,' Doubly Occupied populations by irrep: ', 8(I3,2X))
 5305 format(T8,' Active Occupied populations by irrep: ', 8(I3,2X))
c     
      if (.false.) then
c     
c     copy densnew to dens (original code)
     ...
     $                 dens(ldim1+itriof(irp)), nsize)
         ...
         enddo
      else                      ! copy fractional density to dens.
         do irp = 1, nirrep
            if (nbfirr(irp).ne. 0) then
               nsize=nbfirr(irp)
...
c     
c     calculate semicanonical orbitals
c     
      call active_smican(fock, scra, scrb, densscr, evec, eval,
     $     nocc, nvrt, iclosed, iactive, nirrep)
c     
c     now create full vacuum density and fock matrix
c
      write(6,*) ' @uno_makerhf: full vacuum fock matrix is created '
c
c     copy densnew to dens 
c
      do irp = 1, nirrep
         if (nbfirr(irp).ne. 0) then
            nsize=nbfirr(irp)
...
c     
     ...
     &     1,noconv,scfksiter,scfkslastiter)
c     
      write(luout,5010)etot
 5010 format(t3,'The Full vacuum reference energy is ',f20.10,/)
c     
c     at this point, the values needed for the uno_ref calculation have
c     been placed in the appropriate arrays and will be dumped out
c     with everything else in dmpjob.
...
c     
      write(6,*)
      write(6,*) ' @uno_makerhf: The reference state is changed',
     $     ' to closed shell'
      iuhf = 0
c     
c     update flags !!
