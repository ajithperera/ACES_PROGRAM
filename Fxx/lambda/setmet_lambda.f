...
C IFLGAS2(151):Resonance Raman 
     ...
     &                                 .OR.IFLAGS(18).EQ.13
     &                                 .OR.IFLAGS(18).EQ.11)
     &       .AND.IFLAGS2(5).EQ.0.AND.IFLAGS(54).EQ.0
     &       .AND.IFLAGS2(3).EQ.0
CMN
...
C LCCD and LCCSD (IFLAGS2(117)=6 and 7).
...
C METHOD = 10 (CCSD), = 5 (LCCD), = 6 (LCCSD), 4= 46 (CC2). 
C METHOD = 8  (CCD) 

       IF (METHOD .EQ.  10) THEN 
          write(6,3008)
          CCSD=.TRUE.
       ELSE IF (METHOD .EQ.5 .OR. METHOD.EQ.6 .OR. 
     &          METHOD .EQ. 47) THEN
          write(6,3008)
          CCSD=.TRUE.
          CC2 =.TRUE.
