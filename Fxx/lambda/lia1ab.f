      SUBROUTINE LIA1AB(W,DOO,XIA,ISPIN,POP1,VRT1,POP2,VRT2,
     &                  DISSYW,NUMSYW,LISTW,IRREP,IUHF)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DISSYW,DIRPRD,POP1,VRT1,POP2,VRT2
      DIMENSION W(DISSYW,1),DOO(1),XIA(1),POP1(8),VRT1(8),
     &          POP2(8),VRT2(8),IPD(8),IPW1(8),IPW2(8)
      COMMON /SYMINF/ NSTART,NIRREP,IRREPA(255,2),DIRPRD(8,8)
C
C      PICK UP THE INTEGRALS 
C
      CALL GETLST(W,1,NUMSYW,1,IRREP,LISTW)
C
      IF(ISPIN.EQ.2.OR.IUHF.EQ.0) THEN
C
C   OFFSETS FOR DENSITY MATRIX
C
      IPD(1)=0
      DO 1 IRREPJ=1,NIRREP-1
       IPD(IRREPJ+1)=IPD(IRREPJ)+POP2(IRREPJ)*POP2(IRREPJ)
1     CONTINUE
C
C  LOOP OVER IRREPS OF VIRTUAL ORBITAL A AND OCCUPIED ORBITAL I
C
C  OFFSETS
C
      IOFFWA=0
      IOFFWB=0
      IOFF=0
C
      DO 100 IRREP1=1,NIRREP
C
C  POPULATION CORRESPONDING TO A AND I
C
       NVRT1=VRT1(IRREP1)
       NOCC1=POP1(IRREP1)
C
C  DETERMINE IRREP OF M AND N
C
       IRREP2=DIRPRD(IRREP,IRREP1)
C
C  POPULATION WHICH CORRESPONDS TO M AND N
C
       NOCC2=POP2(IRREP2)
C
C  OFFSET IN D
C
       IOFFD1=IPD(IRREP2)
C
       DO 10 IA=1,NVRT1
       IOFFW2=IOFFWA+(IA-1)*NOCC2
       DO 10 I=1,NOCC1
       IOFFW1=IOFFWB+(I-1)*NOCC2
       IND=IA+(I-1)*NVRT1+IOFF
       DO 10 N=1,NOCC2
       IOFFW3=IOFFW2+N
       IOFFD2=IOFFD1+(N-1)*NOCC2
       XIA(IND)=XIA(IND)
     &       -SDOT(NOCC2,DOO(IOFFD2+1),1,W(IOFFW1+1,IOFFW3),1)
10     CONTINUE
       IOFFWA=IOFFWA+NOCC2*NVRT1
       IOFFWB=IOFFWB+NOCC2*NOCC1
       IOFF=IOFF+NVRT1*NOCC1
100    CONTINUE
 
C
      ELSE IF(ISPIN.EQ.1.AND.IUHF.NE.0) THEN
C
C  LOOP OVER IRREPS OF VIRTUAL ORBITAL A AND OCCUPIED ORBITAL I
C
C  OFFSETS
C
      IPD(1)=0
      DO 2 IRREPJ=1,NIRREP-1
       IRREPI=DIRPRD(IRREP,IRREPJ)
       IPD(IRREPJ+1)=IPD(IRREPJ)+POP1(IRREPJ)*VRT1(IRREPJ)
2     CONTINUE
C
      IOFFD=0
      IOFFWA=0
      IOFFWB=0
C
      DO 200 IRREP2=1,NIRREP
C
C  POPULATION CORRESPONDING TO N AND M
C
       NOCC2=POP2(IRREP2)
C
C  DETERMINE IRREP OF M AND N
C
       IRREP1=DIRPRD(IRREP,IRREP2)
C
C  POPULATION WHICH CORRESPONDS TO A AND I
C
       NOCC1=POP1(IRREP1)
       NVRT1=VRT1(IRREP1)
C
C  OFFSET IN XIA
C
       IOFF=IPD(IRREP1)
C
C
       DO 20 IA=1,NVRT1
       IOFFW2=IOFFWA+IA
       DO 20 I=1,NOCC1
       IOFFW1=IOFFWB+I
       IND=IA+(I-1)*NVRT1+IOFF
       DO 20 N=1,NOCC2
       IOFFW3=IOFFW2+(N-1)*NVRT1
       IOFFD2=IOFFD+(N-1)*NOCC2
       XIA(IND)=XIA(IND)
     &      -SDOT(NOCC2,DOO(IOFFD2+1),1,W(IOFFW1,IOFFW3),NOCC1)
20     CONTINUE
       IOFFWA=IOFFWA+NVRT1*NOCC2
       IOFFWB=IOFFWB+NOCC1*NOCC2
       IOFFD=IOFFD+NOCC2*NOCC2
200    CONTINUE
C
      ENDIF
      RETURN
      END
