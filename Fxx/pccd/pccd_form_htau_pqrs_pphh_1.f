      Subroutine Pccd_form_htau_pqrs_pphh_1(Work,Maxcor,List_v1,
...
C <ie|bm>*G(ae,jm)->H(bi,aj) <ie|bm>=<be,im> is stored as (bi,em) and 
C G(ae,jm) is stored as (ae,jm).Reorder to aj,em
C (to match with my new notes this is <je|am>*G(be,im) + <ej|am>*G(eb,im))
...
C G(ae,jm) stored as ae,jm and reorder to aj,em
...
C <ie|mb>*G(ae,mj) (<ie|mb> ordered as (bi,em) and G(mj,ae) is 
C ordered as (ae,mj). This is the direct contribution: spin-adapt the
C integral or G 
C (to match with my new notes this is <ej|am>*G(eb,im)). 
...
C <ie|mb>*G(ae,jm), Reordered to I(bi,em)*G(aj,em)
...
C <je|am>*G(be,im) + <ej|am>*G(be,im) 
C <ie|bm>*G(ae,jm) + <ie|mb>*G(ae,mj) Note that the second-contribution is
