...
C V(ek,af)*Gamma(ek,if)=V(ek,fa)*Gamma(ek,fi) (V is stored as fa,ek and G stored as ek,fi)
      ...
      Do Irrep_fi = 1, Nirrep
         ...
         I030 = I020 + Max(Nrow_ek,Ncol_fa)
         I040 = I030 + Max(Nrow_ek,Ncol_fa)
         Iend = I040 + Max(Nrow_ek,Ncol_fa)
...
C V(ek,fa) -> V(ek,af)
         ...
         Call Symtr1(Irrep_fa,Vrt(1,Ispin),Vrt(1,Ispin),Nrow_ek,
...
C V(ek,fa)(t)*G(ek,fi) -> S(a,i)
     ...
     +               Nsum,Work(Koff),Nsum,One,Hvo(Ioff),Nrow)
      ...
      ndim=(Nocc*Nvrt)**(1/2)
      call pccd_check_htau("Htau_vo :",Hvo,Ndim,"OV","D")

C V(ni,fm)*Gamma(na,fm)=V(fm,ni)[stored as V(ni,nf)]*Gamma(fm,na)
C V(in,fm)*Gamma(fm,na)
     ...
     +                                      Iend,Maxcor)
         ...
         If (Iend .Gt. Maxcor) Call Insmem("Pccd_form_htau_2d_hphp_ph",
     +                                      Iend,Maxcor)
C V(in,mf)-> V(in,fm) -> V(fm,in)->V(fm,ni)
     ...
     +                Work(I010),Work(I020),Work(I030),Work(I040))
          ...
          Call Symtr1(Irrep_ni,Pop(1,Ispin),Pop(1,Ispin),Ncol_fm,
     +                Work(I000),Work(I020),Work(I030),Work(I040))
