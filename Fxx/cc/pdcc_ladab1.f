      SUBROUTINE PDCC_LADAB1(W,T,Z,MAXSIZE,DISSYW,NUMSYW,DISSYT,NUMSYT,
     &                  LISTW,LISTT,ITYPE,IRREP,T1A,T1B,TAU,POP1,
     &                  POP2,VRT1,VRT2,ELADAB,MODIF,IUHF)
C
...
C
C PICK UP THE RELEVANT T2 AND W PIECES. FOr CC no T2 here but
C only T1 * T1. 
C 
      If (Ispar) Then
...
C
      IF(ITYPE.EQ.6)THEN
       MODIF='abef'
c       IF(IUHF.EQ.0.AND..NOT.GRAD) THEN
...
C
       if (NUMSYT*DISSYT.LT.0) call trap_intovf('LADAB1',1)
