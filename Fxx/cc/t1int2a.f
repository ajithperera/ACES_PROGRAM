      ...
      SUBROUTINE T1INT2A(ICORE,MAXCOR,IUHF)
C
C THIS PROGRAM CALCULATES THE TERM
C
C - SUM M ( <MB//IJ> T(M,A) + <AM//IJ> T(M,B)  
C
C SYMMETRY ADAPTED 
C 
C
C IN RHF :
C
C - SUM m <Ij//Am> T(m,b) + TRANSPOSITION               
C
C IN UHF
C
C - SUM M <IJ//AM> T(M,B) - ANTISYMMETRIZATION IN A,B
C
C - SUM m <ij//am> T(m,a) - ANTISYMMETRIZATION IN a,b
C
C - SUM m <Ij//Am> T(m,b) - SUM M <Ij//Mb> T(M,A)
C
CEND
C
C CODED JG JUNE/90
...
C
     ...
     &                 ICORE(I002),ICORE(I000),POP(1,ISPIN),
     &                 VRT(1,ISPIN),DISSYZ,DISSYW,DISSYW,NUMSYW,NVRTSQ,
