      ...
      SUBROUTINE GAMMA6_DENS(ICORE,MAXCOR,IUHF)
C
C
...
C
      ...
      if(iuhf.eq.0) then
       call checkgam1_der(icore,10,110,two,iuhf,2,pop)
      endif
      IF(IUHF.EQ.1) THEN
       CALL CHECKGAM_der(ICORE,10,110,TWO)
       CALL CHECKGAM_der(ICORE,7,107,TWO)
       CALL CHECKGAM_der(ICORE,8,108,TWO)
       CALL CHECKGAM_der(ICORE,9,109,TWO)
      ENDIF
C
      RETURN
      END
