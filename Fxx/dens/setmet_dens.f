...
C     
CSSS      GRAD=(IFLAGS2(138).EQ.1  .AND.
CSSS     &      (IFLAGS2(5).ne.0  .OR.
CSSS     &       IFLAGS(54).EQ.3)        .OR. 
CSSS     &      (IFLAGS(54).EQ.1  .OR.
CSSS     &       IFLAGS(3) .GT.1 .OR.
CSSS     &       IFLAGS2(138).EQ.1))
...
C        This is trapped in joda and is only for redundancy.
         Write(6,*) "The unrelaxed density is not allowed"
         Write(6,*) "for this type of calculation."
         Call Errex
      Endif
C     
...
C     
     ...
     &                " density matrices."
      ENDIF 
c OLD
c     IF(IFLAGS(38).NE.0.AND.IFLAGS(11).NE.2) THEN
c NEW
