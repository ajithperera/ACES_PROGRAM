      ...
      SUBROUTINE LINEQ_4STD_NONCAN_UHF(AMAT,XIA,XIANEW,XUPDATE,ASMALL,
     &                   ASQUARE,
     &                   EVALA,EVALB,FAB,FIJ,FAJ,WMATOA,WMATVA,
     &                   WMATOB,WMATVB,XSCR,LENAB,LENIJ,LENAJ,
     &                   CONV,NA,NB,KMAX,NOCCA,NOCCB,
     &                   INCORE,IUHF)
C
C  THIS PROGRAM SOLVES THE LINEAR EQUATION for non-canonical 
C  problmes (F(Ij) .NE. 0; F(ab) .NE. 0). 
C
C  SUM B,J Z(B,J) (A(BJ,AI) + (FBA-FIJ)) = X(AI)
C  FBA = FBA*Delta(IJ), FIJ = FIJ*Delta(AB)
C
...
C
      ...
      ELSE
         CALL DCOPY(NF1(1), FIJ(1), 1, FIJ(NF1(1)+1), 1)
      ENDIF 
      CALL GETLST(FAB(1),1,1,1,5,92+ISHIFT)
      IF (IUHF .NE. 0) THEN
         CALL GETLST(FAB(NF2(1)+1),1,1,1,6,92+ISHIFT)
      ELSE
         CALL DCOPY(NF2(1), FAB(1), 1, FAB(NF1(2)+1), 1)
      ENDIF 
      CALL GETLST(FAJ,1,1,1,5,93+ISHIFT)
      IF (IUHF .NE. 0) THEN
...
C
     ...
     &             NA,ONE,XIANEW(1+NA),1)
        CALL ADDFX(XIA,XIANEW,FAB,FIJ,FAJ,LENAB,LENIJ,LENAJ,
     &             NA,NB)
        Write(6,*) "After correcting for F elements"
        Write(6,"(5(F20.10))") (Xia(i), i=1, na)
        Write(6,*) 
        Write(6,"(5(F20.10))") (Xianew(i), i=1, na)
      ELSE
