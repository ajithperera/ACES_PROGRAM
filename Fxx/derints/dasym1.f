      ...
      SUBROUTINE DASYM1(CAODER,AAODER,WORK1,KFDAO,NATOMC,
...
c     MXCONT - ???
      ...
      DIMENSION CAODER(1), AAODER(1), WORK1(1),KFDAO(8)
      LOGICAL FULMAT
      CHARACTER NAMEX*6
...
C
      DO 20 JCDIR = 1,3
         ...
         ISYTYJ = ISYTYP(JCDIR)
         IF (ICL .EQ. 3) THEN
            DO 50 I=1,NINTS,NATOMC
             CAODER(IOFF+I-1)=CAODER(IOFF+I-1)+AAODER(IOFF+I-1)
50          CONTINUE
         ENDIF
C
...
C
     ...
     *              -IPERT(IREPD+1)-1)*NLENT(IREPD+1)
            IF (IREPD .EQ. 0) THEN
               IF (ICL .EQ. 2 .AND. .NOT.ONECEN) THEN
                  CALL SYM1S(AAODER(IOFF),WORK1(KFDAO(IREPD+1)
     ...
     *                       THRESH,IMAT0,NATOMC,IPRINT,ONE)
            ...
            ELSE
               IF (ICL .EQ. 2 .AND. .NOT.ONECEN) THEN
                  CALL SYM1N(AAODER(IOFF),WORK1(KFDAO(IREPD+1)
