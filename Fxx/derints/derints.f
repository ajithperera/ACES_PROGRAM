...
C  UNIVERSITY OF KARLSRUHE,  AUGUST 91    J. GAUSS, J.F. STANTON
      ...
      SUBROUTINE DERINTS(ICORE,ICRSIZ,IUHF)
...
C
      ...
      IF (IFLAGS(h_IFLGAS_calc) .GT. 0) CALL ZERO(ICORE(I0),MAXCOR)
C
...
C
       IF((.NOT.ELECT).AND.GRAD) THEN
        CALL NUCREP_DERINTS(ICORE(IENN),ICORE(IGNN),ICORE(IHNN),
     +                      NCOORD)
       ENDIF
C
      ELSE IF(MAGNET) THEN
