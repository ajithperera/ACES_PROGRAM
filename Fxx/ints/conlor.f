      ...
      SUBROUTINE CONLOR (F,SABB,SCDD,NPL,AINT,XLFT,XRGT,E1,
     $ SAVROT,TMUL,ROOT,NND,NNE,MAXLOT,I2,A2,I1,A1)
...
C-----------------------------------------------------------------------
     ...
     $ XRGT(NNE,MAXLOT,3),E1(2),SAVROT(2),TMUL(2),ROOT(2)
...
C     ROUTINE FOR GENERAL CONTRACTION CASE
   ...
   70 CONTINUE
      ...
      I1(11)=I1(10)+3*NPW
CJFS
C     I2(7)=I2(6)+(NHKTA+NHKTB+NHKTC+NHKTB)*NAQRS
C 1/23/97. Suggestion from Thomas Steinke to JDW.
      I2(7)=I2(6)+(NHKTA+NHKTB+NHKTC+NHKTD)*NAQRS
      if(i2(7).gt.ihwm2)then
        write(6,1010)
1010    format(T3,'@CONLOR-F, VMOL memory error.')
        call errex
       endif
C
C peter szalay's second contribution!
c
...
C
      ...
      DO 40 I=1,3
      IF(IFPL(I)) GO TO 40
      CALL PQ (E1,PV(1,KQ,I),NUCAQ,QV(1,IVA,I),NUCRS,XAND(I,KR,2))
