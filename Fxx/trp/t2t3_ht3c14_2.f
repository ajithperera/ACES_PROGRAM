      ...
      SUBROUTINE T2T3_HT3C14_2(W,ICORE,IADW,IRPI,IRPJ,IRPK,IRPIJ,
     &                         IRPIK,IRPJK,IRPIJK,I,J,K,ISPIN,LAMBDA)
...
C
      ...
      IRPCD = DIRPRD(IRPA,IRPIJK)
c YAU : old
c     CALL ICOPY(IINTFP*VRT(IRPA,ISPIN)*IRPDPD(IRPCD,ISPIN),
c    1           ICORE(IADW(IRPA)),1,W(IADW(IRPA)),1)
c YAU : new
      CALL DCOPY(VRT(IRPA,ISPIN)*IRPDPD(IRPCD,ISPIN),
     1           ICORE(IADW(IRPA)),1,W(IADW(IRPA)),1)
c YAU : end
   20 CONTINUE
C
C      AB  AB                      ACD
...
C    1     VRT(IRPB,ISPIN),1,IRPBK,26+ISPIN)
   40 CONTINUE
C
...
C
      ...
      CALL ZERO(ICORE(IADT2),IRPDPD(IRPIJ,ISPIN))
C
C     IN C1 LOOK AT WHAT GOES INTO EXPVV AND WHAT COMES OUT.
C
C     CALL OUTPUT(ICORE(IADT2E(1)),1,VRT(1,1),1,VRT(1,1),VRT(1,1),
C    1            VRT(1,1),1)
      CALL EXPVV(ICORE(IADT2),ICORE(IADT2E(1)),IADT2E,
     1           IOFFVV(1,1,ISPIN),IRPIJ,ISPIN)
C     CALL OUTPUT(ICORE(IADT2E(1)),1,VRT(1,1),1,VRT(1,1),VRT(1,1),
C    1            VRT(1,1),1)
C     DO   61 AB=1,IRPDPD(IRPIJ,ISPIN)
C     WRITE(6,*) ICORE(IADT2 + AB - 1)
C  61 CONTINUE
C
C     READ T2 INCREMENT, ADD CURRENT PIECE, AND WRITE SUM BACK TO
C     DISK.
C
      IADT2I = IADT2 + IRPDPD(IRPIJ,ISPIN)
      IF(IRPIJ.NE.1)THEN
...
C    1     VRT(IRPB,ISPIN),1,IRPBJ,26+ISPIN)
  140 CONTINUE
C
C     SET ADDRESSES FOR EXPANDED T2 MATRICES. (A,B)
C
...
C
      IADT2 = IADT2E(NIRREP) + LENT2E(NIRREP)
      CALL ZERO(ICORE(IADT2),IRPDPD(IRPIK,ISPIN))
      CALL EXPVV(ICORE(IADT2),ICORE(IADT2E(1)),IADT2E,
     1           IOFFVV(1,1,ISPIN),IRPIK,ISPIN)
C
C     READ T2 INCREMENT, ADD CURRENT PIECE, AND WRITE SUM BACK TO
C     DISK.
C
      IADT2I = IADT2 + IRPDPD(IRPIK,ISPIN)
      IF(IRPIK.NE.1)THEN
...
C    1     VRT(IRPB,ISPIN),1,IRPBI,26+ISPIN)
  240 CONTINUE
C
C     SET ADDRESSES FOR EXPANDED T2 MATRICES. (A,B)
C
...
C
      IADT2 = IADT2E(NIRREP) + LENT2E(NIRREP)
      CALL ZERO(ICORE(IADT2),IRPDPD(IRPJK,ISPIN))
      CALL EXPVV(ICORE(IADT2),ICORE(IADT2E(1)),IADT2E,
     1           IOFFVV(1,1,ISPIN),IRPJK,ISPIN)
C
C     READ T2 INCREMENT, ADD CURRENT PIECE, AND WRITE SUM BACK TO
C     DISK.
C
      IADT2I = IADT2 + IRPDPD(IRPJK,ISPIN)
      IF(IRPJK.NE.1)THEN
