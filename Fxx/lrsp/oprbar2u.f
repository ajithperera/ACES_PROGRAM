...
C
      SUBROUTINE OPRBAR2U(LISTMBAR,MABBAR,MIJBAR,MIABAR,
     &                    MABIJBAR,ICORE,MAXCOR,IUHF,IRREPX,
     &                    ISIDE,MBAR00,IOFFAB,IEOMPROP)
C
...
C
      ...
      DOUBLE PRECISION MABBAR(*),MIJBAR(*),MIABAR(*),MABIJBAR(*)
...
C comment above. IEOMPROP = 1 John's original implementation.
C IEOMPROP = 0 Hideo's modification.
C
       CALL ZERO(ICORE(I000),NUMDSZ*DISSYZ)
       IF(IRREPX .EQ. 1 .AND. ISIDE .EQ. 2 .AND. IEOMPROP .EQ. 0) THEN
...
C
     ...
     &             ICORE(I040),ICORE(I050),ICORE(I060))
C
C Disconnected term for left hand state [L(A,I)*Mbar(j,b)]. 
C IEOMPROP = 0 John's original implementation
C IEOMPROP = 1 Hideo's modification.
C This term is always connected should never be thrown out.
C
...
C
      IF(IFLAGS(1) .GE. 20) THEN
         WRITE(6,500) 
 500     FORMAT(T3, '@OPRBAR2R-I, Checksum of transformed matrix ',
     &          'elements:')                
         isize = idsymsz(irrepx, isytyp(1,lsttmp),isytyp(2,lsttmp))
         write(6,*)'(AI,bj) = ',sdot(isize, icore(i000),
     &                           1, icore(i000), 1)
      ENDIF
C
...
C
      Write(6,*)
      If (Iside .eq.1) call checksum("oprbar2ur :",Icore(I010),
     +                               Isize)
      If (Iside .eq.2) call checksum("oprbar2ul :",Icore(I010),
     +                               Isize)
      CALL PUTALL(ICORE(I010), ISIZE, IRREPX,446)
C
C      CALL SCOPY (ISIZE,ICORE(I010),1,MABIJBAR(IOFFAB(3)),1)
C
C NOW DO AAAA AND BBBB SPIN CASES
C
...
C
C Disconnected term for left hand state [L(A,I)*Mbar(J,B)]. 
C The full contribution is always connected and should always 
C included in John's and Hedio's schemes. 
C
        IF(IRREPZR .EQ. IRREPX .AND. ISIDE .EQ. 2)
     &     THEN
...
C
     ...
     +                               Isize2)
      If (Iside .eq.2) call checksum("oprbar2ul :",Icore(I010),
     +                               Isize2)
       CALL PUTALL(ICORE(I010),ISIZE2,IRREPX,443+ISPIN)
C
C       CALL SCOPY (ISIZE2,ICORE(I010),1,MABIJBAR(IOFFAB(ISPIN)),1)
C
200   CONTINUE
