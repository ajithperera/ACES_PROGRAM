...
C
C Get number of symmetry adapted basis functions and number of
C molecular orbitals
C
      Call Getrec(20, 'JOBARC', 'NBASTOT ', Ione, Nbasis)
      Nbasmo = Nocco(1) + Nvrto(1)
C
      Do 10 I = 1, Nirrep
...
C
 ...
 5020       Format(22X, A, 1X, A, A)
     ...
     &                   Icore(I080))
C
C Build the Fock-matrix in localized orbital basis to be used 
C in correlated calculations. Also write the localized MO's into
C JOBARC file
C
            Call MkFock(Iuhf, Spntyp(I), Nbasis, Nbasmo, Icore(I000),
...
C
 ...
 5030    Format(30X, A) 
     ...
     &                   Icore(I080))
C
C Build the Fock-matrix in localized orbital basis to be
C used in correlated calculations. Also write the localized MO's 
C into JOBARC file for RHF runs
C 
         Call MkFock(Iuhf, Spntyp(1), Nbasis, Nbasmo, Icore(I000), 
     &               Icore(I010), Icore(I020), Icore(I030), 
     &               Icore(I040))
C
      Endif
...
C
C Copy the virtuals in to Scr1 
C
      Nvirt = Nbasis - Norbs
...
C 
      Write(Luout, *)
      Write(Luout, *)
      Write(Luout, 5080) 'THE LOCALIZATION TRANSFORMATION MATRIX'
 5080 Format(20X, A)
