      SUBROUTINE LOAD4(CMO,W,W2,BUF,IBUF,ISYMAO,NBAS,
     &                 NOCC,NSTART,NEND,IOFFAO,IOF4,ISIZE3,
     &                 ISIZE2,NSIZE,ILNBUF,ISPIN,LUINT,LAST,
     &                 IOFFSET)
C
...
C
      ...
      DIMENSION NBAS(8),NOCC(8),IOFFAO(8),IOF4(8,8)
...
C
        ...
        DO 9001 IOCC=NSTARTX4,NENDX4
         W2(IOCC)=CMO(IOFFC4+L-1+(IOCC-1)*NBASX4)*X 
9001    CONTINUE 
C
...
*VOCL LOOP,NOVREC
CDIR$ IVDEP
        DO 9002 IOCC=NSTARTX4,NENDX4
         IADR=IOFFW44+K+NBASX3*(IND-1)+ISIZE34*(IOCC-NSTARTX4)
...
C
        IND=INDXF(II,JJ,NBAS1)+ISIZE2(IRREPX3,IRREPX1,IRREPX2)
        DO 9003 IOCC=NSTARTX3,NENDX3
         W2(IOCC)=CMO(IOFFC3+K-1+(IOCC-1)*NBASX3)*X
9003    CONTINUE 
C
...
*VOCL LOOP,NOVREC
CDIR$ IVDEP
        DO 9004 IOCC=NSTARTX3,NENDX3
         IADR=IOFFW43+L+NBASX4*(IND-1)+ISIZE33*(IOCC-NSTARTX3)
         W(IADR)=W(IADR)+W2(IOCC)
9004    CONTINUE
C
...
C      
      RETURN
900   WRITE(6,1000)
1000  FORMAT(T3,'@LOAD4-F, Unexpected end-of-file on integral file.')
      CALL ERREX
      RETURN
901   WRITE(6,1000)
1001  FORMAT(T3,'@LOAD4-F, I/O error on unit ',I5,'.')
      CALL ERREX
      RETURN
      END
