      SUBROUTINE IV3ALL(G,W,MAXCOR,AIVV,FACT,ISPIN,POP,VRT,
     &             DISSYT,NUMSYT,LISTG,LISTW,IRREP,TMP,icall,bRedundant)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DISSYT,DIRPRD,POP,VRT
      LOGICAL bRedundant, L23TO26
      DIMENSION G(DISSYT,1),W(DISSYT,1),AIVV(1),POP(8),VRT(8),TMP(1)
...
C
     ...
     X    CALL GETGO3(G,W,1,NUMSYT,1,IRREP,LISTG,ispin,listw,dissyt)
       if (icall.eq.2) 
     X    CALL GETGO3R(G,W,1,NUMSYT,1,IRREP,LISTG,ispin,listw,dissyt,2)
       if (icall.eq.3) 
     ...
     &            TMP(1+2*DISSYT))
      ...
      ELSE
         CALL GETLST_NR(W,TMP,MAXCOR*IINTFP,LISTW,IRREP)
      ENDIF

      CALL SYMTR1(IRREP,VRT,POP,DISSYT,W,TMP,TMP(1+DISSYT), 
     &            TMP(1+2*DISSYT))
C
...
C
        CALL XGEMM('T','N',NVRTJ,NVRTJ,DISSYT*NOCCI,FACT,
