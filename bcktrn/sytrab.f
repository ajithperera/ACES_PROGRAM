      ...
      SUBROUTINE SYTRAB(GAB,GBA,CVA,CVB,COA,COB,NAOA,NAOB,
...
CEND
      ...
      DIMENSION GAB(NVA,NOB),GBA(NVB,NOA),BUF(1),RESULT(NAOA*NAOB)
C
      DATA ZILCH /0.0/
      DATA ONE   /1.0/
C
      IF(IACC.EQ.0)CALL ZERO(RESULT,NAOA*NAOB)
C
      IF(MIN(NVA,NOB).NE.0)THEN
       CALL XGEMM('N','N',NAOA,NOB,NVA,ONE,CVA,NAOA,GAB,NVA,
     &            ZILCH,BUF,NAOA)
       CALL XGEMM('N','T',NAOA,NAOB,NOB,ONE,BUF,NAOA,COB,NAOB,
     &            ONE,RESULT,NAOA)
      ENDIF
C
     ...
     &            ZILCH,BUF,NAOA)
       CALL XGEMM('N','T',NAOA,NAOB,NVB,ONE,BUF,NAOA,CVB,NAOB,
     &           ONE,RESULT,NAOA)
      ENDIF
C
      RETURN
