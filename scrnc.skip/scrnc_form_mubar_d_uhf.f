...
C
      SUBROUTINE SCRNC_FORM_MUBAR_D_UHF(MIJBAR,MABBAR,MIABAR,ICORE,
...
C
      ...
      ELSEIF (ISIDE .EQ. 2) THEN
          CALL PUTALL(ICORE(I010), ISIZE, IRREPX,456)
      ENDIF 
C
...
C
     ...
     &              SPCASE(ISPIN),IRREPX)
       If (iside .eq.1)  call Checksum("scrnc_form_mubar_d_uhf_r(f)  :",
     +                                  icore(I010),isize2)
       If (iside .eq.2)  call Checksum("scrnc_form_mubar_d_uhf_l(f)  :",
     +                                  icore(I010),isize2)
C
...
C
       ...
       ELSEIF (ISIDE .EQ. 2) THEN
           CALL PUTALL(ICORE(I010),ISIZE2,IRREPX,453+ISPIN)
       ENDIF 
