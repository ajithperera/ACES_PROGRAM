...
C
      SUBROUTINE SCRNC_FORM_MUBAR_D_RHF(MIJBAR,MABBAR,MIABAR,ICORE,
...
C
     ...
     &               MIJBAR(IOFFM),NCOL,ZILCH,ICORE(IOFFZ),NROW)
         ENDIF
110     CONTINUE
C
...
C
     ...
     &             VRT(1,1),POP(1,1),ICORE(I020),IRREPX,'1324')
       If (iside .eq.1)  call Checksum("scrnc_form_mubar_d_rhf_r(f)  :",
     +                                  icore(I010),isize)

       If (iside .eq.2)  call Checksum("scrnc_form_mubar_d_rhf_l(f)  :",
     +                                  icore(I010),isize)
C
