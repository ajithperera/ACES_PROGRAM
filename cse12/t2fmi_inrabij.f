      ...
      Subroutine T2fmi_inrabij(T2aa_old,T2bb_old,T2ab_old,T2resid_aa,
...
C Rijab_aa(ab,ij) <- Fmi_aa(M,J)*T2aa(A,B,I,M)
      ...
      Do A = 1, Nvrt_a
         T = 0.0D0
      Do M = 1, Nocc_a 
         T = T  + Fmi_aa(M,J)*T2aa_old(A,B,I,M)
      Enddo 
...
C Rijab_bb(ab,ij) <- Fmi_bb(M,J)*T2bb(A,B,I,M)
      ...
      Do A = 1, Nvrt_b
         T = 0.0D0
      Do M = 1, Nocc_b
         T = T + Fmi_bb(m,j)*T2bb_old(a,b,i,m)
      Enddo
...
C Rijab_ab(ab,ij) <- Fmi(a,b)(M,J)*T2ab(A,B,I,M)
      ...
      Do A = 1, Nvrt_a
         T = 0.0D0
      Do M = 1, Nocc_b
         T = T + Fmi_bb(m,j)*T2ab_old(A,b,I,m)
      Enddo
      ...
      Do A = 1, Nvrt_a
         T = 0.0D0 
      Do M = 1, Nocc_a
         T = T + Fmi_aa(M,I)*T2ab_old(A,b,M,j)
      Enddo
         T2resid_ab(A,b,I,j) = T2resid_ab(A,b,I,j)  - T
      Enddo
      Enddo
