...
C READ W INTO W(Ij,Mb) AND REORDER TO W(Ij,bM)
        ...
        call checksum("Wiajk3 :",ICORE(I010), NUMDSW*DISSYW,S)
        ITMP1=I000
...
120     CONTINUE
         Call checksum("ZIjAb1 :",ICORE(I000),NUMDSZ*DISSYZ,s)
C
       ELSE
C
...
C
        ...
        IF(I020.LT.MXCOR)THEN
         CALL GETLST(ICORE(I010),1,NUMDSW,1,IRREPW,LISTW)
        call checksum("Wiajk4 :",ICORE(I010), NUMDSW*DISSYW,S)
C
C COMPUTE OFFSETS FOR W AND Z VECTORS
C
         IW=0
...
150      CONTINUE
C
         Call checksum("ZIjAb2 :",ICORE(I000),NUMDSZ*DISSYZ,s)
C REORDER TARGET BACK TO Z(Ab,Ij)
C
         CALL TRANSP(ICORE(I000),ICORE(I010),DISSYZ,NUMDSZ)
c YAU : old
...
C
         CALL GETLST(ICORE(I010),1,NUMDSW,1,IRREPW,LISTW)
      if (ispin.eq.1)call checksum("Wiajk1 :",ICORE(I010), 
     &                              NUMDSW*DISSYW,S)
      if (ispin.eq.2)call checksum("Wiajk2 :",ICORE(I010), 
     &                              NUMDSW*DISSYW,S)
         ITMP1=I000
         ITMP2=ITMP1+IINTFP*MAXW
         ITMP3=ITMP2+IINTFP*MAXW
...
c YAU : new
         CALL DCOPY(NUMDSZ*DISSYZ,ICORE(I010),1,ICORE(I000),1)
         Call checksum("Zijab  :",ICORE(I000),NUMDSZ*DISSYZ,s)
c YAU : end
         itmp = NUMDSZ*DISSYZ
         if (itmp.gt.0) then
