...
C T2-ABIJ
     ...
     +                                   ISYTYP(2,62))
      ...
      Call checksum("T2-AAAA :",Work,Length_61,S)
      If (Iuhf .ne. 0) Then
      Call Getall(Work, Length_62, Irrepx, 45)
      Call checksum("T2-BBBB :",Work,Length_62,S)
      Endif 
      Call Getall(Work, Length_63, Irrepx, 46)
      Call checksum("T2-ABAB :",Work,Length_63,S)

C T1-AI
      Length_aa =Irpdpd(Irrepx,9)
      call getlst(Work,1,1,1,3,90)
      Call checksum("T1-AA   :",Work,Length_aa,S)
      If (Iuhf .Ne. 0) Then 
      Length_bb =Irpdpd(Irrepx,10)
      call getlst(Work,1,1,1,4,90)
      Call checksum("T1-BB   :",Work,Length_bb,S)
      Endif 
      Write(6,*) 
