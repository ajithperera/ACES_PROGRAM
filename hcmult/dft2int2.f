...
C
     ...
     &              ICORE(I040),ICORE(I050),ICORE(I060))
       ...
       ELSE
       Call checksum("Fmi->Z2:",ICORE(I000),NUMDSZ,s)
C
C DO TRANSPOSITIONS: 
C 
C      Z(AM,jb) -> Z(jb,AM) 
...
c YAU : end
        CALL SYMTR1(IRREPTR,POP(1,2),VRT(1,2),DISSYT,ICORE(I000),
     &              ICORE(I040),ICORE(I050),ICORE(I060))
        Call checksum("Fae->Z2:",ICORE(I000),NUMDSZ,s)
        CALL PUTLST(ICORE(I000),1,NUMDSZ,1,IRREPZR,LISTZ)
C
       ENDIF
C
...
C
      DO 500 ISPIN=1,2
       DO 510 IRREPZR=1,NIRREP
        ...
        Call checksum("Hbar_ae:",ICORE(I020),NFMI(ISPIN),s)
        Call checksum("Hbar_mi:",ICORE(I030),NFEA(ISPIN),s)
        CALL ZERO  (ICORE(I000),DISSYZ*NUMDSZ)
C
C CARRY OUT  CONTRACTION :
C
