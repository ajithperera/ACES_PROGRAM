      ...
      SUBROUTINE Tdee_built_respn_den(ICORE, MAXCOR, IUHF) 
C
...
C
      CALL TDEE_DENS_OO(ICORE(IDOO), ICORE(IO), MXCOR, IUHF)

C Compute the virtual-virtual block of the density matrix.
C      
       CALL TDEE_DENS_VV(ICORE(IDVV),ICORE(IO),MXCOR,IUHF)

C
C Compute the direct contribution to the virtual-occupied block of the 
C density matrix
C
       CALL TDEE_DENS_VO(ICORE(IDOV),ICORE(IO),MXCOR,IUHF)

CSSS       CALL DUMPIT(ICORE(IDOO),ICORE(IDVV),ICORE(IDOV),ICORE(IIOV),
CSSS     &              IUHF)
       call checksum("Doo(AA)",ICORE(IDOO),ND1AA,s)
       if (iuhf.ne.0) call checksum("Doo(BB)",ICORE(IDOO+ND1AA),ND1BB,s)
       call checksum("Dvv(AA)",ICORE(IDVV),ND2AA,s)
       if (iuhf.ne.0) call checksum("Dvv(BB)",ICORE(IDVV+ND2AA),ND2BB,s)
       call checksum("Dvo(AA)",ICORE(IDOV),NTAA,s)
       if (iuhf.ne.0) call checksum("Dvo(BB)",ICORE(IDOV+NTAA),NTBB,s)
C 
C Write out the reduced density matrix to the disk.
C
       CALL TDEE_SAVD(ICORE(IDOO),ICORE(IDOO+IINTFP*ND1AA),
