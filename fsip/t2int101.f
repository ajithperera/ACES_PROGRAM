      SUBROUTINE T2INT101(ICORE,MAXCOR,IUHF)
C     
C     THIS SUBROUTINE CALCULATES THE CONTRIBUTION OF T2 TO
C     T1F (T1<-T2) FOR THE 0,1 SECTOR OF FOCK SPACE.
C     
C     
C     T(IJ) = SUM  T(MN,IE) * <MN||JE> + T(Mn,Ie) * <Mn|Je> [AA]
C       AI    mne    NN AN     NN  IN      NN AN     NN IN
C     
C     T(ij) = SUM  T(mn,ie) * <mn||je> + T(mN,iE) * <mN|jE> [BB]
C       AI    mne    NN AN     NN  IN      NN AN     NN IN
C     
C     T(IJ) = SUM  [2 T(Mn,Ie) - T(Nm,Ie)] <Mn|Je> [SPIN ADAPTED RHF] 
C       AI    mne       NN AN      NN AN    NN IN
C
...
C
      RHF=IUHF.EQ.0
      LENTAR=(NF1AI(1)+MIN(IUHF,1)*NF1AI(2))
      I000=1
...
C     
        IF(ISPIN.EQ.1)THEN
         CALL FSGET(ICORE(I010),1,NUMDSW,1,IRREP,LISTW,'NNIN')
         CALL FSGET(ICORE(I020),1,NUMDST,1,IRREP,LISTT,'NNAN')
         MAXW=MAX(NUMDST,NUMDSW,DISSYT,DISSYW)
         ITMP1=I030
         ITMP2=ITMP1+IINTFP*MAXW
         ITMP3=ITMP2+IINTFP*MAXW
         ITMP4=ITMP3+IINTFP*MAXW
         IF(RHF)THEN
          CALL SPINAD3(IRREP,POP(1,1),DISSYT,NUMDST,ICORE(I020),
     &                 ICORE(ITMP1),ICORE(ITMP2))
         ENDIF
         CALL SYMTR1(IRREP,POPI(1,1),VRT(1,2),DISSYW,ICORE(I010),
     &               ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3)) 
         CALL SYMTR1(IRREP,POPA(1,1),VRT(1,2),DISSYT,ICORE(I020),
     &               ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3)) 
        ELSE
         CALL FSGET(ICORE(I010),1,NUMDSW,1,IRREP,LISTW,'NNNI')
...
C       AI    Mne   NNN A      NNN I
C                         +                                                    
C     H(IJ) = SUM T(MnE,i) * W(MnE,j)             [ISPIN=2]
C       AI    MnE   NNN A      NNN I
C     
        IZ=I000+(ISPIN-1)*NF1AI(1)*IINTFP
        IW=I010
        IT=I020
...
C     
         CALL FSGET(ICORE(I010),1,NUMDSW,1,IRREP,LISTW,'NNIN')
         CALL FSGET(ICORE(I020),1,NUMDST,1,IRREP,LISTT,'NNAN')
         CALL SYMTR1(IRREP,POPI(1,ISPIN),VRT(1,ISPIN),DISSYW,
     &               ICORE(I010),
     &               ICORE(ITMP1),ICORE(ITMP2),ICORE(ITMP3)) 
