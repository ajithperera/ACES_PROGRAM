      ...
      SUBROUTINE REPART(ICORE,MAXCOR,IUHF)
C
C THIS ROUTINE PARTITIONS THE ONE-PARTICLE PART OF THE
C EFFECTIVE HAMILTONIAN AND WRITES THE DIAGONAL ELEMENTS
C TO THE SCFEVALA AND SCFEVALB RECORDS
C
CEND
...
C
      DO 10 ISPIN=1,1+IUHF
       ...
       DO 20 IRREP=1,NIRREP
        ...
        CALL SAXPY(NOCC,ONEM,ICORE(IOFFMI),NOCC+1,ICORE(IOFFMI),NOCC+1)
        CALL SCOPY(NVRT,ICORE(IOFFEA),NVRT+1,ICORE(IOFFV),1)
        CALL SAXPY(NVRT,ONEM,ICORE(IOFFEA),NVRT+1,ICORE(IOFFEA),NVRT+1)
...
20     CONTINUE
       ...
       CALL PUTREC(20,'JOBARC',LABEL(ISPIN),IINTFP*NBAS,ICORE(I000))
10    CONTINUE
C
      RETURN
      END
