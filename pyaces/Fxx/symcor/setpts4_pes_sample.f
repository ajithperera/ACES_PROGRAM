      ...
      subroutine setpts4_pes_sample(natom, nirrepf, type, label,
     &     ipttype, points,
...
C     
C     We don't get this far if GEO_SAMPLE file does not exist. So we
C     can open it.

      iunit = 41
c     
c     GEO_SAMPLE should contain the following:
c     nradial, nequi, nsample
c     nmode
c     reference energy in a.u. 
c     displacement vectors along reduced normal 
c     coordinates of parent state. One entry per line.
c
      write(6,*) ' Start reading GEO_SAMPLE file '
      open(iunit, file='GEO_SAMPLE')
      read(iunit,*)
      read(iunit, *) nradial, nequi, nsample
      write(6,*) ' nradial, nequi, nsample ',
     $     nradial, nequi, nsample
c
      if (.false.) then
      read(iunit,*) numvib
      if (numvib .ne. nmode) then
         write(6,*) ' # of modes on GEO_SAMPLE does not match NMODE'
         write(6,*) ' error in stepts4_geo_sample.F '
         call errex
      endif
...
CSSS  call zero(points(ioffp), nsize)
      do isample = 1, nsample
            ...
            read(iunit,802) j, k, jsample,
     $           (dscr(lscan+i), i=1,nmode)
            write(6,*) ' Point read from geo_sample '
            write(6,802) j, k, jsample,
     $           (dscr(lscan+i), i=1,nmode)
