      ...
      SUBROUTINE DOT33(DOOA,DOOB,T3C,T3D,LENT3,
     1                 IRPI,IRPJ,IRPK,
     1                 I,J,K,NOCA,NOCB)
      IMPLICIT INTEGER (A-Z)
      DOUBLE PRECISION DOOA(NOCA),DOOB(NOCB),T3C(LENT3),
     1                                       T3D(LENT3),SDOT
...
C
      ...
      DOOB(IACT) = DOOB(IACT) - SDOT(LENT3,T3C,1,T3D,1)
      JACT = ABSOCC(J,IRPJ,ISPIN1)
      DOOB(JACT) = DOOB(JACT) - SDOT(LENT3,T3C,1,T3D,1)
      KACT = ABSOCC(K,IRPK,ISPIN2)
      DOOA(KACT) = DOOA(KACT) - SDOT(LENT3,T3C,1,T3D,1)
C
      RETURN
      END
