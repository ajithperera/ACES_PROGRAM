      SUBROUTINE T1T32N(D1T1AA,D1T1BB,S1A,S1B,
     1                  T3,W,
     1                  TIJ,TJK,TIK,
     1                  VJKAB,VIKAB,VIJAA,
     1                  IADT3,IADW,
     1                  IRPI,IRPJ,IRPK,IRPIJ,IRPIK,IRPJK,IRPIJK,
     1                  I,J,K,ISPIN1,ISPIN2,LNVVIJ,LNVVJK,LNVVIK,
     1                  NONHF,LAMBDA)
      IMPLICIT INTEGER (A-Z)
      LOGICAL NONHF,LAMBDA
      DOUBLE PRECISION D1T1AA(1),D1T1BB(1),S1A(1),S1B(1)
      DOUBLE PRECISION T3(1),W(1)
      DOUBLE PRECISION TIJ(LNVVIJ,1),TJK(LNVVJK,1),TIK(LNVVIK,1)
      DOUBLE PRECISION VJKAB(1),VIKAB(1),VIJAA(1),SIGN
...
C
     ...
     1            D1T1AA(IOFFT1(IRPA) + (I-1)*VRT(IRPA,ISPIN1) + 1),
...
C
     ...
     1            D1T1AA(IOFFT1(IRPA) + (J-1)*VRT(IRPA,ISPIN1) + 1),
...
C
     ...
     1            D1T1BB(IOFFT1(IRPC)+(K-1)*VRT(IRPC,ISPIN2) + 1),
...
C
      IF(ISPIN1.EQ.1)THEN
      CALL GETLST(VJKAB,IOFFOO(IRPK,IRPJK,5) + JK,1,2,IRPJK,46+LISTOFF)
      CALL GETLST(VIKAB,IOFFOO(IRPK,IRPIK,5) + IK,1,2,IRPIK,46+LISTOFF)
      ELSE
      CALL GETLST(VJKAB,IOFFOO(IRPJ,IRPJK,5) + JK,1,2,IRPJK,46+LISTOFF)
      CALL GETLST(VIKAB,IOFFOO(IRPI,IRPIK,5) + IK,1,2,IRPIK,46+LISTOFF)
      ENDIF
C
...
C     BB  AAB AAB  AA AA
C
      CALL GETLST(VIJAA,IJ,1,2,IRPIJ,43 + ISPIN1+LISTOFF)
C
