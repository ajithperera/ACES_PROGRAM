      SUBROUTINE GOT314(GOOOV,OOOVAD,LNOOOV,W,CORE,
     1                 IADW,
...
C
      ...
      ENDIF
C
C     ROUTINE TO COMPUTE CONTRIBUTION OF T3 AAA OR T3 BBB IN G(IJ,LA) AAAA
C     OR BBBB. 
C
C
C                        ABC  BC
...
C
     ...
     1            KL,1,1,IRPKL,43+ISPIN+LISTOFF)
   40 CONTINUE
      ENDIF
C
     ...
     1            KL,POP(IRPL,ISPIN),1,IRPKL,43+ISPIN+LISTOFF)
...
C
     ...
     1            JL,1,1,IRPJL,43+ISPIN+LISTOFF)
  140 CONTINUE
      ENDIF
C
...
C    1            JL,1,1,IRPJL,43+ISPIN+LISTOFF)
C 150 CONTINUE
      JL = IOFFOO(IRPJ,IRPJL,ISPIN) + (J-1)*POP(IRPL,ISPIN) + 1
      CALL GETLST(CORE(IADT2(IRPL)),
     1            JL,POP(IRPL,ISPIN),1,IRPJL,43+ISPIN+LISTOFF)
      CALL VMINUS(CORE(IADT2(IRPL)),LENT2(IRPL))
      ENDIF
C
...
C
     ...
     1            IL,1,1,IRPIL,43+ISPIN+LISTOFF)
  240 CONTINUE
      ENDIF
C
...
C    1            IL,1,1,IRPIL,43+ISPIN+LISTOFF)
C 250 CONTINUE
      IL = IOFFOO(IRPI,IRPIL,ISPIN) + (I-1)*POP(IRPL,ISPIN) + 1
      CALL GETLST(CORE(IADT2(IRPL)),
     1            IL,POP(IRPL,ISPIN),1,IRPIL,43+ISPIN+LISTOFF)
      CALL VMINUS(CORE(IADT2(IRPL)),LENT2(IRPL))
      ENDIF
C
