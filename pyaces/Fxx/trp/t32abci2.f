      ...
      SUBROUTINE T32ABCI2(T3,W,ICORE,
     1                    IADT3,IADW,
...
C
C     ROUTINE TO COMPUTE CONTRIBUTION OF T3 AAB TO W(AI,BC).
C
C                        BCD  AD
C     G(AI,BC) = - SUM  T    W
C                  J<K   IJK  JK
...
C
     ...
     1            AI,VRT(IRPA,ISPIN1),2,IRPAI,LISABCI)
...
C
      ...
      CALL GETLST(ICORE(IADG(IRPA)),
     1            AJ,VRT(IRPA,ISPIN1),2,IRPAJ,LISABCI)
      CALL XGEMM('N','T',IRPDPD(IRPBC,ISPIN1),VRT(IRPA,ISPIN1),
     1           VRT(IRPD,ISPIN2),
     1            1.0D+00,T3(IADT3(IRPD)),IRPDPD(IRPBC,ISPIN1),
...
C
  ...
  240 CONTINUE
      ...
      AI = IOFFVO(IRPI,IRPAI,3) + (I-1)*VRT(IRPA,ISPIN2) + 1
      CALL GETLST(ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1            AI,VRT(IRPA,ISPIN2),2,IRPAI,129)
C
      CALL VADD(ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1          ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1          ICORE(IADG(IRPA)),LENG(IRPA), 1.0D+00)
...
C
  ...
  340 CONTINUE
      ...
      AJ = IOFFVO(IRPJ,IRPAJ,3) + (J-1)*VRT(IRPA,ISPIN2) + 1
      CALL GETLST(ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1            AJ,VRT(IRPA,ISPIN2),2,IRPAJ,129)
C
      CALL VADD(ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1          ICORE(IADG(NIRREP) + LENG(NIRREP)),
     1          ICORE(IADG(IRPA)),LENG(IRPA), 1.0D+00)
...
C
  ...
  420 CONTINUE
C
C     CONTRACT TO GET CONTRIBUTION TO  ABCI
C
      ...
      CALL GETLST(ICORE(IADG(IRPA)),
     1            AK,VRT(IRPA,ISPIN1),2,IRPAK,130)
