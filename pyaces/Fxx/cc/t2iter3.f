      SUBROUTINE T2ITER3(ICORE,MAXCOR,IUHF,IORDER)
C
C SOLVES FOR THE MBPT[3,4] ENERGY AND T2 VECTOR ITERATIVELY
C  FOR NONCANONICAL CASES.  EACH IRREP IS HANDLED INDIVIDUALLY
C
C D2 T2(ab,ij)=T2[ab,ij]+P(ab) SUM T2(ae,ij)*F(be)-P(ij) SUM T2(ab,mj)*F(mj)
C                              e                         m
C
CEND
      ...
      DATA ZILCH /0.0/
C
CJDW 3/18/98. In RHF we must form the AAAA aplitudes. Next block of
C             code is taken from NEWT2.
C
      IF(IUHF.EQ.0)THEN
...
200    CONTINUE 
      ENDIF
C
C  Since this is an iterative scheme, we need to store the constant
C  pieces of the T2[2] or T2[3] increment on different lists than 61-63.
C  We create a new set of lists if this is for T2[2].
C
      IF(IORDER.EQ.3) THEN
        imode=0
        CALL INIPCK(1,ISYTYP(1,61),ISYTYP(2,61),94,imode,0,1)
        IF(IUHF.NE.0)THEN
         imode=0
         CALL INIPCK(1,ISYTYP(1,62),ISYTYP(2,62),95,imode,0,1)
        ENDIF
        imode=0
        CALL INIPCK(1,ISYTYP(1,63),ISYTYP(2,63),96,imode,0,1)
        CALL ZEROLIST(ICORE,MAXCOR,94)
        IF(IUHF.NE.0) CALL ZEROLIST(ICORE,MAXCOR,95)
        CALL ZEROLIST(ICORE,MAXCOR,96)
C
C  Now put the constant T2[2] increments on these lists.
C
        NTOTSZ=ISYMSZ(ISYTYP(1,61),ISYTYP(2,61))
        CALL GETALL(ICORE,NTOTSZ,1,61)
        CALL PUTALL(ICORE,NTOTSZ,1,94)
        IF(IUHF.NE.0)THEN
         NTOTSZ=ISYMSZ(ISYTYP(1,62),ISYTYP(2,62))
         CALL GETALL(ICORE,NTOTSZ,1,62)
         CALL PUTALL(ICORE,NTOTSZ,1,95)
        ENDIF
        NTOTSZ=ISYMSZ(ISYTYP(1,63),ISYTYP(2,63))
        CALL GETALL(ICORE,NTOTSZ,1,63)
        CALL PUTALL(ICORE,NTOTSZ,1,96)
        LSTCON=93
      ELSEIF(IORDER.EQ.4) THEN
        imode=0
        CALL INIPCK(1,ISYTYP(1,61),ISYTYP(2,61),74,imode,0,1)
        IF(IUHF.NE.0)THEN
         imode=0
         CALL INIPCK(1,ISYTYP(1,62),ISYTYP(2,62),75,imode,0,1)
        ENDIF
        imode=0
        CALL INIPCK(1,ISYTYP(1,63),ISYTYP(2,63),76,imode,0,1)
        CALL ZEROLIST(ICORE,MAXCOR,74)
        IF(IUHF.NE.0) CALL ZEROLIST(ICORE,MAXCOR,75)
        CALL ZEROLIST(ICORE,MAXCOR,76)
C
C  Now put the constant T2[2] increments on these lists.
C
        NTOTSZ=ISYMSZ(ISYTYP(1,61),ISYTYP(2,61))
        CALL GETALL(ICORE,NTOTSZ,1,61)
        CALL PUTALL(ICORE,NTOTSZ,1,74)
        IF(IUHF.NE.0)THEN
         NTOTSZ=ISYMSZ(ISYTYP(1,62),ISYTYP(2,62))
         CALL GETALL(ICORE,NTOTSZ,1,62)
         CALL PUTALL(ICORE,NTOTSZ,1,75)
        ENDIF
        NTOTSZ=ISYMSZ(ISYTYP(1,63),ISYTYP(2,63))
        CALL GETALL(ICORE,NTOTSZ,1,63)
        CALL PUTALL(ICORE,NTOTSZ,1,76)
        LSTCON=73
      ENDIF
C
      ET2=0.0
...
1000   FORMAT(T3,'@T2ITER3-I, Beginning iterative solution for ',A2,
     &           ' amplitudes.')
       Z=0.0
...
C
        CALL GETLST(ICORE(I010),1,NT2DIS,1,IRREP,60+ISPIN)
C
...
120     CONTINUE
C
C NOW ANTISYMMETRIZE - Z(A<B,I<J) = Z(A<B,IJ) - Z(A<B,JI),
C  DENOMINATOR WEIGHT, AND ADD CONSTANT T2 PART
C
        CALL ASSYM2(IRREP,POP(1,ISPIN),NT2DSZ,ICORE(I000))
        CALL SSCAL(NT2DIS*NT2DSZ,HALF,ICORE(I000),1)
        CALL ASSYM2(IRREP,POP(1,ISPIN),NT2DSZ,ICORE(I010))
        CALL GETLST(ICORE(I040),1,NT2DIS,1,IRREP,LSTCON+ISPIN)
        ...
        if(iter.eq.0)
     &    call vecprd(icore(i000),icore(i040),icore(i000),nt2dis*nt2dsz)
C
...
c YAU : end
        ...
        ELSE
         ...
         CALL PUTLST(ICORE(I010),1,NT2DIS,1,IRREP,60+ISPIN)
...
C
       CALL GETLST(ICORE(I000),1,NT2DIS,1,IRREP,63)
C
...
C
        ...
        IF(IUHF.EQ.0)THEN
          CALL GETLST(ICORE(I030),1,1,1,3,92)
          CALL GETLST(ICORE(I040),1,1,1,3,91)
        ELSE
...
116    CONTINUE
C
C NOW DENOMINATOR WEIGHT AND ADD CONSTANT PART
C
       CALL GETLST(ICORE(I020),1,NT2DIS,1,IRREP,LSTCON+3)
       ...
       if(iter.eq.0)
     &   call vecprd(icore(i000),icore(i020),icore(i000),nt2dis*nt2dsz)
C
C COMPARE WITH PREVIOUS ITERATE
C
       CALL SAXPY(NT2DSZ*NT2DIS,ONEM,ICORE(I010),1,ICORE(I000),1)
...
c YAU : end
       ...
       ELSE
        ITER=ITER+1
        CALL PUTLST(ICORE(I010),1,NT2DIS,1,IRREP,63)
