      ...
      SUBROUTINE FMICONT_MODF(ICORE,MAXCOR,IUHF)
C
C This routine simply compute the term 
C
C 1/2 SUM E F(E,M) T(E,I) 
C
C CODED JG JUNE/90, Modified for EOMPx, Ajith Perera, 04/2015.
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DIRPRD,DISSYT,DISSYZ,POP,VRT
      LOGICAL MBPT3,MBPT4,CC,TRPEND,SNGEND,GRAD,MBPTT,SING1,QCISD,UCC
      LOGICAL LINCC,CICALC,ROHF4,ITRFLG,NONHF,CC2
...
C
       ...
       ENDIF
C 
C F(m,i) * T1(m,a) contribution to T1
...
C This f(mj) * T2 (im,ab) contribution to T2. (Notice that in CCSD
C instead of f(be), the F(mj) intermediate is useed and there is
C no T2 contribution from -1/2 T2(im,ab) * t(j,e) * F(m,e) either.
C
       IF (CC2 .AND. NONHF) THEN

        CALL GETLST(ICORE(I0AA),1,1,1,3,91)
        IF(IUHF.EQ.1) CALL GETLST(ICORE(I0BB),1,1,1,4,91)
C
        IF(.NOT.QCISD.AND..NOT.LINCC.OR.ROHF4) THEN
C
C  USE THE ALLOCATION FOR Z TO STORE f(E,M) for CC2.
C
...
C
        ...
        DO 20 IRREP=1,NIRREP
         NOCC=POP(IRREP,1)
         NVRT=VRT(IRREP,1)
         CALL XGEMM('T','N',NOCC,NOCC,NVRT,1.0D0,ICORE(IOFFEM),NVRT,
