...
C
      CALL GENINT(ICORE,MAXCOR,IUHF,3, DO_HBAR_4LCCSD)
C
C THIRD ORDER STUFF.
C
C
...
C
      CALL GENINT(ICORE,MAXCOR,IUHF,1, DO_HBAR_4LCCSD)
C
...
C
        CALL GENINT(ICORE,MAXCOR,IUHF,0, DO_HBAR_4LCCSD)
C
...
C
      IF(TRIPNI) THEN
        CALL TRPS(ICORE,MAXCOR,IUHF,ENERGY(6,1),.FALSE.,.FALSE.)
        IF(IFLAGS(39).EQ.0) THEN
          WRITE(*,4021)
