...
C
      IF(METHOD.GT.9.AND.SING1.OR.METHOD.EQ.6.AND.SING1.OR.ROHF4)THEN
       ...
       ENDIF
C
C     IF THE CALCULATION IS AOBASED THE F(A,E) TERMS MUST BE CORRECTED
C     BY SUM(m,f) t(m;f) <ma||fe>
C
       IF (AOBASIS) THEN 
        I0CA=I0ZB-NFAA*IINTFP
        IF(I0CA.LE.0) CALL INSMEM('FEACONT',I0CA,MAXCOR)
        CALL GETLST(ICORE(I0CA),1,1,1,7,92)
        CALL SSCAL(NFAA,ONEM,ICORE(I0CA),1)
        CALL SAXPY(NFAA,ONE,ICORE(I0AA),1,ICORE(I0CA),1)
        IF (IUHF.EQ.1) THEN
         I0CB=I0CA-NFBB*IINTFP
         IF(I0CB.LE.0) CALL INSMEM('FEACONT',I0CB,MAXCOR)
         CALL GETLST(ICORE(I0CB),1,1,1,8,92)
         CALL SSCAL(NFBB,ONEM,ICORE(I0CB),1)
         CALL SAXPY(NFBB,ONE,ICORE(I0BB),1,ICORE(I0CB),1)
        ENDIF           
       ENDIF
C
