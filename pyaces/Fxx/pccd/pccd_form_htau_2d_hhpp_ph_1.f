      Subroutine Pccd_form_htau_2d_hhpp_ph_1(Hov,Hvo,Work,Maxcor,
...
C V(fe,na)(t)*G(fe,ni) (stored as (fe,ni)) -> S(a,i))
     ...
     +                     Nsum,Work(Koff),Nsum,One,Hvo(Ioff),Nrow)
      ...
      call output(Hvo,1,Nocc,1,Nvrt,Nocc,Nvrt,1)

C V(fi,nm)*Gamma(fa,nm)[V(nm,fi)*Gamma(nm,fa)]  (V is stored as mn,if) 
...
C V(mn,if) -> (mn,fi)
     ...
     +               Work(I000),Work(I010),Work(I020),Work(I030))
         ...
         Iend = I020 + Nrow_nm*Ncol_fa
         If (Iend .Gt. Maxcor) Call Insmem("Pccd_form_htau_2d_hhpp_ph",
     +                                      Iend,Maxcor)
C G stored as (fa,nm)

         Call Getlst(Work(I010),1,Nrow_nm,2,Irrep_nm,List_g)

C G(fa,nm) -> G(nm,fa)
         Call Transp(Work(I010),Work(I020),Nrow_nm,Ncol_fa)
...
C  V(nm,fi)*Gamma(nm,fa) -> S(i,a)
     ...
     +               Nsum,Work(Koff),Nsum,One,Hov(Ioff),Nrow)
      ...
      call output(Hov,1,Nvrt,1,Nocc,Nvrt,Nocc,1)
