      SUBROUTINE PCCD_UFINISH(ICYCLE,SIDE)
...
20    CONTINUE
      ...
      IF (SIDE .EQ. "R") CALL PUTREC(1,'JOBARC','TOTENERG',IINTFP,
     &                               ECORR(ICYCLE,1)+ESCF)
      IF (SIDE .EQ. "R") THEN
...
204   FORMAT(T7,'A miracle has come to pass. ',
     &          'The right LCCD iterations have converged.')
      IF (PCCD) THEN
      WRITE(6,1010) "pCCD ",ECORR(ICYCLE,1)+ESCF
      ELSE IF (CCD) THEN
      WRITE(6,1010) "CCD ", ECORR(ICYCLE,1)+ESCF
      ELSE IF (LCCD) THEN
      WRITE(6,1010) "LCCD ",ECORR(ICYCLE,1)+ESCF
      ENDIF 
      ELSEIF (SIDE .EQ. "L") THEN
...
207   FORMAT(T7,'A miracle has come to pass. ',
     &          'The left LCCD iterations have converged.')
      IF (PCCD) THEN
      WRITE(6,1020) "pCCD ",ECORR(ICYCLE,1)+ESCF
      ELSE IF (CCD) THEN
      WRITE(6,1020) "CCD ", ECORR(ICYCLE,1)+ESCF
      ELSE IF (LCCD) THEN
      WRITE(6,1020) "LCCD ",ECORR(ICYCLE,1)+ESCF
      ENDIF 
      ENDIF 
