...
C
      SUBROUTINE VDENS(ICORE,ICRSIZ,IUHF)
...
CSSS      IF (.NOT.MBPT2 .AND. .NOT.MRCC .AND. (RELAXED.OR.CANON)) THEN
      ...
      IF (.NOT.MBPT2 .AND. .NOT.MRCC) THEN
       CALL GAMDRV(ICORE(i0),MAXCOR,IUHF)
       IF (IFLAGS(35).NE.0) CALL INCOR(I0,ICRSIZ,IUHF)
       MAXCOR=ICRSIZ
      ENDIF
c
...
C      
       ...
       IF(IFLAGS(1).GE.15.AND.NDRGEO.EQ.0)THEN
        ...
        CALL DUMPIT_DENS(ICORE(IDOO),ICORE(IDVV),ICORE(IIVO),
     &                   ICORE(IIOV),IUHF)
       ENDIF
C
...
C
      IF(IFLAGS(1).GE.15)THEN
       ...
       CALL DUMPIT_DENS(ICORE(IIOO),ICORE(IIVV),ICORE(IIVO),ICORE(IIOV),
     &             IUHF)
      ENDIF
C
...
C
        IF(.NOT.GABCD) THEN
         ...
         IF (IFLAGS(1).GE.15) THEN
            ...
            CALL DUMPIT_DENS(ICORE(IIOO),ICORE(IIVV),ICORE(IIVO),
     &                  ICORE(IIOV),IUHF)
         ENDIF
        ELSE
...
C
     ...
     &             IUHF)
      ...
      IF(IFLAGS(1).GE.10)THEN
       write(6,*) ' D matrices '
       CALL DUMPIT_DENS(ICORE(IDOO),ICORE(IDVV),ICORE(IIVO),ICORE(IIOV),
     &             IUHF)
      ENDIF
C
...
C
      IF(IFLAGS(1).GE.15)THEN
       ...
       CALL DUMPIT_DENS(ICORE(IIOO),ICORE(IDVV),ICORE(IIVO),ICORE(IIOV),
     &             IUHF)
       CALL DUMPIT_DENS(ICORE(IDOO),ICORE(IDVV),ICORE(IIVO),ICORE(IIOV),
     &             IUHF)
       CALL DUMPIT_DENS(ICORE(IXOO),ICORE(IEVAL),ICORE(IIVO),
     &                  ICORE(IIOV),IUHF)
      ENDIF
C
...
CSSS     &    IFLAGS(11).NE.0 .AND. RELAXED) THEN
     &    IFLAGS(11).NE.0) THEN
...
C
CSSS      IF (SEWARD) THEN
         IF (NDRGEO .EQ.0) CALL BLTAO_QUNTITES(ICORE(i0), MAXCOR,
     +                                         IUHF, NONHF)
CSSS      ENDIF
C
