...
C
     ...
     &           BUF,NMO,ZILCH,BUF(IOFFB2),NBAS)
      ...
      call output(buf(Ioffb), 1, nbas, 1, nbas, nbas, nbas, 1)
...
C in SCF is written
         ...
         Iscr1_ = Ioffb4
         IScr2_ = Iscr1_ + Nmo*Nmo
         Iscr3_ = Iscr2_ + Nmo*Nmo
         Iscr4_ = Iscr3_ + Nmo+1
         Ikeep_ = Iscr4_ + Nmo+1
         Iend   = Ikeep_ + 2*Nmo*Nmo
         If (Iend .Gt. Maxcor) Call Insmem("-Natorb",Iend,Maxcor)

         If (Ispin .EQ. 1) Ioff1 = Ioffb2 
         If (Ispin .EQ. 2) Ioff1 = Ioffb3
         Call Process_natorbs(Buf(Ioff1),Buf(Iscr1_),Buf(Iscr2_),
     +                        Buf(Iscr3_),Buf(Iscr4_),Nmo,Length,
     +                        Ispin)
      ...
      CALL PUTNATORBS(BUF(Ikeep_), NBAS*NBAS, IUHF, NIRREP, NBF4IRREP,
     &                NOCC)
