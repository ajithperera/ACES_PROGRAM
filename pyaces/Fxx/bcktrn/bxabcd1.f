      SUBROUTINE BXABCD1(EVEC,BUF1,BUF2,BUF3,BUFTMP,L1MAX,LEN2,LEN3,
     &                   LENTMP,IUHF)
C
...
C
         ISTART=1
C
         LEN1=(NJI*NBXIA+NBI*NJXIA)*NAOXIC
C
         IF(LEN1.NE.0) THEN
          MAXAO=MIN(L1MAX/LEN1,NAOXIB)
         ELSE
          MAXAO=NAOXIB
         ENDIF
C
...
C
         ...
         IEND=MIN(NAOXIB,ISTART+MAXAO-1)
         NAOXIB1=IEND+1-ISTART
         NSIZFL1=NAOXIC*NAOXIB1
C
         IOFF1=1
         ...
         IOFFEVB=INDVRT(IRREPXIB,ISL)+ISTART-1
         IOFFEOA=INDOCC(IRREPXIC,ISL)
         IOFFEOB=INDOCC(IRREPXIB,ISL)+ISTART-1
         IBLKSZA=NAXIC*NIXIB
         IBLKSZB=NAXIB*NIXIC
C
...
c           write(*,*) 'indices before',indexj,indexb
c           write(*,1234) (buf2(ioff2a1-1+i),i=1,2)
c           write(*,1234) (buf2(ioff2b1-1+i),i=1,2)
           CALL SYTRAB2(BUF2(IOFF2A1),BUF2(IOFF2B1),EVEC(IOFFEVA),
     &                  EVEC(IOFFEVB),EVEC(IOFFEOA),EVEC(IOFFEOB),
     &                  NAOXIC,NAOXIB1,NAOXIB,NAXIC,NAXIB,NIXIC,
     &                  NIXIB,BUFTMP,BUF1(IOFF1),0)
c           write(*,*) 'incdices',indexj,indexb
c           write(*,1234) (buf1(ioff1+i-1),i=1,nsizfl1)
...
C
         ...
         DO 160 INDEXJ=1,NJXIA
          DO 170 INDEXB=1,NBI
c           write(*,1234) (buf2(ioff2a2-1+i),i=1,2)
c           write(*,1234) (buf2(ioff2b2-1+i),i=1,2)
           CALL SYTRAB2(BUF2(IOFF2A2),BUF2(IOFF2B2),EVEC(IOFFEVA),
     &                  EVEC(IOFFEVB),EVEC(IOFFEOA),EVEC(IOFFEOB),
     &                  NAOXIC,NAOXIB1,NAOXIB,NAXIC,NAXIB,NIXIC,
     &                  NIXIB,BUFTMP,BUF1(IOFF1),0)
c           write(*,*) 'incdices',indexj,indexb
...
C
         ...
         IOFFEVB=INDVRT(IRREPXIB,ISR)+ISTART-1
         IOFFEOA=INDOCC(IRREPXIC,ISR)
         IOFFEOB=INDOCC(IRREPXIB,ISR)+ISTART-1
         IBLKSZA=NAXIC*NIXIB
         IBLKSZB=NAXIB*NIXIC
C
C DO THE SYMMETRIZED FIRST HALF TRANSFORMATION:
C                                          +  
C T2(XX,BxiaJi) =  Cxic T2(AxicIib,BxiaJi) C xib
C
C                                             +    + 
C               + [Cxib T2(AxibIxic,Bxia,Ji) C xic] 
C
...
c           write(*,1234) (buf3(ioff2a1-1+i),i=1,2)
c           write(*,1234) (buf3(ioff2b1-1+i),i=1,2)
           CALL SYTRAB2(BUF3(IOFF2A1),BUF3(IOFF2B1),EVEC(IOFFEVA),
     &                  EVEC(IOFFEVB),EVEC(IOFFEOA),EVEC(IOFFEOB),
     &                  NAOXIC,NAOXIB1,NAOXIB,NAXIC,NAXIB,NIXIC,
     &                  NIXIB,BUFTMP,
     &                  BUF1(IOFF1),1)
...
C
         ...
         DO 260 INDEXJ=1,NJXIA
          DO 270 INDEXB=1,NBI
c           write(*,1234) (buf3(ioff2a2-1+i),i=1,2)
c           w rite(*,1234) (buf3(ioff2b2-1+i),i=1,2)
           CALL SYTRAB2(BUF3(IOFF2A2),BUF3(IOFF2B2),EVEC(IOFFEVA),
     &                  EVEC(IOFFEVB),EVEC(IOFFEOA),EVEC(IOFFEOB),
     &                  NAOXIC,NAOXIB1,NAOXIB,NAXIC,NAXIB,NIXIC,
     &                  NIXIB,BUFTMP,BUF1(IOFF1),1)
c           write(*,*) 'incdices',indexj,indexb
c           write(*,1234) (buf1(ioff1+i-1),i=1,nsizfl1)
           IOFF2A2=IOFF2A2+IBLKSZA
           IOFF2B2=IOFF2B2+IBLKSZB
...
C
          ...
          IOFF1A=1
          IOFF1AA=1+(ISTART-1)*NAOXIC
          IOFF1B=ILCIXI
          IOFFBA=NSZBJ1+1
          IOFFBB=NSZBJ2+IOFFBA
          IOFFCC=NAOXIA*NAOI+IOFFBB
C
          DO 180 INDXNU=ISTART,IEND
           DO 190 INDXMU=1,NAOXIC
            CALL SCOPY(NSZBJ1,BUF1(IOFF1A),NSIZFL1,BUFTMP,1)
            CALL SCOPY(NSZBJ2,BUF1(IOFF1B),NSIZFL1,
     &                 BUFTMP(IOFFBA),1)
c           write(*,*) 'before '
...
c            write(*,*)
c            write(*,1234) (buftmp(IOFFCC+I-1),I=1,NAOXIA*NAOI)
1234        format(8(F10.7,1x))
            CALL PUTLST(BUFTMP(IOFFCC),IOFF1AA,1,1,4,ITHRU)
            IOFF1A=IOFF1A+1
            IOFF1AA=IOFF1AA+1
            IOFF1B=IOFF1B+1
190        CONTINUE
...
c            write(*,1234) (buftmp(i),i=1,nszbj1)
c            write(*,1234) (buftmp(IOFFBA-1+i),i=1,nszbj2)
           CALL GETLST(BUFTMP(IOFFCC),IOFF1AA,1,1,4,ITHRU)
...
C
         ...
         ISTART=ISTART+NAOXIB1
C
        IF(IEND.NE.NAOXIB) GO TO 10
C
130     CONTINUE
120    CONTINUE
100   CONTINUE
