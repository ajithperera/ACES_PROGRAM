     ...
     &   xocc,ksvxc,factor,ifctr,valao,valgradao,fourind,scrnn,
     &   dprimcoef,
     &   coord,compmemb,angfct,order,init,iradpt,iangpt,
     &   times,ksdint,dograd,nprimao,nprimaol,dcoeff)
...
C  replace xcoeff by dprimcoeff


      integer
     &    nshellatom(natoms),nshellprim(maxshell,natoms),
     &    compmemb(natoms),ifctr,kol,jj,
     ...
     &    times(20),time
      ...
      integer hh, ih,
     &    iatom,ifct,i,iang,imomfct,ispin,xyz,l,m,n,naonao,ii,j
     ...
     &    tfkef,wkef,ddot
      ...
      integer nprimao(naobasfn),nprimaol(naobasfn),iao,iz,ip
      double precision
     &   dprimcoef(totprim,naobasfn),dcoeff(naobasfn,nbastot)
...
c      o contract functions with alpha/beta MO coefficients
         ...
         call xgemm('t','n',nbastot,1,naobasfn,
     &              1.d0,dcoeff,naobasfn,
     &              valao,naobasfn,
     &              0.d0, valmo(1,ispin),  nbastot)

c       write(*,*)'valmo=',valmo(1,1),valmo(2,1),valmo(3,1)       
c         call xgemm('t','n',nbastot,1,totprim,
c     &              1.d0, pcoeff(1,1,ispin),totprim,
c     &                   valprim,          totprim,
c     &              0.d0,valmo(1,ispin),   nbastot)
...
c               call xgemm('t','n',nbastot,1,totprim,
c     &                    1.d0, pcoeff(1,1,ispin),  totprim,
c     &                         gradprim(1,xyz),    totprim,
c     &                    0.d0,gradmo(1,xyz,ispin),nbastot)
...
c             valao(ii)=0.D0
c             do iz=1,3
c             valgradao(ii,iz)=0.D0
c            end do
c           end do
c Contract functions with coefficients and add to get AO values at this point.
c                     do iao=1,naobasfn
c           do ip=nprimao(iao),nprimaol(iao)
c        write(*,*) iao,nprimao(iao),nprimaol(iao),dprimcoef(ip,iao)

c         valao(iao)=valao(iao)+dprimcoef(ip,iao)*valprim(ip)
c           do xyz=1,3
c         valgradao(iao,xyz)=valgradao(iao,xyz)+
c     &             dprimcoef(ip,iao)*gradprim(ip,xyz)
c         end do
c         end do
c         write(*,*) iradpt,iangpt,iao,valao(iao)
c       end do

c       call xgemm('t','n',naobasfn,1,totprim,1.d0,xcoeff,totprim,
c     &    valprim,totprim,0.d0,valao,naobasfn)
...
c       do xyz=1,3
c          call xgemm('t','n',naobasfn,1,totprim,1.d0,xcoeff,totprim,
c     &    gradprim(1,xyz),totprim,0.d0,valgradao(1,xyz),naobasfn) 
c       end do
