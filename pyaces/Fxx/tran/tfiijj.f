      SUBROUTINE TFIIJJ(CMO,ICORE,MAXCOR,IUHF)
C
C DRIVES THE TRANSFORMATION OF IIJJ INTEGRALS IN CASE OF A FULL
C AO-> MO INTEGRAL TRANSFORMATION
C
CEND
...
C
        ISIZE4(IRREP)=ISIZE3(IRREP)*NMO(IRREP)
C
...
C
       ILNBUF=600
C
C TRY FIRST FULL IN CORE TRANSFORNATIONS
C 
       I000=ISTART
C
...
C AND NBASIS*IINTFP, ILNBUF(1+IINTFP) FOR THE FIRST
C STEP, SUBTRACT FROM TOTAL MEMORY AND THEN DETERMINE 
C MAXIMUM NUMBER OF ORBITALS POSSIBLE TO USE IN FIRST 
C STEP
C
        I010=ISTART
        I020=I010+IINTFP*NBASIS
        ...
        I000=IEND
C
C MXCOR IS MAXIMUN OF AVAILABLE CORE MEMORY
C
        MXCOR=MAXCOR-IEND
C
...
C
       IF(ISIZE3(JRREP).NE.0) THEN
        ...
        IF(NORB.EQ.0.AND.FIRST) THEN
         write(*,*) ' @TFIIJJ-F, Sorry, there is not enogh memory !'
         CALL ERREX
        ENDIF
       ELSE
        NORB=NMO(JRREP)+1
...
C 
C CHECK WHETHER MORE IRREPS CAN BE CONSIDERED IN THIS PASS
C
...
950    CONTINUE
C
       CALL LOAD2F(CMO,ICORE(I000),ICORE(I010),ICORE(I020),
     &             ICORE(I030),ICORE(ISYMAO),NBAS,
     &             NMO,NSTART,NEND,ISIZE3,ISIZT,IOFFAO,IOFFI,
     &             NSIZE1,ILNBUF,ISPIN,LUINT,LAST)
C
       CALL INTRN2F(CMO,ICORE(I000),ICORE(I010),RHF,NBASIS,
     &             NBAS,NMO,NSTART,NEND,ISIZE3,IOFFMO,NSIZE1,
     &             ISPIN,ICORE(IREORD))
