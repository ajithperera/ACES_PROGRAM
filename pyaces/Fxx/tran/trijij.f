      SUBROUTINE TRIJIJ(CMO,ICORE,MAXCOR,IUHF)
C
C THIS ROUTINE DRIVES THE TRANSFORMATION OF 
C TWO-ELECTRON INTEGRALS OF TYPE IJIJ
C (PARTIAL TRANSFORMATION ONLY)
C
CEND
...
C
C FILL NSTART AND NEND FOR HANDLING OUT-OF-CORE
C
       DO 6 IRREP=1,NIRREP
        NSTART(IRREP)=1
...
C
        DO 11 IRREP2=1,NIRREP
         ...
         ISIZE3(IRREP1,IRREP2)=NBAS(IRREP2)*ISIZE2(IRREP1,IRREP2)
         ISIZE4(IRREP1,IRREP2)=ISIZE3(IRREP1,IRREP2)
     &                         *POP(IRREP1,ISPIN)
11      CONTINUE
C
10     CONTINUE
...
C
       ...
       DO 15 IRREP1=1,NIRREP
        DO 16 IRREP2=1,NIRREP
         IF(IRREP1.EQ.IRREP2) GO TO 16
C
...
C
       IF(ISIZE3T(JRREP).NE.0) THEN
        ...
        IF(NORB.EQ.0.AND.FIRST) THEN
         write(*,*) ' @TRIJIJ-F, Sorry, there is not enough memory !'
         CALL ERREX
        ENDIF
       ELSE
        NORB=NMO(JRREP)+1
...
C
       ...
       ENDIF
C
C DETERMINE REQUIRED OFFSET FOR OUT-OF-CORE
C
       IOFF=0
       DO 17 IRREP1=1,NIRREP
        DO 18 IRREP2=1,NIRREP
         IF(IRREP1.EQ.IRREP2) GO TO 18
C
          IOFF4(IRREP1,IRREP2)=IOFF
...
950    CONTINUE
C
       CALL LOAD3(CMO,ICORE(I000),ICORE(I010),ICORE(I020),
     &            ICORE(I030),ICORE(ISYMAO),NBAS,
     &            POP(1,ISPIN),NSTART,NEND,IOFFAO,IOFF4,
     &            ISIZE3,NSIZE1,ILNBUF,ISPIN,LUINT,LAST,
     &            IOFFSET)
C
       CALL INTRN3(CMO,ICORE(I000),ICORE(I010),RHF,NBASIS,
