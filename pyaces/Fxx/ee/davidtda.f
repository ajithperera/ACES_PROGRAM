...
C
      ...
      Write(6,"(a,1x,I3)") "  The TDAGUSS record", ILOCX 
      Write(6,"(a,2l)")    "  The nostandard and triplet flags", 
     &                        nonstd,triplet
C
       WRITE(6,*) 
       WRITE(6,1000)NROOT,IRREP
1000   FORMAT(T3,'@DAVID1-I, Beginning search for ',I3,
     &           ' solutions in symmetry block',I2)
C
...
C
       IF(IROOT.GT.0)THEN
        CALL GSCHMIDT_EE(VSPACE(1,ICOL),VSPACE(1,MAXIT+1),NDIMA,IROOT,
     &                   TMP2,RESID)
       ENDIF
C
C FORM ASMALL
...
C
       CALL GSCHMIDT_EE(TMP4,VSPACE(1,ICOL),NDIMA,NSPACE+IROOT,
     &                  TMP2,RESID)
...
1003  FORMAT(T3,'Beginning search for root',I5,'.')
