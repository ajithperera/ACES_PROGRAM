      ...
      SUBROUTINE DOEOMEE_LANCZOS(ICORE,MAXCOR,IUHF)
C
     ...
     &             END_IRP(100,8),NATURE
      ...
      COMMON/PROJECT_/IPROJECT, IPATTERN, NCALC, ICALC, IWINDOW(8)
C
      DATA LISTT2 /444/
...
C Convrg: Strict or light 
     ...
     +                     Convrg 
      ...
      write(*,*) '@VEE: Entered ', "doeomee_lanczos.F"
...
C
      ...
      DO 10 IRREPX=1,NIRREP
         ...
         IF(NSIZEC.EQ.0) THEN 
           Write(6,"(2a)") " The length of R or L vectors cannot", 
     &                     " be zero"
           CALL ERREX
         ENDIF
C
...
C
       ...
       IRL_VEC_BGN = MAXCOR - (2*NSIZEC*(MAXVW+3)) + 1
       MEMLEFT     = IRL_VEC_BGN - 1
       I000        = 1 
       DO ISPIN=3,3-2*IUHF,-1
          CALL ZEROLIST(ICORE,MAXCOR,448-1+ISPIN)
       ENDDO 
       
       CALL DO_LANCZOS_MAIN(ICORE(IRL_VEC_BGN),ICORE(I000),MEMLEFT,
     &                      IUHF,IRREPX,NSIZEC,MAXCOR)
       WRITE(6,*)
