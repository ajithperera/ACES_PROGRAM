...
C
      ...
      CALL SETMET_EE(ICORE(I0),MAXCOR,IUHF)
 ...
 2014      format(' PCCSD epsilon parameter : ', F14.6)
     ...
     $                    "used in W(mbej) intermediate."
      ...
      IF (IFLAGS(35).NE.0) CALL INCOR_EE(I0,ICRSIZ,IUHF)
      I0 = 1
      MAXCOR=ICRSIZ
      CALL ZERO(POLTOT,9)
C
...
C
     ...
     &         IFLAGS2(117) .EQ. 10)
      ...
      IF(ESPROP)CALL NUCDIP_EE(ICORE(I0),MAXCOR)
...
C that the code has taken the lambda route. 
     ...
     &    CALL PDCC_FIXF(ICORE(I0),MAXCOR,IUHF)
          CALL MODF_EE(ICORE(I0),MAXCOR,IUHF,1)
