      ...
      SUBROUTINE GSCHMIDT_EE(VEC,VORTH,NSIZE,NDIM,TMP,RESID)
C
...
C
      print = .false.
      if (print) then
        write(6,*) ' old vector in gschmidt_ee'
        ...
        enddo
        write(6,*) ' old overlap in gschmidt_ee'
...
C
      if (print) then
        write(6,*) ' new vector in gschmidt_ee'
        ...
        do i = 1, ndim
          TMP(i)=SDOT(NSIZE,VORTH(1,I),1,VEC,1)
        enddo
        write(6,*) ' current overlap in gschmidt_ee'
        call output(tmp,1,1,1,ndim,1,ndim,1)
      endif
      RETURN
      END
