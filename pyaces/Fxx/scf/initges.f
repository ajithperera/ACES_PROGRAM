...
C
      ROHFMO     = .FALSE.
      G_FLE_READ = .FALSE.
      Geom_Opt   = (iFlags(47).GT.0)
      Findf_Vib  = (iFlags(54).GT.1) .or. 
     $     (iFlags2(156).EQ.1) .or.
     $     (iFlags2(158).EQ.1)
      Brueckner  = (iFlags(22).GT.0)
      Rohf       = (iFlags(11).EQ.2)
      hfstab     = (iFlags(74).EQ.2)
      Occ_In_Zmt = (iFlags(17).NE.0)
      IFLAG45    = iFLAGS(45)
      QRHF       = (iFlags(77).GT.0)
      Length     = 0
      IKICK      = 0
...
c
      if (IFLAGS2(164) .eq. 1) then
         AUTOMATE = .TRUE. 
      elseif (iFLAGS2(164) .eq. 0) then
         AUTOMATE = .FALSE.
      else
