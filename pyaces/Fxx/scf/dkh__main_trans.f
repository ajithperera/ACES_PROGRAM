...
C#ifdef _DEBUG
C This is only for debugging the OED/DKH path. First OED/DKH code need
C to be modified to write the DKH_INTS in full matrix form and change
C the dkh__init.f.
C 
C         CALL GETREC(20,"JOBARC","DKH_INTS",NBAS*NBAS,ZCORE(ZSCR1))
C         CALL DAXPY(NBAS*NBAS,-1.0D0,ZCORE (1),1,ZCORE(ZSCR1), 1) 
C#endif 
         IF (KNOW .GE. 5) THEN
     ...
     +
C             WRITE (*,*) ""
C             CALL DKH__SQ_PRINT
C     +
C     +               ( NBAS,ZCORE (ZSCR1),"*** DKH diffs***" )
C             WRITE (*,*) ""
         ENDIF
C
C
