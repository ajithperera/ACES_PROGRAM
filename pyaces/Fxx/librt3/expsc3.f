      SUBROUTINE EXPSC3(T3,W,IADBLK,IADW,IRPIJK)
...
C
      ...
      IOFFBC =   IADW(IRPBC) - 1 + 
     1           IOFFVV(IRPC,IRPBC,6) * VRT(IRPA,2)
      IOFFAC =   IADW(IRPAC) - 1 + 
     1           IOFFVV(IRPC,IRPAC,6) * VRT(IRPB,2)
C
      CALL EXPW(T3(IOFFT3+1),W(IOFFBC+1),W(IOFFAC+1),
     1          IRPDPD(IRPAB,2),VRT(IRPC,1),VRT(IRPA,2),VRT(IRPB,2))
C
 1120 CONTINUE
C
      ELSE
...
C
     ...
     1           IOFFVV(IRPC,IRPAC,6) * VRT(IRPB,2)
C
      CALL EXPW2(T3(IOFFT3+1),W(IOFFBC+1),IRPDPD(IRPAB,2),
     1           VRT(IRPC,1),VRT(IRPA,2),VRT(IRPB,2))
C
 1130 CONTINUE
C
      ENDIF
