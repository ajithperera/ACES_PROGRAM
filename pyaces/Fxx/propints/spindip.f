C     
      SUBROUTINE SPinDip(VLIST,NOC,CENTR1,EXPS,CONT,LABEL,NPRIM,NCONTR,
     &                   ITRM,ITRN,CTRN,NCONT,BUF,IBUF,AOINT,LBUF,INDX,
     &                   NGROUP,VS,SCR,MXPRM2)
C
C &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
C &  Evaluates spin dipole contribution to the NMR coupling          &
...
C
     ...
     &          V(10)
C     
      DATA (LABELJA(I, 19), I = 1, 10) /'  SDXX  ', '  SDXY  ',
     &      '  SDXZ  ','  SDYY  ', '  SDYZ  ', '  SDZZ  ',
     &     4*'        '/
C     
      DATA (TITLE(I, 19), I = 1, 10)/'  SDXX  ','  SDXY  ','  SDXZ  ',
     &                               '  SDYY  ','  SDYZ  ','  SDZZ  ',
     &                             4*'        '/
C     DATA (TITLE(I, 19), I = 1, 10)/6H  SDXX, 6H  SDXY, 6H  SDXZ,
C    &      6H  SDYY, 6H  SDYZ, 6H  SDZZ, 4*6H      /
C                 
      IBTAND(I,J) = IAND(I,J)
      IBTOR(I,J)  = IOR(I,J)
      IBTXOR(I,J) = IEOR(I,J)
...
C
                       CALL FILDGR(JL, JM, JN, GAMAP, V, NT, DISTQD)
C
                       DO 51 L = 1, NT
                          VAL(L) = DD*V(L) + VAL(L)
 51                    CONTINUE
...
C                 Write (luout, 999) (VAL(IIII), IIII = 1 ,6)
C 999             format(3X, 6(5X,F12.5)) 
C&&&&&&&&&&&&&&&&
                 DO 30 M = 1, NT
                    VS(IJIND, M) = EFACTP*VAL(M) + VS(IJIND, M)
 30              CONTINUE
C     
 41           CONTINUE
 40        CONTINUE
