C
      SUBROUTINE MagDipl(VLIST,NOC,CENTR1,EXPS,CONT,LABEL,NPRIM,NCONTR,
     &                  ITRM,ITRN,CTRN,NCONT,BUF,IBUF,AOINT,LBUF,INDX,
     &                  NGROUP,VS,SCR,MXPRM2)
C
C &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
C &  Evaluates Magnetic dipole contribution to the NMR coupling      &
C &  constant. Coded by Ajith 03/94 following vprop.f style          &
...
C
     ...
     &          ITRN(256,1),CTRN(256,1),NCONTR(1),NPRIM(1),INDX(1)
      DIMENSION IHOLD(10),IADH(10),VS(MXPRM2,10),TEMP(2,3),TEMP1(2,1)
      DIMENSION BUF(LBUF,10),IBUF(IINTFP*LBUF,10),AOINT(1)
      DIMENSION TITLE(10,25),LFLAG(3),MFLAG(3),NFLAG(3)
      DIMENSION CENTR1(3),SCR(1),GBUF(600),IGNDX(600)
      DIMENSION LMN(200),JMN(200)
      DIMENSION A(3),B(3),EP(3),DISTQD(3),AAA(200),BBB(200),
     &          V(10),DMD(3),DISTQA(3),DV(3)
C     
      DATA LFLAG / 1, 0, 0/
      DATA MFLAG / 0, 1, 0/
      DATA NFLAG / 0, 0, 1/
C     
      DATA (LABELJA(I, 21), I = 1, 10) /'   MDX  ', '   MDY  ',
     &      '   MDZ  ',7*'        '/
C     
      DATA (TITLE(I, 21), I = 1, 10)/'   MDX  ','   MDY  ','   MDZ  ',
     &                             7*'        '/
C     DATA (TITLE(I, 21), I = 1, 10)/6H   MDX, 6H   MDY, 6H   MDZ
C    &      , 7*6H      /
C                 
      IBTAND(I,J) = IAND(I,J)
      IBTOR(I,J)  = IOR(I,J)
...
C
                  ...
                  NB = LMNVAL(3, JTYP)
C
C Compute the CA_X*[<|DEL|>] and store in the first three element
C of variable VAL. This term involves multification of usual dipole
C velocity integrals with the distance vector AC_X
C
C
                  DO 25 K = 1, 3
                     DISTQD(K) = EP(K) - CENTR1(K)
                     DISTQA(K) = A(K)  - CENTR1(K)
                     DMD(K)    = ZERO
 25               CONTINUE
...
C     
                       TEMP1(1, 1) = EFACTP*DD*V(1) + TEMP1(1, 1)
C                          
 212                CONTINUE
 211             CONTINUE
C
...
C
                       CALL OVLAP(JL, JM, JN, GAMAP, V, NT, DISTQD)
C     
                       TEMP1(2, 1) = EFACTP*DD*V(1) + TEMP1(2, 1)
C     
 214                CONTINUE
 213             CONTINUE
C
...
C
                 DX = LB*TEMP1(1, 1) - TWO*EXPS(JEX)*TEMP1(2, 1)
C
...
C
                 DY = MB*TEMP1(1, 1) - TWO*EXPS(JEX)*TEMP1(2, 1)
C
...
C
                 DZ = NB*TEMP1(1, 1) - TWO*EXPS(JEX)*TEMP1(2, 1)
C
C Evalute the contribution from shift of the origin
C 
                 DMD(1) = DISTQA(2)*DZ - DISTQA(3)*DY
                 DMD(2) = DISTQA(1)*DZ - DISTQA(3)*DX
                 DMD(3) = DISTQA(1)*DY - DISTQA(2)*DX
C
C Now compute the other contribution <|X_C X DEL|>. 
C 
           DO 1155 IIA = 1, 3
              DO 1156 JJA = 1, 2
                        TEMP(JJA, IIA) = ZERO
...
C     
                           Call OVLAP(JL, JM, JN, GAMAP, V, NT, DISTQD)
C     
                           TEMP(1, IIQ) = EFACTP*DD*V(1) + TEMP(1, IIQ)
C                          
 227                    CONTINUE
 226                 CONTINUE
C
...
C
                           CALL OVLAP(JL, JM, JN, GAMAP, V, NT, DISTQD)
C
                           TEMP(2, IIQ) = EFACTP*DD*V(1) + TEMP(2, IIQ)
C
 229                    CONTINUE
 228                 CONTINUE
 210              CONTINUE
...
C                 Write (luout, 999) (DMD(IIII), IIII = 1 ,3)
C 999             format(5X,F12.5, 5X,F12.5,5X,F12.5) 
C&&&&&&&&&&&&&&&&
C
                  DO 30 MM = 1, NT
                     VS(IJIND, MM) = DMD(MM) + VS(IJIND, MM)
 30               CONTINUE
C
 41            CONTINUE
 40         CONTINUE
...
C
              ...
              ELSE
                 CALL MXM_PROP(VS(1,M), IE, CONT(JCONT+1), JE, SCR, JCE)
