      SUBROUTINE WOT314(W,CORE,IADW,
     1                 IRPI,IRPJ,IRPK,IRPIJ,IRPIK,IRPJK,
     1                 I,J,K,ISPIN,LZOFF,LWOFF,SIGN)
C    1                 I,J,K,ISPIN,IMODE)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      DOUBLE PRECISION SIGN
      INTEGER LZOOFF,LWOFF
      INTEGER POP,VRT,DIRPRD
      DIMENSION W(1),CORE(1)
      DIMENSION IADW(8)
      DIMENSION IADT2(8),LENT2(8),IADG(8),LENG(8)
C
      COMMON /SYMINF/ NSTART,NIRREP,IRREPA(255),IRREPB(255),
     1                DIRPRD(8,8)
      COMMON /SYMPOP/ IRPDPD(8,22),ISYTYP(2,500),ID(18)
      COMMON /SYM/    POP(8,2),VRT(8,2),NTAA,NTBB,NF1AA,NF2AA,
     1                NF1BB,NF2BB
      COMMON /FILES/  LUOUT,MOINTS
C
c      COMMON /GAMLIS/ LISGO1,LISGO2,LISGO3,LISGO4,LISGV1,LISGV2,
c     1                                            LISGV3,LISGV4
c      COMMON /LISWI/  LWIC11,LWIC12,LWIC13,LWIC14,
c     1                LWIC15,LWIC16,LWIC17,LWIC18,
c     1                LWIC21,LWIC22,LWIC23,LWIC24,
c     1                LWIC25,LWIC26,LWIC27,LWIC28,
c     1                LWIC31,LWIC32,LWIC33,
c     1                LWIC34,LWIC35,LWIC36,
c     1                LWIC37,LWIC38,LWIC39,LWIC40,LWIC41,LWIC42
      COMMON /T3OFF/  IOFFVV(8,8,10),IOFFOO(8,8,10),IOFFVO(8,8,4)
C
      INDEX(I) = I*(I-1)/2
C
C     Generic subroutine to calculate the contribution of AAA/BBB T3
C     amplitudes to either ijka gammas or Hbar intermediates. The spin
C     orbital equations are :
C
C     W(la,ij) =   Sum  t(abc,ijk) * <bc//kl>     (IMODE = 1)
C                  b<c
C                   k
C
C     G(la,ij) = - Sum  t(abc,ijk) * t(bc,kl)     (IMODE = 2)
C                  b<c
C                   k
C
c      IF(IMODE.EQ.1)THEN
c      SIGN  =  1.0D+00
c      LISTG =  LWIC11 - 1 + ISPIN
c      LISTT =          13 + ISPIN
c      ENDIF
c      IF(IMODE.EQ.2)THEN
c      SIGN  = -1.0D+00
c      LISTG =  LISGO1 - 1 + ISPIN
c      LISTT =          43 + ISPIN
c      ENDIF
c      IF(IMODE.NE.1.AND.IMODE.NE.2)THEN
c      WRITE(LUOUT,1000) IMODE
c 1000 FORMAT(' @WOT314-F, Invalid value of IMODE. Given ',I10)
c      STOP
c      ENDIF
C
      LISTT = LWOFF + ISPIN
      LISTG = LZOFF + ISPIN
C
      IF(IRPI.EQ.IRPJ)THEN
      IJ = IOFFOO(IRPJ,IRPIJ,ISPIN) + INDEX(J-1) + I
      ELSE
      IJ = IOFFOO(IRPJ,IRPIJ,ISPIN) + (J-1)*POP(IRPI,ISPIN) + I
      ENDIF
C
      IF(IRPI.EQ.IRPK)THEN
      IK = IOFFOO(IRPK,IRPIK,ISPIN) + INDEX(K-1) + I
      ELSE
      IK = IOFFOO(IRPK,IRPIK,ISPIN) + (K-1)*POP(IRPI,ISPIN) + I
      ENDIF
C
      IF(IRPJ.EQ.IRPK)THEN
      JK = IOFFOO(IRPK,IRPJK,ISPIN) + INDEX(K-1) + J
      ELSE
      JK = IOFFOO(IRPK,IRPJK,ISPIN) + (K-1)*POP(IRPJ,ISPIN) + J
      ENDIF
C
C                        ABC  BC
C     G(IJ,LA) = - SUM  T    T
C                  B<C   IJK  KL
C                   K
C       AA AA            AAA  AA
C       BB BB            BBB  BB
C
C     W IS (B<C,A) REPRESENTATION OF T3 (A<B<C).
C
C     SET ADDRESSES FOR GAMMA INCREMENTS (L,A), LABELLED BY IRPA.
C
      DO   20 IRPA=1,NIRREP
      IRPL = DIRPRD(IRPA,IRPIJ)
      LENG(IRPA) = POP(IRPL,ISPIN) * VRT(IRPA,ISPIN)
      IF(IRPA.EQ.1)THEN
      IADG(IRPA) = 1
      ELSE
      IADG(IRPA) = IADG(IRPA-1) + LENG(IRPA-1)
      ENDIF
   20 CONTINUE
      CALL GETLST(CORE(1),IJ,1,2,IRPIJ,LISTG)
C
C     SET ADDRESSES FOR T2.
C
      DO   30 IRPL=1,NIRREP
      IRPKL = DIRPRD(IRPL,IRPK)
      IRPBC = IRPKL
      LENT2(IRPL) = IRPDPD(IRPBC,ISPIN) * POP(IRPL,ISPIN)
      IF(IRPL.EQ.1)THEN
      IADT2(IRPL) = IADG(NIRREP) + LENG(NIRREP)
      ELSE
      IADT2(IRPL) = IADT2(IRPL-1) + LENT2(IRPL-1)
      ENDIF
   30 CONTINUE
C
C     READ T2(B<C,L).
C
      DO   70 IRPL=1,NIRREP
      IRPA = DIRPRD(IRPIJ,IRPL)
      IF(POP(IRPL,ISPIN).EQ.0.OR.VRT(IRPA,ISPIN).EQ.0) GOTO 70
      IRPKL = DIRPRD(IRPK,IRPL)
C
      IF(IRPK.LT.IRPL)THEN
      DO   40    L=1,POP(IRPL,ISPIN)
      KL = IOFFOO(IRPL,IRPKL,ISPIN) + (L-1)*POP(IRPK,ISPIN) + K
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPKL,ISPIN)),
     1            KL,1,1,IRPKL,LISTT)
   40 CONTINUE
      ENDIF
C
      IF(IRPK.GT.IRPL)THEN
      KL = IOFFOO(IRPK,IRPKL,ISPIN) + (K-1)*POP(IRPL,ISPIN) + 1
      CALL GETLST(CORE(IADT2(IRPL)),
     1            KL,POP(IRPL,ISPIN),1,IRPKL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL)),LENT2(IRPL))
      ENDIF
C
      IF(IRPK.EQ.IRPL)THEN
      DO   60    L=1,POP(IRPL,ISPIN)
C
      IF(K.LT.L)THEN
      KL = IOFFOO(IRPL,IRPKL,ISPIN) + INDEX(L-1) + K
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPKL,ISPIN)),
     1            KL,1,1,IRPKL,LISTT)
      ENDIF
C
      IF(K.GT.L)THEN
      KL = IOFFOO(IRPK,IRPKL,ISPIN) + INDEX(K-1) + L
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPKL,ISPIN)),
     1            KL,1,1,IRPKL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPKL,ISPIN)),
     1            IRPDPD(IRPKL,ISPIN))
      ENDIF
C
      IF(K.EQ.L)THEN
      CALL ZERO(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPKL,ISPIN)),
     1          IRPDPD(IRPKL,ISPIN))
      ENDIF
   60 CONTINUE
C
      ENDIF
C
C     WE NOW HAVE T2 IN THE FORM T2(B<C,L) AND T3 IN THE FORM
C     T3(B<C,A). CONTRACT TOGETHER TO FORM G(L,A).
C
      CALL XGEMM('T','N',
     1           POP(IRPL,ISPIN),VRT(IRPA,ISPIN),IRPDPD(IRPKL,ISPIN),
     1            SIGN,CORE(IADT2(IRPL)),IRPDPD(IRPKL,ISPIN),
     1                    W(IADW(IRPA)),IRPDPD(IRPKL,ISPIN),
     1            1.0D+00,
     1           CORE(IADG(IRPA)),POP(IRPL,ISPIN))
   70 CONTINUE
      CALL PUTLST(CORE(1),IJ,1,2,IRPIJ,LISTG)
C
C                        ABC  BC
C     G(IK,LA) =   SUM  T    T
C                  B<C   IJK  JL
C                   J
C       AA AA            AAA  AA
C       BB BB            BBB  BB
C
      DO  120 IRPA=1,NIRREP
      IRPL = DIRPRD(IRPA,IRPIK)
      LENG(IRPA) = POP(IRPL,ISPIN) * VRT(IRPA,ISPIN)
      IF(IRPA.EQ.1)THEN
      IADG(IRPA) = 1
      ELSE
      IADG(IRPA) = IADG(IRPA-1) + LENG(IRPA-1)
      ENDIF
  120 CONTINUE
      CALL GETLST(CORE(1),IK,1,2,IRPIK,LISTG)
C
      DO  130 IRPL=1,NIRREP
      IRPJL = DIRPRD(IRPL,IRPJ)
      IRPBC = IRPJL
      LENT2(IRPL) = IRPDPD(IRPBC,ISPIN) * POP(IRPL,ISPIN)
      IF(IRPL.EQ.1)THEN
      IADT2(IRPL) = IADG(NIRREP) + LENG(NIRREP)
      ELSE
      IADT2(IRPL) = IADT2(IRPL-1) + LENT2(IRPL-1)
      ENDIF
  130 CONTINUE
C
      DO  170 IRPL=1,NIRREP
      IRPA = DIRPRD(IRPIK,IRPL)
      IF(POP(IRPL,ISPIN).EQ.0.OR.VRT(IRPA,ISPIN).EQ.0) GOTO 170
      IRPJL = DIRPRD(IRPJ,IRPL)
C
      IF(IRPJ.LT.IRPL)THEN
      DO  140    L=1,POP(IRPL,ISPIN)
      JL = IOFFOO(IRPL,IRPJL,ISPIN) + (L-1)*POP(IRPJ,ISPIN) + J
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
     1            JL,1,1,IRPJL,LISTT)
  140 CONTINUE
      ENDIF
C
      IF(IRPJ.GT.IRPL)THEN
C     DO  150    L=1,POP(IRPL,ISPIN)
C     JL = IOFFOO(IRPJ,IRPJL,ISPIN) + (J-1)*POP(IRPL,ISPIN) + L
C     CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
C    1            JL,1,1,IRPJL,LISTT)
C 150 CONTINUE
      JL = IOFFOO(IRPJ,IRPJL,ISPIN) + (J-1)*POP(IRPL,ISPIN) + 1
      CALL GETLST(CORE(IADT2(IRPL)),
     1            JL,POP(IRPL,ISPIN),1,IRPJL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL)),LENT2(IRPL))
      ENDIF
C
      IF(IRPJ.EQ.IRPL)THEN
      DO  160    L=1,POP(IRPL,ISPIN)
C
      IF(J.LT.L)THEN
      JL = IOFFOO(IRPL,IRPJL,ISPIN) + INDEX(L-1) + J
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
     1            JL,1,1,IRPJL,LISTT)
      ENDIF
C
      IF(J.GT.L)THEN
      JL = IOFFOO(IRPJ,IRPJL,ISPIN) + INDEX(J-1) + L
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
     1            JL,1,1,IRPJL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
     1            IRPDPD(IRPJL,ISPIN))
      ENDIF
C
      IF(J.EQ.L)THEN
      CALL ZERO(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPJL,ISPIN)),
     1          IRPDPD(IRPJL,ISPIN))
      ENDIF
  160 CONTINUE
C
      ENDIF
C
      CALL XGEMM('T','N',
     1           POP(IRPL,ISPIN),VRT(IRPA,ISPIN),IRPDPD(IRPJL,ISPIN),
     1           -SIGN,CORE(IADT2(IRPL)),IRPDPD(IRPJL,ISPIN),
     1                    W(IADW(IRPA)),IRPDPD(IRPJL,ISPIN),
     1            1.0D+00,
     1           CORE(IADG(IRPA)),POP(IRPL,ISPIN))
  170 CONTINUE
      CALL PUTLST(CORE(1),IK,1,2,IRPIK,LISTG)
C
C                        ABC  BC
C     G(JK,LA) = - SUM  T    T
C                  B<C   IJK  IL
C                   K
C       AA AA            AAA  AA
C       BB BB            BBB  BB
C
      DO  220 IRPA=1,NIRREP
      IRPL = DIRPRD(IRPA,IRPJK)
      LENG(IRPA) = POP(IRPL,ISPIN) * VRT(IRPA,ISPIN)
      IF(IRPA.EQ.1)THEN
      IADG(IRPA) = 1
      ELSE
      IADG(IRPA) = IADG(IRPA-1) + LENG(IRPA-1)
      ENDIF
  220 CONTINUE
      CALL GETLST(CORE(1),JK,1,2,IRPJK,LISTG)
C
      DO  230 IRPL=1,NIRREP
      IRPIL = DIRPRD(IRPL,IRPI)
      IRPBC = IRPIL
      LENT2(IRPL) = IRPDPD(IRPBC,ISPIN) * POP(IRPL,ISPIN)
      IF(IRPL.EQ.1)THEN
      IADT2(IRPL) = IADG(NIRREP) + LENG(NIRREP)
      ELSE
      IADT2(IRPL) = IADT2(IRPL-1) + LENT2(IRPL-1)
      ENDIF
  230 CONTINUE
C
      DO  270 IRPL=1,NIRREP
      IRPA = DIRPRD(IRPJK,IRPL)
      IF(POP(IRPL,ISPIN).EQ.0.OR.VRT(IRPA,ISPIN).EQ.0) GOTO 270
      IRPIL = DIRPRD(IRPI,IRPL)
C
      IF(IRPI.LT.IRPL)THEN
      DO  240    L=1,POP(IRPL,ISPIN)
      IL = IOFFOO(IRPL,IRPIL,ISPIN) + (L-1)*POP(IRPI,ISPIN) + I
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
     1            IL,1,1,IRPIL,LISTT)
  240 CONTINUE
      ENDIF
C
      IF(IRPI.GT.IRPL)THEN
C     DO  250    L=1,POP(IRPL,ISPIN)
C     IL = IOFFOO(IRPI,IRPIL,ISPIN) + (I-1)*POP(IRPL,ISPIN) + L
C     CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
C    1            IL,1,1,IRPIL,LISTT)
C 250 CONTINUE
      IL = IOFFOO(IRPI,IRPIL,ISPIN) + (I-1)*POP(IRPL,ISPIN) + 1
      CALL GETLST(CORE(IADT2(IRPL)),
     1            IL,POP(IRPL,ISPIN),1,IRPIL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL)),LENT2(IRPL))
      ENDIF
C
      IF(IRPI.EQ.IRPL)THEN
      DO  260    L=1,POP(IRPL,ISPIN)
C
      IF(I.LT.L)THEN
      IL = IOFFOO(IRPL,IRPIL,ISPIN) + INDEX(L-1) + I
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
     1            IL,1,1,IRPIL,LISTT)
      ENDIF
C
      IF(I.GT.L)THEN
      IL = IOFFOO(IRPI,IRPIL,ISPIN) + INDEX(I-1) + L
      CALL GETLST(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
     1            IL,1,1,IRPIL,LISTT)
      CALL VMINUS(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
     1            IRPDPD(IRPIL,ISPIN))
      ENDIF
C
      IF(I.EQ.L)THEN
      CALL ZERO(CORE(IADT2(IRPL) + (L-1)*IRPDPD(IRPIL,ISPIN)),
     1          IRPDPD(IRPIL,ISPIN))
      ENDIF
  260 CONTINUE
C
      ENDIF
C
      CALL XGEMM('T','N',
     1           POP(IRPL,ISPIN),VRT(IRPA,ISPIN),IRPDPD(IRPIL,ISPIN),
     1            SIGN,CORE(IADT2(IRPL)),IRPDPD(IRPIL,ISPIN),
     1                    W(IADW(IRPA)),IRPDPD(IRPIL,ISPIN),
     1            1.0D+00,
     1           CORE(IADG(IRPA)),POP(IRPL,ISPIN))
  270 CONTINUE
      CALL PUTLST(CORE(1),JK,1,2,IRPJK,LISTG)
      RETURN
      END
