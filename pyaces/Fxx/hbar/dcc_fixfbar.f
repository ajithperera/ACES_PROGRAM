      ...
      subroutine Dcc_fixfbar(icore, maxcor,iuhf)
C
...
C
 ...
 100  CONTINUE
      CALL PUTLST(ICORE(I0AA),1,1,1,9,92)
C                                 _
...
c      call addfdiag(icore(i0e), icore(i0aa),nirrep, pop(1,1) )
      CALL PUTLST(ICORE(I0AA),1,1,1,9,91)
C     
C
C   NOW the beta part
...
c           call addfdiag(icore(i0e+noe), icore(i0bb),nirrep,vrt(1,2) )
           CALL PUTLST(ICORE(I0BB),1,1,1,10,92)
C
C                                   _
C  1/2 SUM E  F(e,m) T(e,j)   =. <j|f|m>
C
...
c           call addfdiag(icore(i0e), icore(i0bb),nirrep,pop(1,2) )
           CALL PUTLST(ICORE(I0BB),1,1,1,10,91)
