      ...
      subroutine Ecp_basis_init_4grd(namstr,iqmstr,jcostr,nucstr,
     &                               nrcstr,NHARM,Nmax)
c-----------------------------------------------------------------------      
...
C
              if (ifeld(iat).ne.1) then
                 ...
                 iccoff(iat,ilq,ijco)=nstrt(icount)
...
C
CSSS                 contr(icount,inrco)=PRICCF(iccoff(iat,ilq,ijco)+
CSSS     &                               inuco+   
CSSS     &                               nucstr(iat,ilq,ijco)*(inrco-1))
