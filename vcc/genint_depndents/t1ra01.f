...
C     
            DISTAR=IRPDPD(IRREPDO,ISYTYP(2,22+ISPIN))
            DSZTAR=IRPDPD(IRREPDO,ISYTYP(1,22+ISPIN))
            DSZW  =IRPDPD(IRREPDO,ISYTYP(1,LSTINT))
            DISW  =IRPDPD(IRREPDO,ISYTYP(2,LSTINT))
...
C
C READ IN TRANSPOSE OF INTEGRAL MATRIX, RETURNED MATRIX IS 
C I(BM;F<E) [I(bm;f<e)].
C
            IF(IUHF.NE.0)THEN
...
C
     ...
     &                     ICORE(I020),ICORE(I010))
               CALL SUMSYM3(ICORE(I010),ICORE(I020),ABFULL(1),1,
     &                      ISPIN,LSTFEA)
            ENDIF
C     
...
C
     ...
     &                  ICORE(I040))
            CALL SYMTR3(IRREPDO,VRT(1,ISPIN),POP(1,ISPIN),DSZTAR,
...
C     
               ...
               IF(MIN(NROWI,NCOLT,NCOLI).GT.0)THEN
C
                  CALL XGEMM('N','T',NROWI,NCOLT,NCOLI,ALPHA,
     &                        ICORE(IOFFI),NROWI,ICORE(IOFFT),
     &                        NCOLT,BETA,ICORE(IOFFZ),NROWI)
               ENDIF
C
               IOFFI=IOFFI+NROWI*NCOLI*IINTFP
 5101       CONTINUE
