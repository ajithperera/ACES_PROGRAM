     ...
     &                         Print_header,Stationary_pt,Max_cycles,
     &                         Do_next,Abort)
C
...
C
      ...
      Double Precision Deltaq_hist(Max_cycles),Deltag_hist(Max_cycles)
...
C
     ...
     &                Grad_K1C(1,1))
          Call Getrec(20,'JOBARC','TOTENERG', IINTFP, E_begin)
          E_currnt  = E_begin 
      Write(6,*)
...
C
     ...
     &               Grad_K1C(1,1))
      ...
      Write(6,*)
      Write(6,"(a,a,ES16.9E2)") "@-Constr_search: The energy of the ",
...
C
C Define the vectors k+1: (qk+1 - qk) ((from coordinates with
C mass weighing).
C 
      Do Iatom = 1, Nreals
         Ioff = 1 + 3*(Iatom - 1) 
...
C
      Vec_K1C_length = Vec_K1C_Stat(5)*Dsqrt(Dble(3*Nreals))
         ...
         Write(6, "(4F17.13)") Vec_K1C_length,
...
C
     ...
     &               1)
      ...
      Deltaq_hist(Ncycles) = Delta_q
      Deltag_hist(Ncycles) = Delta_g
C
      Write(6,*)
      Write(6,"(a)") "Check for convergence"
      Write(6,"(a,I2,1x,I2)") "Ncycles and Max_cycles:",Ncycles,
     &                         Max_cycles
...
CSSS      Write(6,"(a,2F17.13)") "Delta_q1",Delta_q1
      If (Delta_g .LT. G_cutoff .AND. Delta_q .LT. R_cutoff) Then
         Constr_Min = "Found"       
      Elseif (Delta_q .Lt. Too_small) Then
         Write(6,*) 
         Write(6,"(3a)") " The geometry update is too small and",
     &                   " less than the convergence tolerence "
         Write(6,"(2a)") " but the gradients are not. Make sure that",
     &                   " the starting structure is a stationary"
         Write(6,"(a)")  " point."

         Write(6,"(10x,10a)") "--------------------------------------"
         Write(6,"(10x,2a)")  "     Delta_q","          Delta_g"
         Write(6,"(10x,10a)") "--------------------------------------"
         Do Icycle = 1, Ncycles 
            Write(6,"(10x,2F17.13)") Deltaq_hist(Icycle),
     &                               Deltag_hist(Icycle)
         Enddo 
         Write(6,"(10x,10a)") "--------------------------------------"
         Write(6,"(2a,2ES16.9E2)") " The current convergence",
     &                             " thresholds:",R_cutoff,G_cutoff
         Call Errex
      Elseif (Ncycles .Ge. Max_cycles) Then
         Write(6,"(2a,i2,a)") " The constrained search did not converge"
     &                       ," in", Max_cycles,  " allotted cycles." 
     ...
     &                             " thresholds:",R_cutoff,G_cutoff
         If (Do_Next) Then
            Write(6,"(2a)") " Proceeding to search for the next IRC",
     &                      " point."
            Abort = .True. 
            Goto 100
         Else
...
C
         ...
         Write(IUnitO, "(a,a7,2a)") " The coordinates map for the ",
     &                               String, " IRC search",
     &                             " (in sqrt(amu)*Angs.)."
         Write(Iunito,*) 
         Endif 
     ...
     &                          (Coords_K1CM(Ioff+J)/B2ang,J = 1,3)
            Ioff = Ioff + 3 
         Enddo
         Write(IUnitO,*)
...
C
      Write(6,*)
      Write(6,"(3a,2(1x,ES16.9E2))") "@-Constr_search: The energy",
     &                             " of the current and pre. ",
     &                             "IRC point = ", E_currnt,
     &                                             E_begin
C
