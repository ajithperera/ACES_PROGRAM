...
c INTERNAL VARIABLES
      integer dimargs, iuhf, i
      double precision var 
      character*80 module
      character*80 args(8)
      character*10 prop
      logical streaming,stationary,drive_irc 
...
c
      if (module(1:11).eq.'PES_scan ') then
         ...
         Stationary  = .False. 
         Drive_IRC   = .True.
         call read_scannmlist_init(Streaming,Stationary,Drive_IRC,Prop)
         If (Streaming .OR. Stationary) Drive_IRC = .False.
         call pes_scan_main(Streaming,Stationary,Drive_IRC,Prop)
         goto 9998
      endif
...
c These modules require lists.
      ...
      if (module(1:7).eq.'extrap ') then
         ...
         call extrap_main(args, dimargs,.true.)
         goto 9997
      end if
