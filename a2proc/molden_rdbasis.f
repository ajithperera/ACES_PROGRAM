...
c-----------------------------------------------------------------------
      ...
      character*80 wrk2
      character*80 wrk3 
      character*30 cbasnam(mxatoms), Basnam_hold(mxatoms)
...
c
        ...
        ENDIF
c
C This block is chnaged to accomodate to changes to basis
...
c      o go back for the next read
        ...
        Else
            Do Iatm = 1,  Num_unique
               read(4,'(A)',end=910)Basnam_hold(iatom)
            Enddo
C
            Katm  = 0
            Do Iatm = 1, Natoms 
               Do Jatm = 1, Num_unique
                If (iatchrg(Iatm) .EQ. iatchrg(Jatm)) Then
                    Katm = Katm + 1
