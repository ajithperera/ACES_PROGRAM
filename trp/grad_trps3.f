      ...
      SUBROUTINE GRAD_TRPS3(T1A,T1B,FOVA,FOVB,D1T1A,D1T1B,S1A,S1B,
...
C
      IF (.NOT. FINAL) THEN
      CALL D1T3_TRP( ICORE(I000),ICORE(I010),
...
CSSS      CALL RMD314(ICORE(I000),ICORE(I060),LENABC,DIJK)
      ...
      Write(*,*)
      Call checksum("T3(C)+T3D-(D)", ICORE(IStart), Lenabc)
...
C
     ...
     &               146)
         ...
         CALL D1T3_TRP( ICORE(IL00),ICORE(IL10),
     1    ICORE(I030),ICORE(I040),ICORE(I050),
     1    OOOV(OOOVAD(IRPIJ,2)+IOFFOO(IRPJ,IRPIJ,2)*IRPDPD(IRPIJ,17),2),
     1    OOOV(OOOVAD(IRPJK,4)+IOFFOO(IRPJ,IRPJK,5)*IRPDPD(IRPJK,12),4),
     1    OOOV(OOOVAD(IRPIK,4)+IOFFOO(IRPI,IRPIK,5)*IRPDPD(IRPIK,12),4),
...
C#ifdef _DEBUG_NONHFTRP
     ...
     &               LENABC,DIJK, E5TEST,"grad_trps3.F")
