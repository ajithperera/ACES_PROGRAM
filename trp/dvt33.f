      ...
      SUBROUTINE DVT33(DVVA,DVVB,T3C,WC,T3D,WD,
     1                  IADT3,IADW,IRPIJK,NVRTA,NVRTB)
...
C
      ...
      ISPIN2 = 1
C
C     ROUTINE TO COMPUTE CONTRIBUTION OF T3 BBA TO DIAGONAL
...
C                  B B A    BBA   BBA
C
C     T3 IS (A<B,C) (BBA) REPRESENTATION OF BBA TRIPLES.
C
      DO   30 IRPC=1,NIRREP
...
C
      ...
      DVVA(C + COFF(IRPC)) = DVVA(C + COFF(IRPC)) + 
...
C                  B B A    BBA   BBA
C
C     W IS (B,C,A) REPRESENTATION OF T3 (A<B,C) (BBA) TRIPLES.
C
...
C
      DVVB(A+COFF(IRPA)) = DVVB(A+COFF(IRPA)) +
