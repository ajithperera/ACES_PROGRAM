      ...
      SUBROUTINE GRAD_TRPS(ICORE,MAXCOR,IUHF,E4T,FINAL)
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI_TRP(ICORE,MAXCOR,1,.FALSE.)
      ENDIF

c     Calculate triples contributions to W(abci) and W(ijka) (CCSDT).
C
      IF(CCSDT) CALL W45T3DRV(ICORE,MAXCOR,IUHF)
C
...
cYAU      WRITE(6,2001) MAXCOR,NALLOC,NFREE
      ...
      CALL GRAD_TRPS14(ICORE(I010),ICORE(I020),ICORE(I025),ICORE(I030),
     1            ICORE(I040),
...
C
      Write(*,*) "The derivative level", IDRLVL 
      IF(IDRLVL.EQ.0)THEN
       IF(IUHF.EQ.0)THEN
       ...
       ENDIF
      ELSE
       Write(*,*) "Enetring setooov"
       cALL SETOOOV_TRP(ICORE,MAXCOR,IUHF,1)
      ENDIF
C
C     I000   T1   alpha
C     I010   T1   beta
...
cYAU      WRITE(6,2011) MAXCOR,NALLOC,NFREE
      ...
      ENDIF
      call wallclock(in_year,in_mon,in_mday,in_hour,in_min,in_sec)
      CALL GRAD_TRPS2(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
     ...
     1           IUHF,.FALSE.,FINAL)
      ...
      IF(IDRLVL.EQ.0)THEN
       ...
       ENDIF
      ELSE
       CALL SETOOOV_TRP(ICORE,MAXCOR,IUHF,2)
      ENDIF
 2000 CONTINUE
C
      IF(IUHF.EQ.0.OR.
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_TRP(ICORE,MAXCOR,1,.FALSE.)
C
...
cYAU      WRITE(6,2021) MAXCOR,NALLOC,NFREE
      ...
      ENDIF
      call wallclock(in_year,in_mon,in_mday,in_hour,in_min,in_sec)
      CALL GRAD_TRPS3(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
     1           ICORE(I040),ICORE(I050),ICORE(I060),ICORE(I070),
     1           ICORE(I080),ICORE(I090),ICORE(I100),ICORE(I110),
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_TRP(ICORE,MAXCOR,2,.FALSE.)
C
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI_TRP(ICORE,MAXCOR,2,.FALSE.)
      ENDIF
C
C     For CCSDT negate certain MBEJ lists.
C
...
C
      IF(CCSDT)THEN
        CALL HBARCRAP_TRP(ICORE,MAXCOR,IUHF,.FALSE.)
...
C
      IF(IFLAGS2(138).EQ.2)THEN
        if (iFlags2(5).ne.0) then
           IDRLVL = 1
           WRITE(6,2052)
        end if
        if (iFlags(54)       .eq.3) then
           IDRLVL = IDROLD
           WRITE(6,2053) IDRLVL
        end if
      ENDIF
