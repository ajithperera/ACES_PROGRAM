      ...
      SUBROUTINE GRAD_TRPS14(OOOV,OOOV2,GOOOV,BUF,EVAL,
...
c
      ...
      CALL D1T14_TRP(ICORE(I000),ICORE(I040),ICORE(I050),ICORE(I060),
     1 OOOV(OOOVAD(IRPIJ) + 
...
C on the disk. 
     ...
     1 IRPDPD(IRPJK,15+ISPIN),NLEFT,.FALSE.)
      ...
      DIJK = EVAL(ABSOCC(I,IRPI,ISPIN)) + EVAL(ABSOCC(J,IRPJ,ISPIN))
     1                                  + EVAL(ABSOCC(K,IRPK,ISPIN))
CSSS      CALL RMD314(ICORE(IM50),ICORE(I100),POPT3(IRPIJK),DIJK)
...
CSSS     &               E4T14,"grad_trps14.F")
C
      CALL DCOPY(POPT3(IRPIJK), ICORE(ISTART), 1, ICORE(IM50), 1)
      ENDIF 
C
...
C
         CALL D1T14_TRP(ICORE(IL00),ICORE(I040),ICORE(I050),ICORE(I060),
     1              OOOV(OOOVAD(IRPIJ) +
     1              IOFFOO(IRPJ,IRPIJ,ISPIN)*IRPDPD(IRPIJ,15+ISPIN)),
     1              OOOV(OOOVAD(IRPIK) +
     1              IOFFOO(IRPK,IRPIK,ISPIN)*IRPDPD(IRPIK,15+ISPIN)),
...
C
     ...
     &               POPT3(IRPIJK),DIJK,
     &               E5TEST,"grad_trps14.F")
CSSS        WRITE(6,*) ' @TRPS14-I, E5ST from disconnected triples ',
CSSS     &                E5TEST
CSSS      Write(*,*)
CSSS      Call checksum("T3(C)+T3D-(I)", ICORE(I140), POPT3(IRREPIJK))
C#endif 
      IF (.NOT. FINAL) 
     & CALL RMD314(ICORE(I140),ICORE(I100),POPT3(IRPIJK),DIJK)
