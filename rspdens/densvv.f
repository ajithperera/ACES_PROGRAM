...
250    CONTINUE
CSSS        Call checksum("T(e,i)*L(j,e)->DVV    :",DVV(1+(ISPIN-1)*
CSSS     &                 NF2(ISPIN)),NF2(ISPIN))
CSSS         Write(6,*) Nf2(1),Nf2(2),nf1(1),nf1(2)
         If (ispin.eq.1) Write(6,"(a)") "T(A,M)*L(M,B)->DVV:"
         If (ispin.eq.2) Write(6,"(a)") "T(a,m)*L(m,b)->DVV:"
         Call dstat("Term-11 ",dvv(1+(Ispin-1)*nf2(ispin)),nvrto(ispin),
     &               nvrto(ispin),.true.,"VV",ispin)
         Call dzero(Dvv(1+(Ispin-1)*nf2(ispin)),nf2(ispin))
300   CONTINUE
C
      ENDIF
C
...
100    CONTINUE
       ENDIF
CSSS        Call checksum("T(im,ef)*L(jm,ef)->DVV:",DVV(1+(ISPIN-1)*
CSSS     &                 NF2(ISPIN)),NF2(ISPIN))
        If (ispin.eq.1)Write(6,"(a)")"T(AE,MN)*L(MN,BE)(AAAA)->DVV:"
        If (ispin.eq.2)Write(6,"(a)")"T(ae,mn)*L(mn,be)(BBBB)->DVV:"
        Call dstat("Term-12",dvv(1+(Ispin-1)*nf2(ispin)),nvrto(ispin),
     &            nvrto(ispin),.true.,"VV",ispin)
        Call dzero(Dvv(1+(Ispin-1)*nf2(ispin)),nf2(ispin))
C
C       AB SPIN CASE
...
CSSS        Call checksum("T(im,ef)*L(jm,ef)->DVV:",DVV(1+(ISPIN-1)*
CSSS     &                 NF2(ISPIN)),NF2(ISPIN))
         Write(6,"(a)")"T(Ae,Mn)*L(Mn,Be)(ABAB)->DVV:"
         Call dstat("Term-12",dvv(1+(Ispin-1)*nf2(ispin)),nvrto(ispin),
     &              nvrto(ispin),.true.,"VV",ispin)
C
