...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__2D2_SPNSPN_CPLINGZZ_PQ_INTEGRALS
     +
     +                    ( SHELLP,SHELLQ,
     +                      MIJ,MKL,NGQP,NGQEXQ,
     +                      PX,PY,PZ,QX,QY,QZ,
     ...
     +                               INT2DZ2 )
C------------------------------------------------------------------------
C  OPERATION   : ERD__2D2_SPNSPN_CPLINGZZ_PQ_INTEGRALS
...
CSSS                 QROOT = RTS(N) *  RTS(N) * RHOSQR * INVRT
                  ...
                  INT2DX2 (N,0,0) = INT2DX (N,0,0)
                  INT2DY2 (N,0,0) = INT2DY (N,0,0)
                  INT2DZ2 (N,0,0) = (PQZ  * INT2DZ1 (N,0,0) 
     +                                    + INT2DZ  (N,0,0) 
     +                                    * RHOINV)
     +                                    * QROOT
                  DO K = 1, SHELLQ
                     ...
                     INT2DX2 (N,0,K) = INT2DX (N,0,K)
                     INT2DY2 (N,0,K) = INT2DY (N,0,K)
                     INT2DZ2 (N,0,K) = (PQZ  * INT2DZ1 (N,0,K) -
     +                                  QKL  * INT2DZ1 (N,0,K1)  * K  
     +                                       + INT2DZ (N,0,K)
     +                                       * RHOINV)
     +                                       * QROOT
                  ...
                  DO I = 1, SHELLP
                     ...
                     INT2DX2 (N,I,0) = INT2DX (N,I,0)
                     INT2DY2 (N,I,0) = INT2DY (N,I,0)
                     INT2DZ2 (N,I,0) = (PQZ  * INT2DZ1 (N,I,0)  +
     +                                  PIJ  * INT2DZ1 (N,I1,0) * I 
     +                                       + INT2DZ  (N,I,0)
     +                                       * RHOINV)
     +                                       * QROOT

C       write(*,*) INT2DZ (N,I,1),INT2DZ (N,I1,1),INT2DZ (N,I,0),
C     +            INT2DZ (N,I1,0), PQZ,PIJ,QKL,PQZC,PIJ,ZC,
C     +            PIJ*ZC,INT2DZ1 (N,I,0),(PQZ*INT2DZ (N,1,1)
c     +            + PIJ*INT2DZ (N,0,1))
C       Write(*,*)
                  ENDDO
                  ...
                  DO I = 1,  SHELLP
                     DO K = 1,  SHELLQ
                        ...
                        INT2DX2 (N,I,K) = INT2DX (N,I,K)
                        INT2DY2 (N,I,K) = INT2DY (N,I,K)
                        INT2DZ2 (N,I,K) = ( PQZ  * INT2DZ1 (N,I,K)  +
     +                                      PIJ  * INT2DZ1 (N,I1,K) * I  
     +                                    - QKL  * INT2DZ1 (N,I,K1) * K 
     +                                           + INT2DZ  (N,I,K)
     +                                           * RHOINV)
     +                                           * QROOT
                     ENDDO
...
CSSS                 QROOT = RTS(N) *  RTS(N) * RHOSQR * INVRT
     ...
     +                                     * QROOT
                  ...
                  INT2DX2 (N,0,1) = INT2DX (N,0,1)
                  INT2DY2 (N,0,1) = INT2DY (N,0,1)
                  INT2DZ2 (N,0,1) = (PQZ   * INT2DZ1 (N,0,1) -
     +                               QKL   * INT2DZ1 (N,0,0) 
     +                                     + INT2DZ  (N,0,1)
     +                                     * RHOINV) 
     +                                     * QROOT
...
CSSS                 QROOT = RTS(N) *  RTS(N) * RHOSQR * INVRT
     ...
     +                                     * QROOT
                  ...
                  INT2DX2 (N,1,0) = INT2DX (N,1,0)
                  INT2DY2 (N,1,0) = INT2DY (N,1,0)
                  INT2DZ2 (N,1,0) = (PQZ   * INT2DZ1 (N,1,0) +
     +                               PIJ   * INT2DZ1 (N,0,0)    
     +                                     + INT2DZ  (N,1,0)
     +                                     * RHOINV)
     +                                     * QROOT

C       write(*,*) INT2DZ (N,1,1),INT2DZ (N,0,1),INT2DZ (N,1,0),
C     +            INT2DZ (N,0,0),PQZ,PIJ,QKL,PQZC,PIJ,ZC,PIJZC,
C     +            INT2DZ1 (N,1,0),(PQZ*INT2DZ (N,1,1)+
C     +             PIJ*INT2DZ (N,0,1))
C       write(*,*)
...
C
                 IF (SHELLQ.LE.SHELLP) THEN
                    ...
                    DO K = 1,SHELLQ
                       ...
                       INT2DX2 (N,1,K) = INT2DX (N,1,K)
                       INT2DY2 (N,1,K) = INT2DY (N,1,K)
                       INT2DZ2 (N,1,K) = (PQZ  *
     +                                           INT2DZ1 (N,1,K) +
     +                                    PIJ  *      
     +                                           INT2DZ1 (N,0,K) -
     ...
     +                                             * QROOT
                 ...
                 ELSE
                    ...
                    DO I = 1,SHELLP
                       ...
                       INT2DX2 (N,I,1) = INT2DX (N,I,1)
                       INT2DY2 (N,I,1) = INT2DY (N,I,1)
                       INT2DZ2 (N,I,1) = (PQZ  *
     +                                           INT2DZ1 (N,I,1)  +
     +                                    PIJ  * I  *
     +                                           INT2DZ1 (N,I1,1) -
