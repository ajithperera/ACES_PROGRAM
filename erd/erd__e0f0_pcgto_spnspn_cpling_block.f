...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__E0F0_PCGTO_SPNSPN_CPLING_BLOCK
     +
     +                    ( NBATCH,NINT2D,
     ...
     +                      INT2DX2,INT2DY2,INT2DZ2,
     +
     +                                BATCH )
     +
C------------------------------------------------------------------------
C  OPERATION   : ERD__E0F0_PCGTO_SPNSPN_CPLING_BLOCK
C  MODULE      : ELECTRON REPULSION INTEGRALS DIRECT
C  MODULE-ID   : ERD
C  SUBROUTINES : ERD__RYS_ROOTS_WEIGHTS
C                ERD__2D_COEFFICIENTS
C                ERD__2D_SPNSPN_CPLING_PQ_INTEGRALS
C                ERD__INT2D_TO_E000
C                ERD__INT2D_TO_E0F0
C                ERD__2D_ATOM_COEFFICIENTS
C                ERD__2D_SPNSPN_CPLING_ATOM_PQ_INTEGRALS
...
C
         ...
         DOUBLE PRECISION  INT2DX2  (1:NINT2D)
         DOUBLE PRECISION  INT2DY2  (1:NINT2D)
         DOUBLE PRECISION  INT2DZ2  (1:NINT2D)
...
CSSS         CASE2D_1 = 3 * MIN0 (2,SHELLQ+1) + MIN0 (2,SHELLP) + 1
C
C
...
CSSS             write(*,*) "CASE2D", CASE2D_0,CASE2D_1
             CALL    ERD__2D_SPNSPN_CPLING_PQ_INTEGRALS
     +
     ...
     +                                INT2DZ2 )
             ...
             IF (SHELLQ.EQ.0) THEN
                 ...
                 IF (XX .EQ. 1 .OR. YY .EQ. 1 .OR. ZZ .EQ.1) THEN

C       write(6,*) " The component", XX, YY, ZZ
C       do i=1,NINT2D
C          write(6,"(1x,3(F10.5))") INT2DX2(i),INT2DY2(i),INT2DZ2(i)
C       enddo 
     ...
     +
     +
C             Write(*,*) "EQ0-1 batches"
C             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)
...
C             Write(*,"(3(1x,F15.10))") (BATCH (I), I=1, NBATCH)
     ...
     +
         ...
         ELSE
C             write(*,*) "Enter  ERD__2D_ATOM_SPNSPN_CPLING_PQ_INTEGRALS"
             CALL    ERD__2D_ATOM_SPNSPN_CPLING_PQ_INTEGRALS
     +
     +                    ( MIJ,MKL,MIJKL,NGQP,MGQIJKL,
     ...
     +                                INT2DZ2 )
     +
C          write(*,*) " End ERD__2D_ATOM_SPNSPN_CPLING_PQ_INTEGRALS"
C          WRITE(*,*) SHELLP, SHELLQ

C       write(6,*) " The component",SHELLP, SHELLQ
C       do i=1,NINT2D
C          write(6,"(1x,3(F10.5))") INT2DX2(i),INT2DY2(i),INT2DZ2(i)
C       enddo 
             ...
             IF (SHELLQ.EQ.0) THEN
                ...
                IF (XX .EQ. 1 .OR. YY .EQ. 1 .OR. ZZ .EQ.1) THEN
C                 write(*,*) "Enter  ERD__ATOM_INT2D_TO_E000"
