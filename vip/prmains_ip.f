      ...
      SUBROUTINE PRMAINS_IP(IUHF, ISPIN, SCR, DMAXCOR)
C
...
C
      ...
      DO 1 A = 1, POP(SIRREP, ISPIN)
         IF (ABS(SCR(A)).GT.THRESH) THEN
            IF (FIRST) THEN
               WRITE(6,1300)
               WRITE(6,*)'   SINGLE IP COEFFIENTS '
...
C
      DO 100 MSPIN = 1, 1+IUHF
         ...
         DO 200 XIRREP = 1, NIRREP
            ...
            NUMDSS = VRT(MIRREP, MSPIN) * NS(SIRREP)
            IF (ISPIN.EQ.MSPIN) THEN
               DISSYS = IRPDPD(XIRREP, 20 + ISPIN)
               DISSYA = IRPDPD(XIRREP, 2  + ISPIN)
            ELSE
            ...
            DO 10 S= 1, NS(SIRREP)
               DO 20 M = 1,VRT(MIRREP, MSPIN)
                 ...
                 IF ((IUHF. NE.0) .AND. (MSPIN.EQ.ISPIN)) THEN
                     ...
                     ENDIF
                     DO 40 B = 1, POP(BIRREP, ISPIN)
                        DO 50 A = 1, POP(AIRREP, MSPIN)
 ...
 10            CONTINUE
               IF (ICOUNT .NE. I000 + NUMDSS*DISSYS) THEN
                  WRITE(6,*)' SOMETHING WRONG IN PRMAINS_IP',
