...
C CODED JG JUNE/90
C 
      SUBROUTINE QUAD2(ICORE,MAXCOR,IUHF,INCREM,FACT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DIRPRD,DISSYW,DISSYT,POP,VRT
      LOGICAL TAU,UCC
C     LOGICAL MBPT3,MBPT4,CC,TRPEND,SNGEND,GRAD,MBPTT,SING1,QCISD,UCC
      DIMENSION ICORE(MAXCOR)
      COMMON /SYM/ POP(8,2),VRT(8,2),NTAA,NTBB,NF1AA,NF1BB,NF2AA,NF2BB
C     COMMON /INFO/ NOCCO(2),NVRTO(2)
      COMMON /MACHSP/ IINTLN,IFLTLN,IINTFP,IALONE,IBITWD
C     COMMON /FILES/ LUOUT,MOINTS
C     COMMON /SWITCH/ MBPT3,MBPT4,CC,TRPEND,SNGEND,GRAD,MBPTT,SING1,
C    &                QCISD,UCC
      COMMON /SYMINF/ NSTART,NIRREP,IRREPA(255),IRREPB(255),
     &DIRPRD(8,8)
      COMMON /SYMPOP/ IRPDPD(8,22),ISYTYP(2,500),NTOT(18)
C     COMMON /CONTROL/ IPRNT,IXXX,IXXX2
...
C     make UCC false.
      UCC=.FALSE.
C     Allocate memory for T1 amplitudes but keep TAU false.
...
C
       LISTT=ISPIN+43
C      LISTW=ISPIN+13
C      Use t2 amplitudes here instead of integrals for y2 formation.
       LISTW=ISPIN+43
C
C
