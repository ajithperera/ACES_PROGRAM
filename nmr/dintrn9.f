      SUBROUTINE DINTRN9(EVEC,AODINT,BUF,RHF,NSTART,NEND,
     &                   IRREPX,ISPIN)
C
C THIS ROUTINE TRANSFORMS DERIVATIVE INTEGRALS FROM THE AO TO THE MO
C BASIS. THIS IS THE JKII VERSION.
C
CEND
...
C
       ...
       DO 1001 IRREP3=1,NIRREP
        IF(IRREP3.NE.IRREP1.AND.IRREP3.NE.IRREP2) THEN
         AODSZ=AODSZ+AOPOP(IRREP3)*AOPOP(IRREP3)
         MODSZ=MODSZ+AOPOP(IRREP3)*AOPOP(IRREP3)
        ENDIF
1001   CONTINUE
C
        ...
        DO 22 JMO=1,NMO2
         DO 23 IRREP3=1,NIRREP
          IF(IRREP3.NE.IRREP1.AND.IRREP3.NE.IRREP2) THEN
           ...
           IX=NAO3*NAO3+1
c YAU : old
c          CALL ICOPY(NAO3*NAO3*IINTFP,AODINT(IOFF5),1,BUF,1)
c YAU : new
           CALL DCOPY(NAO3*NAO3,AODINT(IOFF5),1,BUF,1)
c YAU : end
           CALL GHTRAN(BUF,BUF,EVEC,IOFFC3,IOFFC3,1,1,
     &                 BUF(IX),0,1,NAO3,NAO3,NMO3,NMO3)
c YAU : old
c          CALL ICOPY(NMO3*NMO3*IINTFP,BUF,1,AODINT(IOFF6),1)
c YAU : new
           CALL DCOPY(NMO3*NMO3,BUF,1,AODINT(IOFF6),1)
c YAU : end
           IOFF5=IOFF5+NAO3*NAO3
           IOFF6=IOFF6+NMO3*NMO3
          ENDIF
23       CONTINUE
22      CONTINUE
21     CONTINUE
...
C
      ...
      DO 1010 IRREP1=1,NIRREP
       IRREP2=DIRPRD(IRREP1,IRREPX)
       DO 1015 IRREP3=1,NIRREP
        IF(IRREP1.NE.IRREP3.AND.IRREP2.NE.IRREP1) THEN
         IOFFBUF1=MAX(IOFFBUF1,1+AOPOP(IRREP3)*AOPOP(IRREP3)*
     &              AOPOP(IRREP2))
        ENDIF
1015   CONTINUE
1010  CONTINUE
...
1020  CONTINUE
C
      CALL DUMP9(AODINT,BUF,BUF(IOFFBUF1),BUF(IOFFBUF2),
     &           NSTART,NEND,IRREPX)
C
      CALL TIMER(1)
      TIMEI=TIMEI+TIMENEW
