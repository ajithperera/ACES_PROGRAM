...
C***CATEGORY  D4A2
C***TYPE      DOUBLE PRECISION (RG-S, CG-C)
...
C          REAL*8 array, dimensioned A(NM,N).
C
...
C          one-dimensional REAL*8 arrays, dimensioned WR(N) and WI(N).
C
...
C          Z is a two-dimensional REAL*8 array, dimensioned Z(NM,N).
C
...
C          dimension N.  IV1 is of type INTEGER and FV1 of type REAL*8.
C
...
C
      ...
      DOUBLE PRECISION A(NM,*),WR(*),WI(*),Z(NM,*),FV1(*)
      INTEGER IV1(*)
C
