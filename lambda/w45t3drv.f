      SUBROUTINE W45T3DRV_LAMBDA(CORE,MAXCOR,IUHF,
...
C
c      CALL INITRP
      IF(IUHF.NE.0) CALL TSPABCI_LAMBDA(CORE,MAXCOR/IINTFP,LZVOFF+3,1)
C
C
C-----------------------------------------------------------------------
...
C
      IF(IPASS.EQ.2)THEN
      CALL TSPABCI2_LAMBDA(CORE(1),MAXCOR,LZVOFF+3,LZVOFF+4,1)
...
C
      CALL WVT323_LAMBDA(CORE(I000),CORE(I010),CORE(ISTART),IADT3,
     1            IADW2,
     ...
     1            ISPIN1,ISPIN2,LZVOFF,LWOFF,-1.0D+00)
      CALL WOT323_LAMBDA(CORE(I000),CORE(I010),CORE(ISTART),IADT3,IADW2,
...
C
      IF(IPASS.EQ.2)THEN
      CALL TSPABCI2_LAMBDA(CORE(1),MAXCOR,LZVOFF+3,LZVOFF+4,2)
...
C     is already handled by TRPS.
C     
      IF(IUHF.NE.0) CALL TSPABCI_LAMBDA(CORE,MAXCOR/IINTFP,LZVOFF+3,2)
C     -----------------------------------------------------------------
C
      RETURN
      END
