      ...
      SUBROUTINE tdcc_mutilde_0_d_uhf_debug(MIJBAR,MABBAR,MIABAR,
...
C     
      ...
      LISTT0=100*(ISIDE-1)+34
        Write(6,*)
        Write(6,"(a)") "---Entered tdcc_mutilde_0_d_uhf_debug---"
        Write(6,*)
C LOOP OVER IRREPS 
C
...
C   
     ...
     &                 MIABAR(1+IRPDPD(IRREPX,9)),1,ONE,ICORE(I000),
     &                 DISSYZ)
       ENDIF
C
       IF(IRREPZL .EQ. IRREPX .AND. ISIDE .EQ. 2) THEN
          CALL GETLST(ICORE(I020),1,1,1,2,190)
          CALL XGEMM ('N','N',DISSYZ,NUMDSZ,1,ONE,MIABAR(1),DISSYZ,
     &                 ICORE(I020),1,ONE,ICORE(I000),DISSYZ)
       ENDIF
...
C
     ...
     &            VRT(1,2),POP(1,2),ICORE(I020),IRREPX,'1324')
      Write(6,*)
      call checksum("Tdcc_mutilde_0_d_uhf_debug,mu^t(Ab)(f)   :",
     +               icore(I010),isize,S)
C
CSSS      CALL PUTALL(ICORE(I010), ISIZE, IRREPX,446)
...
C   
     ...
     &                              1, icore(i000), 1)
       ...
       ENDIF
C
CSSS       CALL SSCAL (ISIZE,-HALF,ICORE(I000),1)
C What is this???
       CALL DSST03I(ICORE(I000),ICORE(I010),ISIZE,ISIZE,ICORE(I020),
     &              SPCASE(ISPIN),IRREPX)
      
      Write(6,*)
      call checksum("Tdcc_mutilde_0_d_uhf_debug,mu^t(AA/bb)(f):",
     +               icore(I010),isize2,S)
C
C  PUT ON A PROPER LIST
C
CSSS       CALL PUTALL(ICORE(I010),ISIZE2,IRREPX,323+ISPIN)
C
