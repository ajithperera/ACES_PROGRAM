      ...
      SUBROUTINE tdcc_mu_dot_db_debug(ICORE,MAXCOR,IUHF,IRREPX)
C
...
C
      ...
      DATA ZILCH/0.0D0/
        ...
        Write(6,*)
        Write(6,"(a)") "---Entered tdcc_mu_dot_db_debug---"
        Write(6,*)

      CALL GETDT1(ICORE,MAXCOR,MXCOR,IUHF,IOFFT1,IRREPX,390)
      RHF=IUHF.EQ.0
C
C
...
C     
     ...
     &               ICORE(ITMP3))
CSS        CALL GETLST (ICORE(I010),1,NUMDSZ,1,IRREPZR,LISTZ)
...
100   CONTINUE
      ...
      i000=1
      Isize_abij_z=Idsymsz(Irrepx,Isytyp(1,336),
     +           Isytyp(2,336))
      call getall(Icore(i000),Isize_abij_z,irrepx,336)
      call checksum("Tdcc_mu_dot_db_debug Z(Ab,Ij)(f):",Icore(i000),
     +               Isize_abij_z,S)
C
      IF(RHF)RETURN
C
...
C  
	CALL DZERO(ICORE(I000),NUMDSZ0*DISSYZ)
     ...
     &               ICORE(IT),MAX(1,NSUM),ZILCH,ICORE(IZ),NROW)
          ENDIF 
230      CONTINUE
C
        ELSE
