...
C    STORED IN SCR(1..3) AND SCR(4..6), RESPECTIVELY.
      ...
      CALL DCOPY(3,Q1(1,IREFATM),1,SCR(1),1)
      CALL DCOPY(3,Q2(1,IREFATM),1,SCR(4),1)
      X1=DNRM2(3,SCR(1),1)
      X2=DNRM2(3,SCR(4),1)
      Z1=ONE/X1
      Z2=ONE/X2
      CALL DSCAL(3,Z1,SCR(1),1)
      CALL DSCAL(3,Z2,SCR(4),1)
...
C NOW FIND ANOTHER ATOM WHICH IS NOT ALONG THE NEW Z AXIS
      ...
      DO IATOM=1,NATOM
         ...
         END IF
C
C This block is added to avoid the divison by zero errors in the
C next step. This assumes that at least there is a one atom that has
C non zero length from the Z-axis. Ajith Perera, 05/2021, 

         ICHECK1 = I1+3*(IREFATM-1)
         ICHECK2 = I2+3*(IREFATM-1)
         X1=DNRM2(2,SCR(ICHECK1),1)
         X2=DNRM2(2,SCR(ICHECK2),1)
         IF (X1 .EQ. ZILCH .OR. X2 .EQ. ZILCH) THEN
            XMAX=ZILCH 
         ENDIF 
         IOFF=IOFF+3
      END DO
...
C FIND MATRIX WHICH ROTATES PROJECTION OF VECTOR ALONG AXIS
         ...
         FACT1=ONE/X1
         FACT2=ONE/X2
         CALL DSCAL(2,FACT1,SCR(IOFF1),1)
         CALL DSCAL(2,FACT2,SCR(IOFF2),1)
...
C CALCULATE COMPLETE W1 AND W2 TRANSFORMATION MATRICES
         ...
         CALL DCOPY(9,SCR(I1),1,W1,1)
         CALL DCOPY(9,SCR(I2),1,W2,1)
