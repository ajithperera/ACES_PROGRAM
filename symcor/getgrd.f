...
c     original reference geometry from the finite difference grid
      ...
      Write(6,"(a)") " The current geometry (principal axis)"  
      ioff = 1
      Do j=1,Nratom
      ioff = (j-1)*3 
      Write(6,"(3(1x,F15.7))") (dscr(ioff+i),i=1,3)
      enddo 
      Write(6,"(a)") " The current geometry (orginal axis) "
      Do j=1,Nratom
      Write(6,"(3(1x,F15.7))") (Refgeom(i,j),i=1,3)
      enddo 
...
c   o pick up VMOL->ZMAT mapping vector
      ...
      Write(6,"(2a)") " The analytical gradients (computational",
     &               " orientation)"
      Do j=1,Nratom
      Write(6,"(3(1x,F15.7))") (grdint(i,j),i=1,3)
      enddo 
...
c   o transform the gradient from int orient to ext orient
     ...
     &           0.d0,DSCR,  3)
      ...
      Write(6,"(2a)") " The analytical gradients (reference",
     &                " orientation)"
      Do j=1,Natom
      Write(6,"(3(1x,F15.7))") (grdxyz(i,j),i=1,3)
      enddo 
c   o mass-weight the gradient
      ...
      END DO
      write(6,*)
      Write(6,"(a)") " mass weighted analytical gradients"
      Koff=0
      Do j=1,Nratom
      Write(6,"(3(1x,F15.7))") (Dscr(i+koff),i=1,3)
      koff = Koff + 3
      enddo 
...
c   o transform the gradient to symmetry adapted coordinates
     ...
     &           )
      Write(6,"(a)") "The symmetry adaptation transformation"
      call output(Dscr(ltmp),1,nsize,1,nsize,nsize,nsize,1)
      CALL XGEMM('T','N',NSIZE,1,NSIZE,
     &           1.d0,DSCR(lTMP),NSIZE,
     &                DSCR,   NSIZE,
     &           0.d0,GRDINT, NSIZE)
      write(6,*)
      Write(6,"(a)") " mass weighted/transformed analytical gradients"
      Do j=1,Nratom
      Write(6,"(3(1x,F15.7))") (grdint(i,j),i=1,3)
      enddo
