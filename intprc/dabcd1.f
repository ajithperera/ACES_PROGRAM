...
C DRIVER FOR SORTING <AB||CD> INTEGRALS, UHF ONLY

      SUBROUTINE DABCD1(ICORE,MEMSIZ,NUMIRW,ISYM,IPW,IPDIS,NWDIS,
     &                  ISPIN,NMO,IRRVEC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DISSIZ,DIRPRD
      DIMENSION ICORE(MEMSIZ),NUMIRW(2*NIRREP),ISYM(1),
     &          IPW(8),IPDIS(8),IRRVEC(NMO,2)
...
C
C  SUBTRACT SPACE FOR A SCRATCH ARRAY IN CABCD WHICH HOLDS ONE
C  DISTRIBUTION
C
      MEMSIZ=MEMSIZ-DISSIZ*IINTFP
C
...
4000   FORMAT(T3,'@DABCD1-F, Not enough core to hold one integral ',
     &        'distribution.',/,
...
C
      ...
      I070=4*ILNBUF*IINTFP+I060
      I080=4*ILNBUF+I070
      I090=4*ILNBUF+I080
      I095=4*ILNBUF+I090
      I100=NBKINT*NBUCK+I095 
CSSS      WRITE(6,*)' I100 Is ',I100,'  MEMSIZ IS ',MEMSZ
      IF(I100.GT.MEMSZ)THEN
       WRITE(6,5000)
5000   FORMAT(T3,'@DABCD1-F, Insufficient core to perform sort.')
       CALL INSMEM('DABCD1',I100,MEMSZ)
       CALL ERREX
      ENDIF
      WRITE(6,1000)MEMSZ,MEMSIZ,DISSIZ*LENINT,NUMDIS,NINCOR,NBUCK,
     &        NDBCK,NBKINT,IRECL,I060
1000   FORMAT(T3,'@DABCD1-I, Information for AO integral sort',/,
     ...
     &   '        Total utilized memory (words)          ',I10,'.')
      CALL SABCD1(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
     &           ICORE(I040),ICORE(I050),NBUCK,NDBCK,
     &           NBKINT,NREC,IRECL,ILNBUF,ISYM,ISPIN,ICORE(I060),
...
C
      ...
      I015=I010+DISSIZ*NDBCK*IINTFP
      I020=I015+DISSIZ*IINTFP
      I030=I020+NBKINT*IINTFP
      I040=I030+NBKINT
      WRITE(6,200)I040,MEMSZ
200   FORMAT(T3,'@DABCD1-I, Sort chain processing uses ',I10,
     &          ' out of ',I10,' words.')
      IF(I040.GT.MEMSZ)THEN
       WRITE(6,300)I060,MEMSZ
       CALL ERREX
300    FORMAT(T3,'@DABCD1-F, Core screwup. ',I10,' words needed.',
     &           I10,' words available.')
      ENDIF
      CALL CABCD1(ICORE(I010),ICORE(I015),ICORE(I020),ICORE(I030),
     &            ICORE(I000),DISSIZ,NDBCK,NBKINT,NBUCK,IRECL,
     &            NUMIRW,ISYM,IPW,IPDIS,NWDIS,ISPIN,NMO,IRRVEC)
      RETURN
      END
