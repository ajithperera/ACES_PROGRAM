...
C  in the file COPYRIGHT.
         SUBROUTINE  ERD__2D_SPNSPN_CPLINGZY_PQ_INTEGRALS
     +
     +                    ( SHELLP,SHELLQ,
     +                      MIJ,MKL,NGQP,NGQEXQ,
     +                      PX,PY,PZ,QX,QY,QZ,
     ...
     +
C------------------------------------------------------------------------
C  OPERATION   : ERD__2D_SPNSPN_CPLINGZY_PQ_INTEGRALS
...
C
         ...
         DO IJ = 1,MIJ
            ...
            DO KL = 1,MKL
               ...
               DO ROOT = 1, NGQP
                  N = N + 1
CSSS                  QROOT = RTS(N) * TWORHO
                  QROOT = RTS(N) * (1.0D0 - RTS(N)) * TWORHO
...
CSSS                  QROOT = RTS(N) * TWORHO
                  ...
                  INT2DY1 (N,0,0) = INT2DY (N,0,0) * PQZ
                  INT2DY1 (N,1,0) = INT2DY (N,1,0) * PQZ +
     +                              INT2DY (N,0,0) * PIJ
                  INT2DY1 (N,0,1) = INT2DY (N,0,1) * PQZ -
     +                              INT2DY (N,0,0) * QKL
...
C
                 IF (SHELLQ.LE.SHELLP) THEN
                    ...
                    DO K = 1,SHELLQ
                       ...
                       INT2DY1 (N,1,K) = INT2DY (N,1,K)  * PQZ +
     +                                   INT2DY (N,0,K)  * PIJ -
     +                                   INT2DY (N,1,K1) * QKL * K
