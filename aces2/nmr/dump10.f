...
C
        ...
        IF(IRREP3.EQ.IRREP1.OR.IRREP2.EQ.IRREP1) GO TO 2000
C
...
C
         ...
         DO 2001 IND2=1,NBAS2
          CALL SCOPY(NBAS13,AODINT(IOFF2),1,BUF(IOFF1),1)
          IOFF1=IOFF1+NBAS13
          IOFF2=IOFF2+2*NBAS13
2001     CONTINUE
C
C REORDER THEM AS IRREP1,IRREP1,IRREP1,IRREP3 (slowest to fastest)
C
        DO 3010 IP=1,NBAS3
         CALL SCOPY(NBAS12,BUF(IP),NBAS3,BUF2,1)
         CALL TRANSP(BUF2,BUF2(1+NBAS12),NBAS2,NBAS1)
         CALL SCOPY(NBAS12,BUF2(1+NBAS12),1,BUF(IP),NBAS3) 
3010    CONTINUE
C
C THE ORDERING IS NOW :  (FASTEST TO SLOWEST) IRREP3,IRREP2; IRREP1,
C                                       FIXED OCC(IRREP1)
C
        LENGTH=NOCC3*NOCC2
...
C
        ...
        IF(LENGTH.NE.0) THEN
         DO 35 IQ=1,NVRT1
          ...
          IDISIJKA=IOFFOV(IRREP1,IRREP1)+INDXF(IND,IQ,NOCC1)
...
2101     CONTINUE
C
C REORDER THEM AS IRREP1,IRREP3,IRREP2,IRREP1 (slowest to fastest)
C
        DO 3110 IP=1,NBAS1
         CALL SCOPY(NBAS23,BUF(IP),NBAS1,BUF2,1)
         CALL TRANSP(BUF2,BUF2(1+NBAS23),NBAS2,NBAS3)
         CALL SCOPY(NBAS23,BUF2(1+NBAS23),1,BUF(IP),NBAS1) 
3110    CONTINUE
C
C THE ORDERING IS NOW :  (FASTEST TO SLOWEST) IRREP1,IRREP2; IRREP3,
C                                             FIXED OCC(IRREP1)
C
        LENGTH=NOCC1*NOCC2
