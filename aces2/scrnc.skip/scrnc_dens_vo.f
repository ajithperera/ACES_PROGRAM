      ...
      SUBROUTINE Scrnc_DENS_VO(DVO,ICORE,MAXCOR,IUHF)
C
...
CSSS        Write(6,*) 
CSSS        write(6,*) "DAI individual contributions"
CSSS        call checksum("DAIAB-AE", DVO(I0D(ISPIN)), NT(ISPIN),s)
C
C SECOND TERM
C
C   - SUM M T(A,M) G(I,M) 
...
CSSS        call checksum("DAIAB-MI", DVO(I0D(ISPIN)), NT(ISPIN),s)
C
C THIRD TERM
C
...
C
         ENDIF
CSSS         call checksum("L1      ", ICORE(I0L(ISPIN)), NT(ISPIN),s)
CSSS         call checksum("T2aaaa  ", ICORE, NT(ISPIN)*NT(ISPIN),s)
CSSS         call checksum("DAIABTL1", DVO(I0D(ISPIN)), NT(ISPIN),s)
C
C ABAB (BABA)  SPIN CASE
C
         IF(MIN(NT(1),NT(2)).NE.0) THEN
...
CSSS         call checksum("DAIABTL2", DVO(I0D(ISPIN)), NT(ISPIN),S)
C
C THE FOURTH TERM
C
...
C
        IF(TRIP1) THEN
C
         CALL GETLST(ICORE,1,1,1,4+ISPIN,160)
