      ...
      SUBROUTINE CALC_RPA_DENS(C,DOO,DVV,IRREPC,ISPIN,IUHF)
C
...
C
     ...
     &                 C(IOFFCR),NUMA,ZILCH,DOO(IOFFD),NUMI)
       ENDIF 
10    CONTINUE
      CALL PUTLST(DOO,IRREPC,1,1,7+ISPIN,91)
...
C
     ...
     &                C(IOFFCL),NUMB,ZILCH,DVV(IOFFD),NUMA)
       ENDIF 
20    CONTINUE
      CALL PUTLST(DVV,1,IRREPC,1,7+ISPIN,92)
C
CSSS      CALL PUTLST(C,1,1,1,ISPIN,490)
CSSS      CALL PUTLST(C,1,1,1,2+ISPIN,490)
C
      RETURN
      END
