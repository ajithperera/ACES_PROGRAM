...
C
      DIMENSION WORK(MAXCOR),EIGVAL(NSIZ)
...
CSSS     &                 EIGVAL,WORK(I030P),SCR_SIZE,INFO)
            ...
            IENDD = IDUMY + ROWDIM
            IF (IENDD .GE. MAXCOR) CALL INSMEM("@-do_rhf_rpa",IENDD,
     &                                          MAXCOR)
...
C
     ...
     &                     0.0D0, WORK(ISVEC), ROWDIM)
              ...
              ENDDO
C
              CALL PROCESS_RHF_RPA_ROOTS(WORK(IZVEC),WORK(IYVEC), 
     &                               EIGVAL,ROWDIM,IRREP,1,
     &                               WORK(IENDD),MEMLEFT)
     ...
     &                                  1.0D0/4.0D0*ECOR
C This test has been moved up (see above).
            INFO = 0
            IF (INFO.NE.0) THEN
               WRITE(*,*)
