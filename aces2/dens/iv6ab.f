      SUBROUTINE IV6AB(G,W,AIVV,FACT,ISPIN,POP1,POP2,VRT1,VRT2,
     &                  DISSYT,NUMSYT,DISSYW,NUMSYW,LISTG,LISTW,IRREP,
     &                  TMP,IUHF)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER DISSYT,DISSYW,DIRPRD,POP1,POP2,VRT1,VRT2
      DIMENSION G(DISSYT,1),W(DISSYW,1),AIVV(1),TMP(1)
...
C
       if (ndrgeo.eq.0) then
        ...
        if (iuhf.eq.1 .and. ispin.eq.1) then 
         CALL GETGO6R(G,W,1,NUMSYT,1,IRREP,LISTG,ispin,listw,dissyt)
        else  
         CALL GETGO6(G,W,1,NUMSYT,1,IRREP,LISTG,ispin,listw,dissyt)
        endif 
       endif 
C
...
c       CALL SYMTRA2(IRREP,POP1,POP2,DISSYW,NUMSYW,W,G)
