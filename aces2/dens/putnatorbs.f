      ...
      SUBROUTINE PUTNATORBS(A,LDIM,IUHF,NIRREP,NBFIRR,NOCC)
...
C
      IF(IFLAGS(1).GE.10)THEN
       WRITE(6,1000)
 1000  FORMAT(' @PUTNATORBS-I, Writing natural orbitals to NATORBS.')
      ENDIF
C
      NCOLS = 4
C
 ...
 1010  FORMAT(' @PUTMOS-I, Symmetry ',I3,' Full ',I3, ' Partial ',I3)
   ...
   10 CONTINUE
C
C     If NATORBS already exists, delete it and open a new one.
C
      INQUIRE(FILE='NATORBS',EXIST=YESNO)
      IF(YESNO)THEN
       OPEN(71,FILE='NATORBS',STATUS='OLD',ACCESS='SEQUENTIAL',
     1      FORM='FORMATTED')
       CLOSE(UNIT=71,STATUS='DELETE')
       IF(IFLAGS(1).GE.10)THEN
        WRITE(6,1030)
       ENDIF
 1030  FORMAT("@-PUTNATORBS NATORBS already exists and will be ",
     &         "deleted.")
      ENDIF
      OPEN(71,FILE='NATORBS',STATUS='NEW',ACCESS='SEQUENTIAL',
     1     FORM='FORMATTED')
