...
250     CONTINUE
      Call checksum("T(a,m)*L(b,m)->DOO    :",DOO(1+(ISPIN-1)*
     &               NF1(ISPIN)), NF1(ISPIN))
...
C
      ...
      ENDIF 
C
C ADD TRIPLE CONTRIBUTION TO DOO
C
      IF(TRIP1) THEN
cold (only canonical perturbed orbitals allowed).
ccc       IF(CANON) THEN
ccc        CALL ADDDT(DOO(IOFF),ICORE,NF1(ISPIN),NIRREP,POP(1,ISPIN),
ccc     &             160,ISPIN)
ccc       ELSE
...
ccc        CALL ERREX
ccc       ENDIF
cnew standard perturbed orbitals allowed as well. JDW 7/5/97.
        CALL ADDDT(DOO(IOFF),ICORE,NF1(ISPIN),NIRREP,POP(1,ISPIN),
     &             160,ISPIN,CANON)
        Call checksum("CCSD(T)->DOO          :",DOO(1+(ISPIN-1)*
     &               NF1(ISPIN)),NF1(ISPIN))
      ENDIF
C
C  FOR ALL METHODS EXCEPT MBPT2 SYMMETRIZE DENSITY MATRIX HERE
C     
      IF(.NOT.MBPT2)THEN
