       SUBROUTINE INTV4(AIVV,ICORE,MAXCOR,IUHF)     
C
C
C THIS ROUTINE CALCULATES THE FOURTH TERM OF THE VIRTUAL VIRTUAL BLOCK
C OF THE INTERMEDIATE I
C
C THE GENERAL FORMULA IS
C
C 1. TERM :  - 1/2 SUM E,F SUM M <EF//AM> G(EF,BM)
C
...
C IN CORE VERSION
C
          CALL IV4AB(ICORE(I001),ICORE(I002),AIVV(IOFF),
...
C
          ...
          IF(NDIS.GE.0) THEN
C
           CALL IV4ABO(ICORE(I003),ICORE(I001),AIVV(IOFF),
     &                 FACT,ISPIN,POP(1,ISPIN),POP(1,3-ISPIN),
     &                 VRT(1,ISPIN),VRT(1,3-ISPIN),DISSYT,NUMSYT,
...
C           
          ELSE
C
C NO OUT-OF-CORE VERISON IS CURRENTLY POSSIBLE
C
           CALL INSMEM('IV4AB',NDIS,MXCOR)
          ENDIF
         ENDIF
        ELSE
C
