...
C Note that for <mu|delta(r-Rn)|nu>, SHELLP is SHELL2 (ie. no 
C recurrencr relations are use).

         MXSHELL = MAX (SHELL1,SHELL2)
         SHELLP =  MXSHELL

cPrakash we don;t need that
CSSS         CASE1 = MOD (SHELLP,2) .EQ. 1
CSSS         CASE2 = SPHERIC. AND. (SHELL1.NE.SHELL2)
C
c         IF (ATOMIC .AND. (CASE1.OR.CASE2)) THEN
c             EMPTY = .TRUE.
c             RETURN
c         END IF
cEnd Prakash above comment
...
C
CSSS         NXYZP  = (SHELLP+1)*(SHELLP+2)/2
CSSS         NXYZET =   ((SHELLP+1)*(SHELLP+2)*(SHELLP+3)/6)
CSSS     +            - ((SHELLA  )*(SHELLA+1)*(SHELLA+2)/6)
C 
c
         NXYZET=NXYZA*NXYZB
