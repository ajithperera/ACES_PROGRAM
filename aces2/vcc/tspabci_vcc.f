C     Local copy of vcc5t/tspabci.f, renamed to avoid collision with the
C     differently-shaped TSPABCI in trp/librt3/lambda (4 and 5-arg
C     versions). vcc is not linked against vcc5t in the shared
C     Makefiles/GNUmakefile LIBS chain, so without this local copy the
C     linker resolves TSPABCI from trp's library instead -- a 4-arg
C     interface mismatch (t3_iter.f only ever passes 3 actual arguments)
C     that reads garbage into EXTRNL_CCSD and crashes deep inside
C     GETLST/PUTLST. Same bug as lcct/tspabci_lcct.f, found via ACES II
C     test suite cases_pending/124d.fno, 124e.fno.
      SUBROUTINE TSPABCI(CORE,MAXCOR,ICODE)
...
C
      ENDIF
C
      IF(IDRLVL.GT.0.AND.ICODE.EQ.2)THEN
C
