      ...
      SUBROUTINE FINISH(ICYCLE,pCCD,pCCDS,pCCDTS)
C
...
cAP - 500 is the maximum number of CC iterations (not basis functions)
     ...
     & '             ','             ','  CC2        ','             ',
     & '             ',' CCD         ','  CCSD       ','   pCCD      ',
     & '  OO-LCCD    ',' OO-CCD      ','  pCCD-(S)   ','   pCCD-(TS) '/

C Entries at 50 and 51 correspond to external CC runs (the first runs 
C are exactly identical to standard CCD and CCSD calculation)
C
cjp
...
cjp info about homotopicsolution
       if(isbwcc .and. i-1.gt.ihomotop.and.ihomotop.gt.0) then
          ...
          if (pCCD) Then 
              if(o)WRITE(6,1000)I-1,ECORR(I,1),
     &                              ECORR(I,1)+ESCF,STAR
          else 
...
cjp
     ...
     &ECORR(ICYCLE,1)+ESCF
       ...
       IF (pCCD) THEN
       if(o)WRITE(6,1010) ITCALC(52),ECORR(ICYCLE,1)+ESCF
       ELSE IF (pCCDS) THEN
       if(o)WRITE(6,1010) ITCALC(55),ECORR(ICYCLE,1)+ESCF
       ELSE IF (pCCDTS) THEN
       if(o)WRITE(6,1010) ITCALC(56),ECORR(ICYCLE,1)+ESCF
       ELSE 
