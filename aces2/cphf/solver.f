      SUBROUTINE SOLVER(IRREP,NPERT,BAI,ICORE,MAXCOR,IUHF)
C
C  THIS ROUTINE CONTROLS THE FORMATION OF THE A MATRIX
C  AND CALLS THE ACTUAL SOLVER FOR THE CPHF EQUATION
C
CEND
C
C  CODED JULY ADN AUGUST/90  JG
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      LOGICAL INCORE,ROHF,SEMI,FIELD,GEOM,THIRD,MAGN,SPIN
      LOGICAL JSO,JFC,JSD
...
C
      DATA TOL/0.0D+0/
C
C  SET PARAMETERS FOR SOLVING THE CPHF-EQUATION
C
      KMAX=IFLAGS(31)
...
C
C CHECK WHICH KIND OF PERTURBATION
C
        IF(MAGN.OR.JSO) THEN
C
...
C
        ELSE IF(JFC.OR.JSD) THEN
C
C A-MATRIX FOR SPIN SYMMETRY BREAKING PERTURBATIONS
C
         CALL MKARHF4(IRREP,ICORE(I010),ICORE(I020))
C
        ELSE
C
...
C
     ...
     &              ICORE(I031),ICORE(I051),NF2(1),NF1(1),NT(2))
      ELSE
       INCORE=.FALSE.
       CALL INSMEM('SOLVER',MAX(IEND1,IEND2),MAXCOR)
      ENDIF
C
...
C
      IF(MAX(IEND1,IEND2).LT.MXCOR) THEN
       INCORE=.TRUE.
       IF(MAGN) THEN
        CALL MKAUHF3(IRREP,ICORE(I010),ICORE(I020))
       ELSE
