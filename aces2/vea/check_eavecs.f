      ...
      SUBROUTINE CHECK_EAVECS(IUHF, ISPIN, ICORE, MAXCOR, LS1OUT,
     $                        LS2OUT)
C
...
C
      ...
      ENDIF
C
      NS1 = NS(SIRREP)*VRT(SIRREP,ISPIN) 
      IOFF = IONE
      JOFF = IONE
      CALL GETLST(ICORE(IOFF),1,1,1,ISPIN,LS1OUT)
      call checksum("chk_eas1:",icore,NS(SIRREP)*POP(SIRREP,ISPIN))
C
      ...
      ENDIF
         ...
         DO 100 IRREPR = 1 , NIRREP
            ...
            NUMDSS = POP(IRREPR, MSPIN) * NS(SIRREP)
 ...
 100     CONTINUE
      ...
      call checksum("chk_eas2:",icore(joff),len)
      Joff = Joff+Len 

 50   CONTINUE
      call checksum("chck_eaf:",icore,Ioff-1)
      write(6,*)
      RETURN
      END
