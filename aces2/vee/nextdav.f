     ...
     &                    IUHF,ITER,MAXITER,IOFF,L_NORM,R_NORM)
C
...
CEND
      ...
      LOGICAL LCCD,LCCSD,CC2,GRAD_CALC,DOUBLE_EES
      LOGICAL ESTATE_GEOM_OPT,CORE_PROJECT,CORE_SEARCH,G_TDENS
...
C
      ...
      DIMENSION IOFF(8)
      DIMENSION BUF3(*),sjunk(100),ijunk(100),R_NORM(200)
      DOUBLE PRECISION L_NORM(200)
C
     ...
     &                      " block!!"
      ...
      IF (CONVRG) THEN
       IF (DIFF .GT. DEGTOL) THEN
         WRITE(6,"(a,a,a,ES8.2E2)")" Convergence criteria is below",
...
C When generalized transition densities are requested we need to store both
C left and right vector on the disk for latter processing. The storge 
C pattern is as follows. List-472; do states; put R on 472(3+state) and
C L on 472(4+state).....Then the last converged be on 472(2) and 472(3)

       IF(ESPROP)THEN
         CALL TIMER(1)      
         TDAVID = TDAVID + TIMENEW
         ISTART = IOFF(IRREPX) + IROOT - 1
        IF(ISIDE.EQ.1)THEN
...
CSSS
         ...
         IF (G_TDENS) THEN
             IRPUT = 17+(2*ISTART-1)+ISIDE-1
             ILPUT = 17+(2*ISTART-1)+ISIDE
             CALL PUTLST(SCR(I000),ILPUT,1,1,3,472)
         ENDIF 

C         If (g_tdens) then
C         write(6,*) "From Tdens_debug"
C         call Gden_debug(SCR,MAXCOR,Iuhf)
C         endif
         CALL UPDATES(IRREPX,SCR(I000),444,0,490,IUHF)
         CALL LNORM(IRREPX,ROOT,SCR,MAXCOR,IUHF,Z,Z0)

         Write(6, "(a,2F12.8)") "The <L|R>;R0: ", Z,Z0
         IF (G_TDENS) L_NORM(ILPUT-19) = Z

         IF(LAMTHERE)THEN
          CALL TDENS(IRREPX,SCR,MAXCOR,IUHF,ISIDE,
     &               Z*(ONE+DFLOAT(1-IUHF)),Z0,ROOT,CORE_SEARCH,
     &               DOUBLE_EES)
          CALL PRINTSUM(ROOT,FSTR,CORE_SEARCH,DOUBLE_EES)
         ...
         Write(6,*)
         Write(6,"(a,1x,i2)")"Writing the left vector of state  :",ilput
         Call checksum("list-472      :",scr(i000),nsizec,s)
         Write(6,*)
         ITROOT=ITROOT+1

         IF (IFLAGS(91) .LT. 2) THEN
           IF (G_TDENS) THEN
              CALL EDENS_4GTDENS(IRREPX,SCR,MAXCOR,IUHF,ISIDE, 
     &                           Z*(ONE+DFLOAT(1-IUHF)),Z0,IRPUT,
     &                           NSIZEC)
           ELSE
