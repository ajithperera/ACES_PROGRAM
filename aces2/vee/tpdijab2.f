...
C
        ENDIF
CSSS        call checksum('tpdijab2',icore(i0z(ispin)),DISSYT2,s)
C
...
C
700      CONTINUE
CSSS        call checksum('tpdijab1',icore(i0r2l2),irpdpd(1,20+ispin),
CSSS     &                 s)
C
        ENDIF
C
...
C
800     CONTINUE
CSSSS        call checksum('tpdijab2',icore(i0r2l2),irpdpd(irrepx,20+ispin),
CSSSS     &                 s)
C
C MULTIPLY WITH T1
C
...
C
     ...
     &             IRREP,IRREPX,ISPIN)
         ENDIF
C
CSSS         call checksum('tpdijab2',icore(i000),numsyg*dissyg,s)
C
