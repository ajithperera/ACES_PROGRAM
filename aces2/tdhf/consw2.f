      SUBROUTINE CONSW2(FI,FJ,EI,EJ,UPI,UPJ,UMI,UMJ,DENP,DENM,FP,FM,
     X UP2,UM2,F2,E2,EVAL,EVEC,EHF,OMEGA,MCOMP,HMO,XX,IX,NINTMX,IA,
     X IVRT,IOCC,NSIZ1,NSIZ3,NSIZO)
      ...
      DIMENSION UPI(NSIZ1,NSIZO,3),UMI(NSIZ1,NSIZO,3),UPJ(NSIZ1,NSIZO,3)
     X ,UMJ(NSIZ1,NSIZO,3),UP2(NSIZ1,NSIZO,MCOMP),UM2(NSIZ1,NSIZO,MCOMP)
      DIMENSION FI(NSIZ1,NSIZ1,3),F2(NSIZ1,NSIZ1,MCOMP)
     X ,FJ(NSIZ1,NSIZ1,3),E2(NSIZO,NSIZO,MCOMP),EI(NSIZO,NSIZO,3)
     X ,EJ(NSIZO,NSIZO,3),EVAL(1),EVEC(1),DENP(1),DENM(1),FP(1),FM(1)
...
C ..... This routine can handle D(+w,+w) and D(0,+w)
      ...
      UM2(I,J,IRL)=ZERO
      DO 11 K=1,NUMSCF
      UP2(I,J,IRL)=UP2(I,J,IRL)+FI(I,K,IC)*UPJ(K,J,JC)
     X +FJ(I,K,JC)*UPI(K,J,IC)
      UM2(I,J,IRL)=UM2(I,J,IRL)+FI(K,I,IC)*UMJ(K,J,JC)
     X +FJ(K,I,JC)*UMI(K,J,IC)
   11 CONTINUE
      DO 13 K=1,NOC
      UP2(I,J,IRL)=UP2(I,J,IRL)
     X -UPI(I,K,IC)*EJ(K,J,JC)-UPJ(I,K,JC)*EI(K,J,IC)
      UM2(I,J,IRL)=UM2(I,J,IRL)
     X -UMI(I,K,IC)*EJ(J,K,JC)-UMJ(I,K,JC)*EI(J,K,IC)
   13 CONTINUE
      ELSE
      UP2(I,J,IRL)=ZERO
      DO 12 K=1,NUMSCF
      UP2(I,J,IRL)=UP2(I,J,IRL)+UMI(K,I,IC)*UPJ(K,J,JC)
     X                       +UMJ(K,I,JC)*UPI(K,J,IC)
   12 CONTINUE
...
C     DENPO(IJ)=ZERO
      ...
      DTMLOW= ZERO
      DO 16 K=1,NOC
      DTPLOW=DTPLOW+ UPI(I,K,IC)*UMJ(J,K,JC)+UPJ(I,K,JC)*UMI(J,K,IC)
      DTMLOW=DTMLOW+ UMI(I,K,IC)*UPJ(J,K,JC)+UMJ(I,K,JC)*UPI(J,K,IC)
   16 CONTINUE
      DTPLOW=DTPLOW*TWO
      DTMLOW=DTMLOW*TWO
...
C     CALL RTPUT(F2(1,1,IRL),FP,NUMSCF)
C     WRITE(6,*) ' FP by FMOK '
C     CALL OUTMXD(F2(1,1,IRL),NSIZ1,NUMSCF,NUMSCF)
      CALL FMOL(DENM,FM,XX,IX,NINTMX,IA)
C     CALL RTPUT(F2(1,1,IRL),FM,NUMSCF)
C     WRITE(6,*) ' FM by FMOL '
C     CALL OUTMXD(F2(1,1,IRL),NSIZ1,NUMSCF,NUMSCF)
      ELSE
...
C     write(6,*) I,J,DENP(IJL),DENM(IJL),' FP ',FP(IJL),' FM ',FM(IJL)
   20 CONTINUE
      ...
      E2(I,J,IRL)=ZERO
      DO 25 K=1,NUMSCF
      E2(I,J,IRL)=E2(I,J,IRL)+FI(I,K,IC)*UPJ(K,J,JC)
     X +FJ(I,K,JC)*UPI(K,J,IC)
   25 CONTINUE
      DO 26 K=1,NOC
   ...
   30 CONTINUE
      ...
      ELSE
      E2(I,J,IRL)= E2(I,J,IRL) -UP2(I,J,IRL)*(EVAL(J)-EVAL(I)-OMEGA)
      END IF
   40 CONTINUE
      IF(IOPFE.GE.2) THEN
      WRITE(6,*) ' E2 '
