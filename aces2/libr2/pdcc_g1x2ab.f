      ...
      SUBROUTINE PDCC_G1X2AB(T2,Z2,T,Z,T1A,T1B,TAU,FEAAA,FEABB,POP1,
     &                       POP2,VRT1,VRT2,DISSYT,DISSYZ,NUMSYT,NUMSYZ,
     &                       NFSIZA,NFSIZB,LISTT,LISTZ,IRREP,IUHF,TMP,
     &                       FACT)
...
C
C  IF TAU IS TRUE, FORM TAU AMPLITUDES ( ONLY REQUIRED FOR CCSD)
C
       IF(TAU) THEN
...
C
         CALL DSCAL(DISSYZ*NUMSYZ,FACT,Z2,1)
C
...
C
        CALL DSCAL(DISSYZ*NUMSYZ,FACT,Z,1)
        Write(6,*) 
        write(6,"(a,F5.2)") "Gae*tau-AB scaling factor :", Fact
        call checksum("Gae*Tau ", Z, DISSYZ*NUMSYZ)
C
