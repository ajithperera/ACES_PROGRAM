      ...
      SUBROUTINE DENSVO(DVO,ICORE,MAXCOR,IUHF,bRedundant)
C
...
C
         ...
         ELSE
            CALL GETLST_NR(ICORE,ICORE(I010),MXCOR-I010+1,
     &                     LISTT1,1)
         END IF
C
...
c YAU : end
C 
CSSS        Call checksum("T(a,i)->DOV                :",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))

         If(ispin.eq.1) Write(6,"(a)") "T(A,I)->DVO:"
         If(ispin.eq.2) Write(6,"(a)") "T(a,i)->DVO:"
         Call dstat("Term-1 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &               nvrto(ispin),.false.,"VO",ispin)
         call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C AND ADD L(A,I) TO DVO. 
C
         CALL SAXPY(NT(ISPIN),ONE,ICORE(I0L(ISPIN)),1,DVO(I0D(ISPIN)),1)
C
CSSS        Call checksum("L(a,i)->DOV                :",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))
         If (ispin.eq.1) Write(6,"(a)") "L(I,A)->DVO:"
         If (ispin.eq.2) Write(6,"(a)") "L(i,a)->DVO:"
         Call dstat("Term-2 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &               nvrto(ispin),.false.,"VO",ispin)
         call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C FIRST TERM :
C
...
CSSS        call checksum("G(a,e)*T(I,E)->DOV         :",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))
         if(ispin.eq.1) Write(6,"(2a)") "L2(MN,EF)*T2(AF,MN)*T(E,I)",
     &                                 "->DVO"
         if(ispin.eq.2) Write(6,"(2a)") "L2(mn,ef)*T2(af,mn)*T(e,i)",
     &                                 "->DVO"
         Call dstat("Term-5 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &               nvrto(ispin),.false.,"VO",ispin)
         call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C
C SECOND TERM
...
C
200     CONTINUE
CSSS        call checksum("G(M,I)*T(A,M)->DOV         :",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))
        If(Ispin.eq.1)Write(6,"(2a)") "T2(EF,IN)*L2(MN,Ef)*T(a,m)",
     &                                "->DVO:"
        If(Ispin.eq.2)Write(6,"(2a)") "T2(ef,in)*L2(mn,ef)*T(a,m)",
     &                                "->DVO:"
        Call dstat("Term-6 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &              nvrto(ispin),.false.,"DVO",ispin)
        call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C
C THIRD TERM
C
...
CSSS        call checksum("T2(ae,im)*L(m,e)->DOV      :",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))
        If (ispin.eq.1) Write(6,"(a)") "T2(AE,IM)*L(M,E)->DVO:"
        If (ispin.eq.2) Write(6,"(a)") "T2(ae,im)*L(m,e)->DVO:"
        Call dstat("Term-i3 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &              nvrto(ispin),.false.,"VO",ispin)
        call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C
C THE FOURTH TERM
...
C  
400     CONTINUE
CSSS        call checksum("T1(e,i)*T1(a.m)*L(m,e)->DOV:",DVO(I0D(ISPIN)),
CSSS     &                 NT(ISPIN))
        If (ispin.eq.1)Write(6,"(a)") "T1(E,I)*T1(A,M)*L(M,E)->DVO:"
        If (ispin.eq.2)Write(6,"(a)") "T1(e,i)*T1(a,m)*L(m,e)->DVO:"
        Call dstat("Term-4 ",dvo(1+(Ispin-1)*nt(ispin)),nocco(ispin),
     &              nvrto(ispin),.false.,"VO",ispin)
        call dzero(dvo(1+(Ispin-1)*nt(ispin)),nt(ispin))
C
