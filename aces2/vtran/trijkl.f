      SUBROUTINE TRIJKL(CMO,ICORE,MAXCOR,IUHF)
C
C DRIVER FOR THE TRANSFORMATION OF TWO-ELECTRON INTEGRALS
C OF TYPE IJKL (PARTIAL TRANSFORMATION ONLY)
C
CEND
...
C
           ...
           ENDIF
15        CONTINUE
          MSIZE=MAX(MSIZE,ITMP*NBAS(IRREP3),
     &              2*NBAS(IRREP1)*NBAS(IRREP2))
         ENDIF
14      CONTINUE
13     CONTINUE
C
       DO 16 IRREP1=1,NIRREP
        DO 17 IRREP2=1,NIRREP
         IF(IRREP2.EQ.IRREP1) GO TO 17
         IRREP12=DIRPRD(IRREP1,IRREP2)
         DO 18 IRREP3=1,NIRREP
          IF(IRREP3.EQ.IRREP1.OR.IRREP3.EQ.IRREP2) GO TO 18
           IRREP4=DIRPRD(IRREP3,IRREP12)
...
C
        JSTART=1
C
C SET JEND TO TOTAL NUMBER OF OCCUPIED ORBITALS
C
        JEND=0
...
C
       IF(ISIZE3TT(JRREP).NE.0) THEN
        ...
        IF(NORB.EQ.0.AND.FIRST) THEN
         write(*,*) ' @TRIJKL-F, Sorry, there is not enough memory !'
         CALL ERREX
        ENDIF
       ELSE
        NORB=NMO(JRREP)+1
...
C SLOWEST INDEX (OCCUPIED ORBITALS ONLY)
C
       DO 23 IRREP4=1,NIRREP
C
C SECOND SLOWEST INDEX
C
        DO 24 IRREP3=1,NIRREP
         IF(IRREP3.NE.IRREP4) THEN
          IRREP12=DIRPRD(IRREP3,IRREP4)
C
C THIRD SLOWEST INDEX (SHOULD ALWAYS BE GREATER THAN IRREP2)
...
950    CONTINUE
C
       CALL LOAD4(CMO,ICORE(I000),ICORE(I010),ICORE(I020),
     &            ICORE(I030),ICORE(ISYMAO),NBAS,
     &            POP(1,ISPIN),NSTART,NEND,IOFFAO,
     &            IOFF4,ISIZE3T,IOFF2,NSIZE1,ILNBUF,
     &            ISPIN,LUINT,LAST,IOFFSET)
C
       CALL INTRN4(CMO,ICORE(I000),ICORE(I010),RHF,NBASIS,
     &             NBAS,NMO,POP(1,ISPIN),POP(1,3-ISPIN),
     &             NSTART,NEND,ISIZE3,IOFFMO,NSIZE1,
     &             ISPIN,ICORE(IREORD))
