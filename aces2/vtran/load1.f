...
C         write(6,"(a,4(1x,i3),F15.8)") " @-load1, i,j,k,l:",
C     +                                  i,j,k,l,x
...
C Keep the "IIII" file around becasuse in some occasions we need the 
C one electron Hamiltonian integrals (for example, orbital kinetic energies).
C Ajith Perera, 04/2022. 
CSSS            close(unit=iUnit,status='DELETE')
         end if
      else
         rewind(iUnit)
