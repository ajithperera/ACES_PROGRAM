      SUBROUTINE OSL2AIAI3(ICORE,MAXCOR,IUHF,W)
C
C  THIS ROUTINE COMPUTES:
C
C     L2INC(Am,Mi) = W [ L2(Na,In) + SUM(b) T1(b,n) * L2(Ba,In)
C                                  - SUM(J) T1(N,J) * L2(Na,Ij)
C                                  - SUM(Jb) TAU(Nb,Jn) * L2(Ba,Ij) ]
C
C     L2INC(Na,In) = W [ L2(Am,Mi) + SUM(B) T1(B,M) * L2(Ab,Mi)
C                                  - SUM(j) T1(m,j) * L2(Am,Ji)
C                                  - SUM(jB) TAU(Bm,Mj) * L2(Ab,Ji) ]
CEND
C
CODED BY PS SEPT/93
C
...
C
            LISTL=146
            IRREPL=DIRPRD(IRREPO(3-ISPIN),IRREPI)
            IF(ISPIN.EQ.1) THEN
               DISSYL=FSDPDAI(IRREPL,ISYTYP(1,LISTL))
               NUMSYL=FSDPDIA(IRREPL,ISYTYP(2,LISTL))
               STRING='AIIA'
            ELSE
               DISSYL=FSDPDIA(IRREPL,ISYTYP(1,LISTL))
               NUMSYL=FSDPDAI(IRREPL,ISYTYP(2,LISTL))
               STRING='IAAI'
            ENDIF
C
            IF(DISTAR.NE.DISSYL.OR.NUMTAR.NE.NUMSYL) THEN
                WRITE(6,*)'@OSL2AIAI3-E FATAL ERROR: DISTAR.NE.DISSYL',
     $               DISTAR,DISSYL,' OR NUMTAR.NE.NUMSYL',NUMTAR,NUMSYL
                CALL ERREX
            ENDIF
C
            CALL FSGET(ICORE(I000),1,NUMSYL,1,IRREPL,LISTL,STRING)
C
C     SECOND TERM
C
            NSIZT=NTIA(3-ISPIN)
...
C
            IF(NSIZT.NE.DISSYL) THEN
               WRITE(6,*)'@OSL2AIAI3-E FATAL ERROR: NSIZT.NE.DISSYL',
     $              NSIZT,DISSYL
               CALL ERREX
            ENDIF
C
...
C
            ...
            DO 101 IRREP2=1,IRREPI-1
               ...
               IOFF=IOFF+VRTI(IRREPC,3-ISPIN)*POPI(IRREP2,ISPIN)*
     $             DISSYL*IINTFP
101         CONTINUE
            IRREPC=DIRPRD(IRREPI,IRREPL)
            NSIZL=VRTI(IRREPC,3-ISPIN)*POPI(IRREPI,ISPIN)
C
            IF(DISTAR*NUMTAR.NE.NSIZL) THEN
               WRITE(6,*)'@OSL21AIAI3-E FATAL ERROR: DISTAR*NUMTAR.',
...
C
            ...
            IF(I050.GT.MAXCOR) CALL INSMEM('OSL2AIAI3',I050,MAXCOR)
C
