      ...
      Subroutine L3_iter(Icore, Maxcor, Iuhf)
...
C  
      Logical Int1, Int2, NONHF, L3STOR, PRESNT, INCORE, INIT
...
C
      ...
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI_LAMBDA(ICORE,MAXCOR,1)
      ENDIF
C
...
C
      ...
      CALL L3_Guess(ICore, Maxcor, Iuhf, .TRUE., .TRUE., INCORE)
...
C
 ...
 2012 FORMAT(' T3_iter, Insufficient memory for T3IT_TRPS2
     ...
     &                NFREE,INT1,INT2,NONHF,IUHF,L3STOR,
     &                .FALSE.)
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_LAMBDA(ICORE,MAXCOR,1)
...
C
 ...
 2022 FORMAT(' T3_iter, Insufficient memory for T3IT_TRPS3 
     ...
     &                IUHF,L3STOR,.FALSE.)
      ...
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_LAMBDA(ICORE,MAXCOR,2)
