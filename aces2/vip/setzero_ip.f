      ...
      SUBROUTINE SETZERO_IP(IUHF, ISPIN, ICORE, MAXCOR, LS1OUT,
     $                      LS2OUT)
C
...
C
      NS1 = NS(SIRREP)
      CALL ZERO(ICORE(1), NS1 * POP(SIRREP,ISPIN) )
...
C
      ...
      ENDIF
         ...
         DO 100 IRREPR = 1 , NIRREP
            ...
            NUMDSS = VRT(IRREPR, MSPIN) * NS1
            IF ((NUMDSS * DISSYS) .GT. 0) THEN
               CALL ZERO(ICORE, DISSYS*NUMDSS)
               CALL PUTLST(ICORE, 1, NUMDSS, 1, IRREPL, LISTS2EX)
            ENDIF
 100     CONTINUE
 50   CONTINUE
C
