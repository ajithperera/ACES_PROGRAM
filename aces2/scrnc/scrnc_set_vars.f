...
c files.com : end
      ...
      If (Iflags2(261) .Gt.0)  Parteom = .True.
      If (Iflags(2)    .Eq.10)  Ccsd    = .True.
      If (Iflags(2)    .Eq.48)  Rccd    = .True.
      If (Iflags(2)    .Eq.49)  Drccd   = .True.
      If (Iflags(19) .Eq.1)   Respn_d = .True.
      Mbpt  = .Not. (Ccsd .OR. Rccd .OR. Drccd)
      Luout = 6

      Aoladder = (Iflags(93).eq. 2)
