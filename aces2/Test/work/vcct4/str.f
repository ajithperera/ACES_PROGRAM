...
c write last record
      icurrec=icurrec+1
c      write(6,*)'last record,irec:',irec
      write(45,rec=irec)buf,ibuf,in
c      write(6,*)'nent,ii,j,k,in',nent(ipn),ii,j,k,in
c write zero record (only when last was nonzero)
      if(in.ne.0)then
         irec=irec+1
         icurrec=icurrec+1
         in=0
      ...
      if(nent(ipn).eq.1)then
         ...
         ico=ikk-icurrec
         iacta=irec+ico+3
c         write(6,*)'no. of rec',icurrec,'  next rec:',iacta
      endif
      return
      end
