      SUBROUTINE TRPS(ICORE,MAXCOR,IUHF,E4T)
     ...
     1                 ETCCSD,EPAREN
      ...
      LOGICAL T3GRAD
      PARAMETER(MAXNBF=600)
      PARAMETER(NUMT3 =200000)
...
C
     ...
     1                E4STA ,E4STB
      COMMON /DENST3/ DOOA(MAXNBF),DOOB(MAXNBF),DVVA(MAXNBF),
     1                                          DVVB(MAXNBF)
...
C
        T3GRAD = .TRUE.
C
C     If this is a numerical optimization and IDRLVL > 0, turn the
C     derivative flag off, but turn on at end.
C
        IF(IFLAGS(47).EQ.6 .OR. IFLAGS2(138).EQ.1)THEN
          IDRLVL = 0
          T3GRAD = .FALSE.
          WRITE(6,1053)
        ENDIF
C
C     If this an energy only numerical frequency calculation, turn the
C     derivative level flag off, but turn back on at end. I don't know
C     what on earth to do for the MIXED option of FD_CALTYPE !
C
        IF(IFLAGS(81).EQ.1.AND.IFLAGS(54).EQ.3)THEN
          IDROLD = IDRLVL
          IDRLVL = 0
          T3GRAD = .FALSE.
          WRITE(6,1054)
...
C     COMPUTE NUMBER OF FREE INTEGER WORDS
     ...
     1            ICORE(I130),
     1            NFREE,IUHF,ISPIN,NO,NV,INT1,INT2,NONHF)
  110 CONTINUE
 1000 CONTINUE
C
