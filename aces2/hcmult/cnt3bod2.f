...
C LOOP OVER IRREPS
C
      Write(6,"(a)") " Printing from 3-body contributions"
      DO 100 IRREPZR=1,NIRREP
       ...
       call checksum("T2AB   :",ICORE(I010),NUMDSW*DISSYW,S)
       CALL GETLST(ICORE(I020),1,1,1,1+IUHF,491)
       call checksum("QIJ    :",ICORE(I020),IRPDPD(IRREPX,22),S)
       CALL GETLST(ICORE(I030),1,1,1,1+IUHF,492)
       call checksum("QAB    :",ICORE(I030),IRPDPD(IRREPX,20),S)
C
...
CSSS        call output(ICORE(IOFFW),1,NROW,1,NSUM,NROW,NSUM,1)
CSSS        call output(ICORE(IOFFG),1,NSUM,1,NROW,NSUM,NROW,1)
        CALL XGEMM('N','N',NROW,NCOL,NSUM,
     &             ONE,ICORE(IOFFW),NROW,
     &                 ICORE(IOFFG),NSUM,
     &             ONE,ICORE(IOFFZ),NROW)
120    CONTINUE
        call checksum("Z(ijab):",ICORE(I000),NUMDSZ*DISSYZ,S)
CSSS`        call output(ICORE(I000),1,DISSYZ,1,NUMDSZ,DISSYZ,NUMDSZ,1)
...
C
      DO 500 ISPIN=1,2
       DO 510 IRREPZR=1,NIRREP
        ...
        call checksum("T2AA   :",ICORE(I010),NUMDST*DISSYT,S)
        CALL GETLST(ICORE(I020),1,1,1,ISPIN,491)
        call checksum("QIJ    :",ICORE(I020),
     &                 IRPDPD(IRREPX,20+ISPIN),S)
        CALL GETLST(ICORE(I030),1,1,1,ISPIN,492)
        call checksum("QAB    :",ICORE(I030),
