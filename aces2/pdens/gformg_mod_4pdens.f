      ...
      SUBROUTINE GFORMG_mod_4pdens(IRREPC,IRREPT,LISTC0,LISTT0,
     &                             LISTG,ICORE,
...
CEND
      ...
      DOUBLE PRECISION ONE,ONEM,FACT,FAC1,FAC2
...
CSSS       CALL SETL21(ICORE, MAXCOR, 1, 1, 190, 134, 1, .TRUE.)
C
...
CSSS          CALL XGEMM('T','N',NROW,NCOL,NSUM,ONE,ICORE(IOFFT),NSUM,
CSSS     &               ICORE(IOFFC),NSUM,ONE,ICORE(IOFFG),NROW)
     ...
     &       (Iflags(18) .EQ. 10)) THEN
          ...
          CALL XGEMM('T','N',NROW,NCOL,NSUM,FAC1,ICORE(IOFFT),NSUM,
     &             ICORE(IOFFC),NSUM,ONE,ICORE(IOFFG),NROW)
120      CONTINUE
C
C
...
C
     ...
     &       (Iflags(18) .EQ. 10)) THEN
          ...
          ELSE
              FAC2 = ONEM
          ENDIF
          CALL XGEMM('T','N',NROW,NCOL,NSUM,FAC2,ICORE(IOFFT),NSUM,
     &               ICORE(IOFFC),NSUM,ONE,ICORE(IOFFG),NROW)
140      CONTINUE
C
C
105     CONTINUE
CSSS        If (LISTG .EQ. 400) Then
CSSS        FACT = 0.0D0
CSSS        CALL SSCAL (LENGIJ,FACT,ICORE(I0GIJ),1)
CSSS        CALL SSCAL (LENGAB,FACT,ICORE(I0GAB),1)
CSSS        else
        CALL SSCAL (LENGIJ,FACT,ICORE(I0GIJ),1)
        CALL SSCAL (LENGAB,FACT,ICORE(I0GAB),1)
CSSS        endif 
