...
C    
C-----
CSSS      Print*, "The input arrays in GDENSVO"
CSSS      Print*, "Values in lists: Alpha", LSTGRL+91, LSTGRL+92
      len1 = IRPDPD(IRREPX,21)
      len2 = IRPDPD(IRREPX,19)
CSSS      call output(SCR(I0GRLIJ(1)), 1, len1, 1, 1, len1, 1, 0)
CSSS      call output(SCR(I0GRLAB(1)), 1, len2, 1, 1, len2, 1, 0)
      len3 = IRPDPD(1,9)
      len4 = IRPDPD(IRREPX,9)
CSSS      Print*, "Values in lists: ", 90, LSTL1
CSSS      call output(SCR(I0T1(1)), 1, len3, 1, 1, len3, 1, 0)
CSSS      call output(SCR(I0L1(1)), 1, len3, 1, 1, len3, 1, 0)
CSSS      Print*, "Values in lists: ", lstr1
CSSS      call output(SCR(I0R1(1)), 1, len4, 1, 1, len4, 1, 0)

CSSS      Print*, "Values in lists: Beta", 90, LSTL1
      len1 = IRPDPD(IRREPX,22)
      len2 = IRPDPD(IRREPX,20)
CSSS      call output(SCR(I0GRLIJ(2)), 1, len1, 1, 1, len1, 1, 0)
CSSS      call output(SCR(I0GRLAB(2)), 1, len2, 1, 1, len2, 1, 0)
      len3 = IRPDPD(1,10)           
      len4 = IRPDPD(IRREPX,10)     
CSSS      Print*, "Values in lists: ", 90, LSTL1
CSSS      call output(SCR(I0T1(2)), 1, len3, 1, 1, len3, 1, 0)
CSSS      call output(SCR(I0L1(2)), 1, len3, 1, 1, len3, 1, 0)
CSSS      Print*, "Values in lists: ", lstr1
CSSS      call output(SCR(I0R1(2)), 1, len4, 1, 1, len4, 1, 0)
C---------      
...
C
      TRIPLET_PERT = .FALSE.
CSSS      IF (IFLAGS(18) .EQ. 9) TRIPLET_PERT =  .TRUE.
C
      DO 4 ISPIN=1,1+IUHF
CSSS      IF (TRIPLET_PERT .AND. ISPIN .EQ. 2) THEN
CSSS         RT = R0
CSSS      ELSE
CSSS         RT =-R0
CSSS      ENDIF
...
C
     ...
     &             DOV(IDVO),1)
CSSS        Print*, "The occ-vrt density"
CSSS        length1 = IRPDPD(IRREPX,8+ISPIN)
CSSS        call output(dov(IDVO), 1,length1, 1, 1, length1, 1, 1)
CSSS        call output(SCR(I0L1(ISPIN)), 1, length1, 1, 1, length1,
CSSS     &              1, 1)
CSSS        call output(dov(IDVO), 1, length1, 1,  1, length1, 1, 1)
...
C
        CALL GT2XF_MOD_4PDENS(SCR(ITOP),IRREPXR,IRREPXL,LSTR2RS,
     &             LSTL1,LSTL1OFF,
...
c       FACT=ZNORM-R0
       FACT=ZNORM
       IF(IRREPX.EQ.1)THEN
CSSS        Print*, "The ZNORM", ZNORM
CSSS        IF (TRIPLET_PERT  .AND. ISPIN .EQ. 2) FACT=-ZNORM
