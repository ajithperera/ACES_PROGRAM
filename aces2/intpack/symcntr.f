...
c first  : the first non-zero coordinate
      ...
      do ixyz=3,1,-1
        ...
        if (coord(ixyz,atom).eq.zero) nzero=nzero+1
        if (coord(ixyz,atom).eq.zero) first0=ixyz
        if (coord(ixyz,atom).ne.zero) first=ixyz
      end do
...
c       - - -    8    -  -  -
        ...
        if      (nzero.eq.3) then
          ...
          integaxis(1,atom)=1
          integaxis(3,atom)=3
        else if (coord(3,atom).eq.zero) then
          numoct=4
          integaxis(3,atom)=3
        else if (nzero.eq.2) then
          numoct=4
