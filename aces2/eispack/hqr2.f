...
C***CATEGORY  D4C2B
C***TYPE      DOUBLE PRECISION (HQR2-S, COMQR2-C)
...
C     of a REAL*8 UPPER Hessenberg matrix by the QR method.  The
C     eigenvectors of a REAL*8 GENERAL matrix can also be found
...
C          REAL*8 array, dimensioned H(NM,N).
C
C        Z contains the transformation matrix produced by  ELTRAN
C          after the reduction by  ELMHES, or by  ORTRAN  after the
C          reduction by  ORTHES, if performed.  If the eigenvectors
C          of the Hessenberg matrix are desired, Z must contain the
C          identity matrix.  Z is a two-dimensional REAL*8 array,
...
C          dimensional REAL*8 arrays, dimensioned WR(N) and WI(N).
C
...
C
      ...
      DOUBLE PRECISION H(NM,*),WR(*),WI(*),Z(NM,*)
      DOUBLE PRECISION P,Q,R,S,T,W,X,Y,RA,SA,VI,VR,ZZ,NORM,S1,S2
      LOGICAL NOTLAS,OVFLSW
      DATA OVFLSW /.TRUE./
C
...
C                COLUMNS M TO EN ..........
  ...
  190    P = P + S
         ...
         Q = Q / P
         R = R / P
cYAU: The following loops are inverted so the conditional is evaluated once.
cYAU|C     .......... ROW MODIFICATION ..........
cYAU|         DO 210 J = K, N
cYAU|            P = H(K,J) + Q * H(K+1,J)
cYAU|            IF (.NOT. NOTLAS) GO TO 200
cYAU|            P = P + R * H(K+2,J)
cYAU|            H(K+2,J) = H(K+2,J) - P * ZZ
cYAU|  200       H(K+1,J) = H(K+1,J) - P * Y
cYAU|            H(K,J) = H(K,J) - P * X
cYAU|  210    CONTINUE
cYAU|         J = MIN(EN,K+3)
cYAU|C     .......... COLUMN MODIFICATION ..........
cYAU|         DO 230 I = 1, J
cYAU|            P = X * H(I,K) + Y * H(I,K+1)
cYAU|            IF (.NOT. NOTLAS) GO TO 220
cYAU|            P = P + ZZ * H(I,K+2)
cYAU|            H(I,K+2) = H(I,K+2) - P * R
cYAU|  220       H(I,K+1) = H(I,K+1) - P * Q
cYAU|            H(I,K) = H(I,K) - P
cYAU|  230    CONTINUE
cYAU|C     .......... ACCUMULATE TRANSFORMATIONS ..........
cYAU|         DO 250 I = LOW, IGH
cYAU|            P = X * Z(I,K) + Y * Z(I,K+1)
cYAU|            IF (.NOT. NOTLAS) GO TO 240
cYAU|            P = P + ZZ * Z(I,K+2)
cYAU|            Z(I,K+2) = Z(I,K+2) - P * R
cYAU|  240       Z(I,K+1) = Z(I,K+1) - P * Q
cYAU|            Z(I,K) = Z(I,K) - P
cYAU|  250    CONTINUE
cYAU: begin new code
         IF (NOTLAS) THEN
...
cYAU: The following code was incorporated from a later version.
c     .......... overflow control ..........
  680       IF (OVFLSW) THEN
               S1 = ABS(H(I,EN))
               IF (S1 .NE. 0.0d0) THEN
