      ...
      SUBROUTINE PCCD_HESS_OV(HESS,ICORE,LENGTH,MAXCOR,ISPIN,IUHF)
C
...
C OR READ HESSIAN H(ab,ij).
C
CEND
      IMPLICIT INTEGER (A-Z)
      DIMENSION ICORE(MAXCOR),IOFFO(2),IOFFV(2)
      DIMENSION HESS(LENGTH)
...
C 
      ...
      IF (APPRX_HESS) Then
         ...
         CALL GETALL(ICORE(I000),NSIZE,IRREPX,LIST_H)

C Reorder (ab,ij) -> (ai,bj)

         CALL SSTGEN(ICORE(I000),HESS,NSIZE,VRT(1,ISPIN),
