...
C     PGrp    "Computational" point group
      ...
      IF(IPRNT.GE.0 .AND. NOSILENT) THEN
         WRITE(6,*) 
         WRITE(LUOUT,8202)
 8202    FORMAT(T3,'Coordinates at entry to symmetry_auto:')
...
CSSS      IF(IPRNT.GE.0 .AND. NOSILENT) THEN
CSSS        WRITE(6,*) 
CSSS        WRITE(LUOUT,8202)
CSSS 8202   FORMAT(T3,'Principal axis orientation for molecule:')
CSSS        WRITE(LUOUT,80)(NEWQ(I),I=1,NX)
CSSS      ENDIF
C
...
C
      ...
      Write(6,*)
      Write(6,"(2a)") "@-SYM_AUTO Current coordinates (in /COORD/)",
     +                " common block"
      Write(6, "(3F12.7)"), (Q(I), I=1, 3*NATOMS)
      Write(6,*)
      Write(6,*) "Coordinates at exit from symmetry_auto)"
