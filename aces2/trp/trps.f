      ...
      SUBROUTINE TRPS(ICORE,MAXCOR,IUHF,E4T,FINAL,EXTRNL_CCSD)
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI_TRP(ICORE,MAXCOR,1,EXTRNL_CCSD)
      ENDIF
...
cYAU      WRITE(6,2001) MAXCOR,NALLOC,NFREE
     ...
     1            INT2,NONHF,.FALSE.,FINAL,EXTRNL_CCSD)
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_TRP(ICORE,MAXCOR,1,EXTRNL_CCSD)
C
...
C
      IF(IUHF.EQ.1)THEN
        CALL TSPABCI2_TRP(ICORE,MAXCOR,2,EXTRNL_CCSD)
C
...
C
      IF(IUHF.EQ.1)THEN
         CALL TSPABCI_TRP(ICORE,MAXCOR,2,EXTRNL_CCSD)
      ENDIF
C
C     For CCSDT negate certain MBEJ lists.
C
...
CSSS          call check_ints(icore,maxcor,iuhf,.false.)
          ...
          IF (IFLAGS(2) .EQ. 50) IFLAGS(2) = 51
CSSS          IFLAGS(2) = 10
CSSS          IF (IFLAGS(38) .EQ. 0) IFLAGS(38) = 1
