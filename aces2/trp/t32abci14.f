      ...
      SUBROUTINE T32ABCI14(W,ICORE,IADW,IRPI,IRPJ,IRPK,IRPIJ,IRPIK,
     1                     IRPJK,IRPIJK,I,J,K,ISPIN,LAMBDA)
...
C       BB BB            BBB  BB
C
C     T IS (B<C,D) REPRESENTATION OF T3 (B<C<D).
C
C     SET ADDRESSES FOR EXPANDED W(D,A).
C
      DSZEXP = 0
      DO   10 IRPA=1,NIRREP
   ...
   10 CONTINUE
C
C     READ W(D<A,J<K) AND EXPAND IN PLACE.
C
      IF(IRPJK.EQ.1)THEN
      ...
      ENDIF
      CALL GETLST(ICORE(1),JK,1,1,IRPJK,13+ISPIN+LISTOFF)
C
...
C
     ...
     1            AI,VRT(IRPA,ISPIN),2,IRPAI,LISABCI)
...
C
  ...
  110 CONTINUE
C
C     READ W(D<A,I<K) AND EXPAND IN PLACE.
C
      IF(IRPIK.EQ.1)THEN
      ...
      ENDIF
      CALL GETLST(ICORE(1),IK,1,1,IRPIK,13+ISPIN+LISTOFF)
C
...
C
      ...
      CALL GETLST(ICORE(IADG(IRPA)),
     1            AJ,VRT(IRPA,ISPIN),2,IRPAJ,LISABCI)
      CALL XGEMM('N','N',IRPDPD(IRPBC,ISPIN),VRT(IRPA,ISPIN),
     1           VRT(IRPD,ISPIN),-1.0D+00,
     1           W(IADW(IRPD)),IRPDPD(IRPBC,ISPIN),
     1           ICORE(IADT2E(IRPA)),VRT(IRPD,ISPIN),
...
C
  ...
  210 CONTINUE
C
C     READ W(D<A,I<J) AND EXPAND IN PLACE.
C
      IF(IRPIJ.EQ.1)THEN
      ...
      ENDIF
      CALL GETLST(ICORE(1),IJ,1,1,IRPIJ,13+ISPIN+LISTOFF)
C
...
C
      ...
      CALL GETLST(ICORE(IADG(IRPA)),
     1            AK,VRT(IRPA,ISPIN),2,IRPAK,LISABCI)
      CALL XGEMM('N','N',IRPDPD(IRPBC,ISPIN),VRT(IRPA,ISPIN),
     1           VRT(IRPD,ISPIN), 1.0D+00,
     1           W(IADW(IRPD)),IRPDPD(IRPBC,ISPIN),
     1           ICORE(IADT2E(IRPA)),VRT(IRPD,ISPIN),
