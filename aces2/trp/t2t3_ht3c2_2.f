      ...
      SUBROUTINE T2T3_HT3C2_2(T3,W,ICORE,IADT3,IADW,IRPI,IRPJ,IRPK,
     &                        IRPIJ,IRPIK,IRPJK,IRPIJK,I,J,K,IUHF,
     &                        SCR1,SCR2,SCR3,LAMBDA) 
...
C
     ...
     1            IM,1,1,IRPIM,LIST2I1+LISTOFF)
   10 CONTINUE
C
...
C
      ...
      CALL GETLST(ICORE(IOFFT2),IM,POP(IRPM,1),1,IRPIM,LIST2I1+LISTOFF)
C
      CALL XGEMM('N','T',IRPDPD(IRPAB,1),POP(IRPM,1),VRT(IRPC,2),
     1           1.0D+00,
     1           T3(IADT3(IRPC)),IRPDPD(IRPAB,1),
     ...
     1           1.0D+00,ICORE(IOFFT2),IRPDPD(IRPAB,1))
C
      CALL PUTLST(ICORE(IOFFT2),IM,POP(IRPM,1),1,IRPIM,LIST2I1+LISTOFF)
      ENDIF
C
...
C
      ...
      CALL GETLST(ICORE(IOFFT2+(M-1)*IRPDPD(IRPAB,1)),
     1            JM,1,1,IRPJM,LIST2I1+LISTOFF)
   60 CONTINUE
C
C    D2T2(A<B,M) = T3(A<B,c) * V(c,M)
C
...
C
      ...
      CALL GETLST(ICORE(IOFFT2),JM,POP(IRPM,1),1,IRPJM,LIST2I1+LISTOFF)
C
      CALL XGEMM('N','T',IRPDPD(IRPAB,1),POP(IRPM,1),VRT(IRPC,2),
     1          -1.0D+00,
     1           T3(IADT3(IRPC)),IRPDPD(IRPAB,1),
     ...
     1           1.0D+00,ICORE(IOFFT2),IRPDPD(IRPAB,1))
C
      CALL PUTLST(ICORE(IOFFT2),JM,POP(IRPM,1),1,IRPJM,LIST2I1+LISTOFF)
      ENDIF
C
...
C
      ...
      CALL GETLST(ICORE(IOFFT2),MK,POP(IRPM,1),1,IRPMK,LIST2I3+LISTOFF)
C
...
C
     ...
     1           1.0D+00,ICORE(IOFFT2),IRPDPD(IRPBC,13))
      CALL PUTLST(ICORE(IOFFT2),MK,POP(IRPM,1),1,IRPMK,LIST2I3+LISTOFF)
C
...
C
     ...
     1            IM,1,1,IRPIM,LIST2I3+LISTOFF)
  160 CONTINUE
C
...
C
      ...
      CALL GETLST(ICORE(IOFFT2+(M-1)*IRPDPD(IRPBC,13)),
     1            JM,1,1,IRPJM,LIST2I3+LISTOFF)
  210 CONTINUE
C
      CALL XGEMM('N','N',IRPDPD(IRPBC,13),POP(IRPM,2),VRT(IRPA,1),
     1           -1.0D+00,
...
C
      ...
      IRPBC = IRPMK
C
      CALL XGEMM('N','T',IRPDPD(IRPMK,13),POP(IRPM,1),VRT(IRPA,1),
     1            1.0D+00,
     1            W(IADW(IRPA)),IRPDPD(IRPBC,13),
     1            ICORE(IOFFV),POP(IRPM,1),0.0D+00,
     1            ICORE(IADT2),IRPDPD(IRPBC,13))
C
      IADT2I = IADT2        + IRPDPD(IRPMK,13) * POP(IRPM,1)
      DO  520 M=1,POP(IRPM,1)
C
      MK = IOFFOO(IRPK,IRPMK,5) + (K-1)*POP(IRPM,1) + M
      CALL GETLST(ICORE(IADT2I),MK,1,1,IRPMK,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPMK,13)),
     1            IRPDPD(IRPMK,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),MK,1,1,IRPMK,LIST2I3+LISTOFF)
C
      CALL SYMTR3(IRPMK,VRT(1,2),VRT(1,1),IRPDPD(IRPMK,13),1,
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPMK,13)),
     1            SCR1,SCR2,SCR3)
C
      KM = IOFFOO(IRPM,IRPMK,6) + (M-1)*POP(IRPK,2) + K
      CALL GETLST(ICORE(IADT2I),KM,1,1,IRPMK,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPMK,13)),
     1            IRPDPD(IRPMK,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),KM,1,1,IRPMK,LIST2I3+LISTOFF)
  520 CONTINUE
C
      IOFFV = IOFFV + POP(IRPM,1) * VRT(IRPA,1)
  550 CONTINUE
...
C
      ...
      IRPBC = IRPIM
C
      CALL XGEMM('N','T',IRPDPD(IRPIM,13),POP(IRPM,2),VRT(IRPA,1),
     1            1.0D+00,
     1            W(IADW(IRPA)),IRPDPD(IRPIM,13),
     1            ICORE(IOFFV),POP(IRPM,2), 0.0D+00,
     1            ICORE(IADT2),IRPDPD(IRPIM,13))
C
      IADT2I = IADT2        + IRPDPD(IRPIM,13) * POP(IRPM,2)
C
      DO  560 M=1,POP(IRPM,2)
C
      IM = IOFFOO(IRPM,IRPIM,5) + (M-1)*POP(IRPI,1) + I
      CALL GETLST(ICORE(IADT2I),IM,1,1,IRPIM,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPIM,13)),
     1            IRPDPD(IRPIM,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),IM,1,1,IRPIM,LIST2I3+LISTOFF)
C
      CALL SYMTR3(IRPIM,VRT(1,2),VRT(1,1),IRPDPD(IRPIM,13),1,
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPIM,13)),
     1            SCR1,SCR2,SCR3)
C
      MI = IOFFOO(IRPI,IRPIM,6) + (I-1)*POP(IRPM,2) + M
      CALL GETLST(ICORE(IADT2I),MI,1,1,IRPIM,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPIM,13)),
     1            IRPDPD(IRPIM,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),MI,1,1,IRPIM,LIST2I3+LISTOFF)
  560 CONTINUE
C
...
C
      ...
      IRPBC = IRPJM
C
      CALL XGEMM('N','T',IRPDPD(IRPJM,13),POP(IRPM,2),VRT(IRPA,1),
     1            -1.0D+00,
     1            W(IADW(IRPA)),IRPDPD(IRPJM,13),
     1            ICORE(IOFFV),POP(IRPM,2), 0.0D+00,
     1            ICORE(IADT2),IRPDPD(IRPJM,13))
C
      IADT2I = IADT2        + IRPDPD(IRPJM,13) * POP(IRPM,2)
C
      DO  660 M=1,POP(IRPM,2)
C
      JM = IOFFOO(IRPM,IRPJM,5) + (M-1)*POP(IRPJ,1) + J
      CALL GETLST(ICORE(IADT2I),JM,1,1,IRPJM,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPJM,13)),
     1            IRPDPD(IRPJM,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),JM,1,1,IRPJM,LIST2I3+LISTOFF)
C
      CALL SYMTR3(IRPJM,VRT(1,2),VRT(1,1),IRPDPD(IRPJM,13),1,
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPJM,13)),
     1            SCR1,SCR2,SCR3)
C
      MJ = IOFFOO(IRPJ,IRPJM,6) + (J-1)*POP(IRPM,2) + M
      CALL GETLST(ICORE(IADT2I),MJ,1,1,IRPJM,LIST2I3+LISTOFF)
      CALL   VADD(ICORE(IADT2I),ICORE(IADT2I),
     1            ICORE(IADT2 + (M-1)*IRPDPD(IRPJM,13)),
     1            IRPDPD(IRPJM,13), 1.0D+00)
      CALL PUTLST(ICORE(IADT2I),MJ,1,1,IRPJM,LIST2I3+LISTOFF)
  660 CONTINUE
C
      IOFFV = IOFFV + POP(IRPM,2) * VRT(IRPA,1)
  700 CONTINUE
