      ...
      Subroutine Newt2(T2aa_new,T2bb_new,T2ab_new,Resid_aa,Resid_bb,
     +                 Resid_ab,fockoo_a,fockoo_b,fockvv_a,fockvv_b,
...
C AAAA block 
     ...
     +            Fockvv_a(A,A)*E_a-Fockvv_a(B,B)*E_b)
         If (Dabs(Daaaa) .Gt. Denom_tol) Then  
             T2aa_new(A,B,I,J) = Resid_aa(A,B,I,J)/Daaaa
         ELse
...
C BBBB block 
     ...
     +            Fockvv_b(a,a)*E_a-Fockvv_b(b,b)*E_b)
         If (Dabs(Dbbbb) .Gt. Denom_tol) Then  
            T2bb_new(a,b,i,j) = Resid_bb(a,b,i,j)/Dbbbb
         Else 
...
C ABAB block 
     ...
     +            Fockvv_a(A,A)*E_a-Fockvv_b(b,b)*E_b)
         If (Dabs(Dabab) .Gt. Denom_tol) Then  
            T2ab_new(A,b,I,j) = Resid_ab(A,b,I,j)/Dabab
         Else 
