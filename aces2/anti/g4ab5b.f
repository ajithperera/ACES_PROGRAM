      SUBROUTINE G4AB5B(GAMMA,BUF,GOFF,DOIRR,LENGAM,FABCI)
C
C THIS ROUTINE PROCESSES THE G<Ab,Ci> AND G<Ab,Ic> GAMMA LISTS 
C  AND FILLS OUT THE ABCD SYMMETRY CASE MULLIKEN ORDERED GAMMA FOR 
C  SPIN CASE BBAA.  THIS IS FOR UHF CALCULATIONS ONLY!
C
CEND
      IMPLICIT INTEGER (A-Z)
      DOUBLE PRECISION GAMMA(LENGAM),BUF(1),X,X1,X2 
      DOUBLE PRECISION FABIJ,FIJKL,FABCD,FAIBJ,FABCI,FIJKA
      INTEGER GOFF(8,8,8,8),IDID(8),IOFFSL(8)
      INTEGER IOFFSR(8),DOIRR(42,4)
      LOGICAL MBPT2,MBPT3,M4DQ,M4SDQ,M4SDTQ,CCD,QCISD,CCSD
      COMMON /METH/ MBPT2,MBPT3,M4DQ,M4SDQ,M4SDTQ,CCD,QCISD,CCSD
      COMMON /MACHSP/ IINTLN,IFLTLN,IINTFP,IALONE,IBITWD
      COMMON /SYMINF/ NSTART,NIRREP,IRREPS(255,2),DIRPRD(8,8)
      COMMON /SYM/ POP(8,2),VRT(8,2),NT(2),NFMI(2),NFEA(2)
      COMMON /SYMPOP/ IRPDPD(8,22),ISYTYP(2,500),ID(18)
      COMMON /MOPOPS/ MOPOP(8)
C
      INDXF(I,J,N)=I+(J-1)*N
      INDXT(I,J)  =I+(J*(J-1))/2
      NNP1O2(I)   =(I*(I+1))/2
C
C CALCULATE HOW MANY UNIQUE ABCD SYMMETRY SETS THERE ARE
C
      IF(NIRREP.EQ.8)THEN
       NUMABCD=42
      ELSEIF(NIRREP.EQ.4)THEN
       NUMABCD=3
      ENDIF
C     
      DO 100 ICOUNT=1,NUMABCD
       IRREP1M=DOIRR(ICOUNT,1)
       IRREP2M=DOIRR(ICOUNT,2)
       IRREP3M=DOIRR(ICOUNT,3)
       IRREP4M=DOIRR(ICOUNT,4)
       IOFFSET=GOFF(IRREP1M,IRREP2M,IRREP3M,IRREP4M)
C
C FIRST DO LIST 130 
C
C          12 34        31 42
C        G(bi,AC) <-- g<Ab,Ci>
C
C          12 34        42 31
C        G(ib,CA) <-- g<Ab,Ci>
C
C          12 34        41 32
C        G(bi,CA) <-- g<Ab,Ci>
C
C          12 34        32 41
C        G(ib,AC) <-- g<Ab,Ci>
C
       LISTABCI=130
       IRREPGET=DIRPRD(IRREP2M,IRREP4M)
C
       IOFFR=1
       IOFFL=1
       DO 2000 IRREPA=1,NIRREP
        IRREPB=DIRPRD(IRREPA,IRREPGET)
        IOFFSR(IRREPA)=IOFFR
        IOFFSL(IRREPA)=IOFFL
        IOFFR=IOFFR+VRT(IRREPB,1)*POP(IRREPA,2)
        IOFFL=IOFFL+VRT(IRREPB,1)*VRT(IRREPA,2)
2000   CONTINUE
C
C          12 34        31 42
C        G(bi,AC) <-- g<Ab,Ci>
C
       ILOGREC=IOFFSR(IRREP2M)
       IFIRST=IOFFSL(IRREP1M)
       NUMI=POP(IRREP2M,2)
       NUMC=VRT(IRREP4M,1)
       NUMB=VRT(IRREP1M,2)
       NUMA=VRT(IRREP3M,1) 
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 120 INDXI=1,NUMI
        DO 130 INDXC=1,NUMC
         INDI=INDXI
         INDC=INDXC+POP(IRREP4M,1)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP1M)
         DO 140 INDXB=1,NUMB
          DO 150 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP1M,2)
           INDA=INDXA+POP(IRREP3M,1)
           GINDXL=INDXF(INDB,INDI,MOPOP(IRREP1M))
           GINDXR=INDXF(INDA,INDC,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
150       CONTINUE
140      CONTINUE
130     CONTINUE
120    CONTINUE
C
C          12 34        42 31
C        G(ib,CA) <-- g<Ab,Ci>
C
       ILOGREC=IOFFSR(IRREP1M)
       IFIRST=IOFFSL(IRREP2M)
       NUMI=POP(IRREP1M,2)
       NUMC=VRT(IRREP3M,1) 
       NUMB=VRT(IRREP2M,2)
       NUMA=VRT(IRREP4M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 420 INDXI=1,NUMI
        DO 430 INDXC=1,NUMC
         INDI=INDXI
         INDC=INDXC+POP(IRREP3M,1)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP2M)
         DO 440 INDXB=1,NUMB
          DO 450 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP2M,2)
           INDA=INDXA+POP(IRREP4M,1)
           GINDXL=INDXF(INDI,INDB,MOPOP(IRREP1M))
           GINDXR=INDXF(INDC,INDA,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
450       CONTINUE
440      CONTINUE
430     CONTINUE
420    CONTINUE
C
C          12 34        41 32
C        G(bi,CA) <-- g<Ab,Ci>
C
       IRREPGET=DIRPRD(IRREP1M,IRREP4M)
       IOFFR=1
       IOFFL=1
C
       DO 1000 IRREPA=1,NIRREP
        IRREPB=DIRPRD(IRREPA,IRREPGET)
        IOFFSR(IRREPA)=IOFFR
        IOFFSL(IRREPA)=IOFFL
        IOFFR=IOFFR+VRT(IRREPB,1)*POP(IRREPA,2)
        IOFFL=IOFFL+VRT(IRREPB,1)*VRT(IRREPA,2)
1000   CONTINUE
C
       ILOGREC=IOFFSR(IRREP2M)
       IFIRST=IOFFSL(IRREP1M)
       NUMI=POP(IRREP2M,2)
       NUMC=VRT(IRREP3M,1) 
       NUMB=VRT(IRREP1M,2)
       NUMA=VRT(IRREP4M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 220 INDXI=1,NUMI
        DO 230 INDXC=1,NUMC
         INDI=INDXI
         INDC=INDXC+POP(IRREP3M,1)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP1M)
         DO 240 INDXB=1,NUMB
          DO 250 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP1M,2)
           INDA=INDXA+POP(IRREP4M,1)
           GINDXL=INDXF(INDB,INDI,MOPOP(IRREP1M))
           GINDXR=INDXF(INDC,INDA,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
250       CONTINUE
240      CONTINUE
230     CONTINUE
220    CONTINUE
C
C          12 34        32 41
C        G(ib,AC) <-- g<Ab,Ci>
C
       ILOGREC=IOFFSR(IRREP1M)
       IFIRST=IOFFSL(IRREP2M)
       NUMI=POP(IRREP1M,2)
       NUMC=VRT(IRREP4M,1) 
       NUMB=VRT(IRREP2M,2)
       NUMA=VRT(IRREP3M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 320 INDXI=1,NUMI
        DO 330 INDXC=1,NUMC
         INDI=INDXI
         INDC=INDXC+POP(IRREP4M,1)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP2M)
         DO 340 INDXB=1,NUMB
          DO 350 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP2M,2)
           INDA=INDXA+POP(IRREP3M,1)
           GINDXL=INDXF(INDI,INDB,MOPOP(IRREP1M))
           GINDXR=INDXF(INDA,INDC,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
350       CONTINUE
340      CONTINUE
330     CONTINUE
320    CONTINUE
C
C NOW DO LIST 129
C
C          12 34        31 42
C        G(bc,AI) <-- g<Ab,Ic>
C
C          12 34        42 31
C        G(cb,IA) <-- g<Ab,Ic>
C
C          12 34        41 32
C        G(bc,IA) <-- g<Ab,Ic>
C
C          12 34        32 41
C        G(cb,AI) <-- g<Ab,Ic>
C
       LISTABCI=129
       IRREPGET=DIRPRD(IRREP2M,IRREP4M)
C
       IOFFR=1
       IOFFL=1
       DO 4000 IRREPA=1,NIRREP
        IRREPB=DIRPRD(IRREPA,IRREPGET)
        IOFFSR(IRREPA)=IOFFR
        IOFFSL(IRREPA)=IOFFL
        IOFFR=IOFFR+POP(IRREPB,1)*VRT(IRREPA,2)
        IOFFL=IOFFL+VRT(IRREPB,1)*VRT(IRREPA,2)
4000   CONTINUE
C
C          12 34        31 42
C        G(bc,AI) <-- g<Ab,Ic>
C
       ILOGREC=IOFFSR(IRREP2M)
       IFIRST=IOFFSL(IRREP1M)
       NUMC=VRT(IRREP2M,2)
       NUMI=POP(IRREP4M,1)
       NUMB=VRT(IRREP1M,2)
       NUMA=VRT(IRREP3M,1) 
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 1120 INDXC=1,NUMC
        DO 1130 INDXI=1,NUMI
         INDI=INDXI
         INDC=INDXC+POP(IRREP2M,2)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP1M)
         DO 1140 INDXB=1,NUMB
          DO 1150 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP1M,2)
           INDA=INDXA+POP(IRREP3M,1)
           GINDXL=INDXF(INDB,INDC,MOPOP(IRREP1M))
           GINDXR=INDXF(INDA,INDI,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
1150       CONTINUE
1140      CONTINUE
1130     CONTINUE
1120    CONTINUE
C
C          12 34        42 31
C        G(cb,IA) <-- g<Ab,Ic>
C
       ILOGREC=IOFFSR(IRREP1M)
       IFIRST=IOFFSL(IRREP2M)
       NUMC=VRT(IRREP1M,2) 
       NUMI=POP(IRREP3M,1)
       NUMB=VRT(IRREP2M,2)
       NUMA=VRT(IRREP4M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 1420 INDXC=1,NUMC
        DO 1430 INDXI=1,NUMI
         INDI=INDXI
         INDC=INDXC+POP(IRREP1M,2)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP2M)
         DO 1440 INDXB=1,NUMB
          DO 1450 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP2M,2)
           INDA=INDXA+POP(IRREP4M,1)
           GINDXL=INDXF(INDC,INDB,MOPOP(IRREP1M))
           GINDXR=INDXF(INDI,INDA,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
1450       CONTINUE
1440      CONTINUE
1430     CONTINUE
1420    CONTINUE
C
C          12 34        41 32
C        G(bc,IA) <-- g<Ab,Ic>
C
       IRREPGET=DIRPRD(IRREP1M,IRREP4M)
       IOFFR=1
       IOFFL=1
C
       DO 5000 IRREPA=1,NIRREP
        IRREPB=DIRPRD(IRREPA,IRREPGET)
        IOFFSR(IRREPA)=IOFFR
        IOFFSL(IRREPA)=IOFFL
        IOFFR=IOFFR+POP(IRREPB,1)*VRT(IRREPA,2)
        IOFFL=IOFFL+VRT(IRREPB,1)*VRT(IRREPA,2)
5000   CONTINUE
C
       ILOGREC=IOFFSR(IRREP2M)
       IFIRST=IOFFSL(IRREP1M)
       NUMC=VRT(IRREP2M,2) 
       NUMI=POP(IRREP3M,1)
       NUMB=VRT(IRREP1M,2)
       NUMA=VRT(IRREP4M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 1220 INDXC=1,NUMC
        DO 1230 INDXI=1,NUMI
         INDI=INDXI
         INDC=INDXC+POP(IRREP2M,2)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP1M)
         DO 1240 INDXB=1,NUMB
          DO 1250 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP1M,2)
           INDA=INDXA+POP(IRREP4M,1)
           GINDXL=INDXF(INDB,INDC,MOPOP(IRREP1M))
           GINDXR=INDXF(INDI,INDA,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
1250       CONTINUE
1240      CONTINUE
1230     CONTINUE
1220    CONTINUE
C
C          12 34        32 41
C        G(cb,AI) <-- g<Ab,Ic>
C
       ILOGREC=IOFFSR(IRREP1M)
       IFIRST=IOFFSL(IRREP2M)
       NUMC=VRT(IRREP1M,2) 
       NUMI=POP(IRREP4M,1)
       NUMB=VRT(IRREP2M,2)
       NUMA=VRT(IRREP3M,1)
       MAXL=MOPOP(IRREP1M)*MOPOP(IRREP2M)
       DO 1320 INDXC=1,NUMC
        DO 1330 INDXI=1,NUMI
         INDI=INDXI
         INDC=INDXC+POP(IRREP1M,2)
         CALL GETLST(BUF,ILOGREC,1,1,IRREPGET,LISTABCI)
         ILOGREC=ILOGREC+1
         IPOS=IOFFSL(IRREP2M)
         DO 1340 INDXB=1,NUMB
          DO 1350 INDXA=1,NUMA
           X=BUF(IPOS)*FABCI
           IPOS=IPOS+1
           INDB=INDXB+POP(IRREP2M,2)
           INDA=INDXA+POP(IRREP3M,1)
           GINDXL=INDXF(INDC,INDB,MOPOP(IRREP1M))
           GINDXR=INDXF(INDA,INDI,MOPOP(IRREP3M))
           GINDX0=INDXF(GINDXL,GINDXR,MAXL)
           GINDX =GINDX0+IOFFSET
           GAMMA(GINDX)=GAMMA(GINDX)+X
1350       CONTINUE
1340      CONTINUE
1330     CONTINUE
1320    CONTINUE
C
100   CONTINUE
      RETURN
      END
