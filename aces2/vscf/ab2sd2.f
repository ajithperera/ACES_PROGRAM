      ...
      SUBROUTINE AB2SD2(DENSA,DENSB,DENSSD,LEN)
C
...
C
      DO 100 I=1,NIRREP
        ...
        ELSE
Cjdw 2. These two lines make sure we form doubles density even if the
C       symmetry block has open-shell orbitals (cf AB2SD).
          CALL SCOPY(ITRILN(I),DENSB(ITRIOF(I)),1,DENSSD(ITRIOF(I)),1)
          CALL SSCAL(ITRILN(I),TWO,DENSSD(ITRIOF(I)),1)
