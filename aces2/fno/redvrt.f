...
c     open the ZMAT file and search for the fno_gen namelist
      ...
      Write(6,*) "The FNO keep string", iflags2(144)
      if (iflags2(144).lt.10) 
     &   write(6,1) iflags2(144)
      if ((iflags2(144).ge.10).and.
     &   (iflags2(144).lt.100)) 
     &   write(6,2) iflags2(144)
      if (iflags2(144).eq.100) 
     &   write(6,3) iflags2(144)
...
c     alpha spin
 ...
 56            continue
            ...
            goto 900 
         else if(iflags2(144).ne.0)then
            vrto=nvrto(spin)
            if (rohf) vrto=nvrto(1)
            idrop=nint(vrto*((100-iflags2(144))/100.0d0))
     &         +dropmo
