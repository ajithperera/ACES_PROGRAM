...
C DRIVER FOR SORTING <AB|CD> INTEGRALS, RHF CASE.

      SUBROUTINE DABCD0(ICORE,MEMSIZ,NUMIRW,ISYM,IPW,IPDIS,
     &                  IPDSZ,NWDIS,IUHF,GRAD,NMO,IRRVEC)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)  
      LOGICAL GRAD
      INTEGER DISSIZ,DIRPRD
      DIMENSION ICORE(MEMSIZ),NUMIRW(2*NIRREP),ISYM(1),
     &          IPW(8),IPDIS(8),IPDSZ(8),IRRVEC(NMO,2)
...
4000   FORMAT(T3,'@DABCD0-F, Not enough core to hold one integral ',
     &        'distribution.',/,
...
CSSS      WRITE(6,*)' I100 Is ',I100,'  MEMSIZ IS ',MEMSIZ
      IF(I100.GT.MEMSZ)THEN
       WRITE(6,5000)
5000   FORMAT(T3,'@DABCD0-F, Insufficient core to perform sort.')
       CALL INSMEM('DABCD0',I100,MEMSZ)
       CALL ERREX
      ENDIF
      WRITE(6,1000)MEMSZ,MEMSIZ,DISSIZ*LENINT,NUMDIS,NINCOR,NBUCK,
     &        NDBCK,NBKINT,IRECL,I100
1000   FORMAT(T3,'@DABCD0-I, Information for AO integral sort',/,
     ...
     &   '        Total utilized memory (words)          ',I10,'.')
      CALL SABCD0(ICORE(I000),ICORE(I010),ICORE(I020),ICORE(I030),
     &           ICORE(I040),ICORE(I050),NBUCK,NDBCK,
     &           NBKINT,NREC,IRECL,ILNBUF,ISYM,IUHF,GRAD,
...
200   FORMAT(T3,'@DABCD0-I, Sort chain processing uses ',I10,
     &          ' out of ',I10,' words.')
      IF(I040.GT.MEMSZ)THEN
       WRITE(6,300)I040,MEMSZ
       CALL ERREX
300    FORMAT(T3,'@DABCD0-F, Core screwup. ',I10,' words needed.',
     &           I10,' words available.')
      ENDIF
      CALL CABCD0(ICORE(I010),ICORE(I020),ICORE(I030),ICORE(I000),
     &            DISSIZ,NDBCK,NBKINT,NBUCK,IRECL,NUMIRW,ISYM,IPW,
     &            IPDIS,IPDSZ,NWDIS,IUHF,GRAD,IRRVEC(1,1),IRRVEC(1,2))
      RETURN
      END
