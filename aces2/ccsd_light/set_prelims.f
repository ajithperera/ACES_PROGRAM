...
c maxbasfn.par : end
     ...
     +                             NOcc_a,Nocc_b,Nvrt_a,Nvrt_b
      ...
      Denom_tol  = Denom_tol*100
      Save_ints  = IFLAGS(26) .GT. 0
      Frac_occ   = .False. 
      Brueck     = IFLAGS(22) .GT. 0
      Icontl     = IFLAGS(76)
      Brueck_tol = 10.D0**(-Icontl)
      lineq_mxcyc= IFLAGS(97)
      Icontl     = IFLAGS(98)
      lineq_tol  = 10.D0**(-Icontl)
      Damp_cc    = IFLAGS2(170)*0.01D0
...
C following logic assuares that.
     ...
     +                        Active_space
      Write(6,"(a,l,F15.10)") " Regularize and Regularization factor =", 
     +                          Regular, Rfac
      Write(6,*)
      Return
      End 
