...
C
      IMODE = 0
      CALL INIPCK(1, 9,  9 , 200, IMODE, 0, 1)
      CALL INIPCK(1, 10, 10, 201, IMODE, 0, 1)
      CALL INIPCK(1, 9,  10, 202, IMODE, 0, 1)
...
C
      ...
      DO IRREP=1,NIRREP
         WRITE(6,*) 
         WRITE(6,"(a,i1)") " Serching for roots in symmetry block: ",
     &                       IRREP
C
...
CSS         IF(IEND.GT.MAXCOR)CALL INSMEM('DO_UHF_RPA', IENDD, MAXCOR)
...
C
         DO ILOOP = 1, ROWDIM
            ...
            IF (WORK(I000P + IOFF) .LT. 0.0D0) Then
               WRITE(6,*)
               WRITE(6,"(3a)")" The A+B matrix is not positive",
     &                        " definite in this irrep and the",
     &                        " roots are not"
               WRITE(6,"(2a)") " searched. Continuing to search for",
     &                         " roots in the in the next irrep." 
CSSS               CALL ERREX
               GO TO 100
            ENDIF
...
C
            ...
            IENDD = IDUMY + ROWDIM
            IF(IENDD.GT.MAXCOR)CALL INSMEM('DO_UHF_RPA', IENDD, MAXCOR)
            MEMLEFT  = MAXCOR - IENDD 
...
C
     ...
     &                   0.0D0, WORK(ISVEC), ROWDIM)
             ...
             ENDDO
             CALL PROCESS_UHF_RPA_ROOTS(WORK(IZVEC),WORK(IYVEC),
     &                                  EIGVAL,ROWDIM,IRREP, 
     &                                  SIZEAA,IUHF,WORK(IENDD),
     &                                  MEMLEFT)
...
C
             ...
             IT2BA = IT2AB + LT2ABAB
             IENDD = IT2BA + LT2BABA
             IF(IENDD.GT.MAXCOR)CALL INSMEM('DO_UHF_RPA', IENDD, 
     &                                       MAXCOR)
...
C
     ...
     &                     1)             
            ...
            CALL PUTLST(WORK(IT2AB), 1, LOBVA, 1, IRREP, 202)
...
C
      ...
      IF (.NOT. RCCD_EOM) CALL MKT1AT2(WORK, MAXCOR)
C
      RETURN
      END
