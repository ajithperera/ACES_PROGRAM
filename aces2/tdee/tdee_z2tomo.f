      ...
      SUBROUTINE TDEE_Z2TOMO(ICORE,MAXCOR,IUHF,TAU,IRREPX,LSTMO,
     &                       LSTMOINC,LSTAOINC,NOINC,ISIDE)
C
...
C
      ...
      IF(ISIDE.EQ.2)THEN
       I0T(1)=1
       CALL GETLST(ICORE(I0T(1)),1,1,1,1,396)
       IF(IUHF.NE.0)THEN
        I0T(2)=I0T(1)+IRPDPD(IRREPX,9)*IINTFP
        CALL GETLST(ICORE(I0T(2)),1,1,1,2,396)
       ELSE
        I0T(2)=I0T(1)
       ENDIF
...
C
      IF(ISIDE.EQ.2)THEN
       DO 9 ISPIN=1,1+IUHF
        CALL PUTLST(ICORE(I0T(ISPIN)),1,1,1,ISPIN,396)
        If (ispin .eq.1) call checksum("396-AA:  ",ICORE(I0T(Ispin)),
     +                                  IRPDPD(IRREPX,9),s)
        If (ispin .eq.2) call checksum("396-BBi: ",ICORE(I0T(Ispin)),
     +                                  IRPDPD(IRREPX,10),s)
9      CONTINUE
      ENDIF
C
