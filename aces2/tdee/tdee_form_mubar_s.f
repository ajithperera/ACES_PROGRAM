...
C
      SUBROUTINE Tdee_form_mubar_s(MIJ,MAB,MAI,MIJBAR,MABBAR,MAIBAR,
...
C
       ...
       ENDIF
       If (iside .eq.1)  call Checksum("tdee_form_mubar_r(f)  :", 
     +                                  MAIBAR(Ioffvo0),Lenvo,S)

       If (iside .eq.2)  call Checksum("tdee_form_mubar_l(f)  :", 
     +                                 MAIBAR(Ioffvo0),Lenvo,S)
       IF (ISIDE .EQ. 1) THEN
          CALL PUTLST(MAIBAR(IOFFVO0),1,1,1,ISPIN,390)
       ELSE IF (ISIDE .EQ. 2) THEN
          CALL PUTLST(MAIBAR(IOFFVO0),1,1,1,ISPIN,392)
       ENDIF 
C
210   CONTINUE
C
