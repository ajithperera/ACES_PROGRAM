      ...
      SUBROUTINE TDEE_DLADDER(ICORE,MAXCOR,IUHF,IRREPX,ITYPE,
...
C Note that ISIDE=1 block in the following block does not get executed. 
     ...
     &              IRPDPD(IRREPX,10),DISSYZ,NUMDSZ)
         ...
         IF (ISIDE .EQ.1)THEN
            CALL GETLST(ICORE(I0L(1)),1,1,1,1,394)
         ELSE IF (ISIDE .EQ.2) THEN
            CALL GETLST(ICORE(I0L(1)),1,1,1,1,396)
         ENDIF 
         IF(IUHF.NE.0)THEN
          I0L(2)=I0L(1)+IINTFP*IRPDPD(IRREPX,9)
         IF (ISIDE .EQ.1)THEN
            CALL GETLST(ICORE(I0L(2)),1,1,1,2,394)
         ELSE IF (ISIDE .EQ.2) THEN
            CALL GETLST(ICORE(I0L(2)),1,1,1,2,396)
         ENDIF 
         ELSE
          I0L(2)=I0L(1)
         ENDIF
     ...
     &                 'STST')
          IF (ISIDE .EQ. 1) THEN
             CALL PUTLST(ICORE(I0L(1)),1,1,1,1,394)
          ELSE IF (ISIDE .EQ.2) THEN
             CALL PUTLST(ICORE(I0L(1)),1,1,1,1,396)
          ENDIF 
     ...
     &                 POP(1,ISPIN),'TSTS')
          IF (ISIDE .EQ. 1) THEN
             CALL PUTLST(ICORE(I0L(ISPIN)),1,1,1,ISPIN,394)
          ELSE IF (ISIDE .EQ.2) THEN 
             CALL PUTLST(ICORE(I0L(ISPIN)),1,1,1,ISPIN,396)
          ENDIF 
     ...
     &                 'STST')
          IF (ISIDE .EQ. 1) THEN
             CALL PUTLST(ICORE(I0L(1)),1,1,1,1,394)
             CALL PUTLST(ICORE(I0L(2)),1,1,1,2,394)
          ELSE IF (ISIDE .EQ.2) THEN
             CALL PUTLST(ICORE(I0L(1)),1,1,1,1,396)
             CALL PUTLST(ICORE(I0L(2)),1,1,1,2,396)
          ENDIF 
         ENDIF
        ENDIF
100    CONTINUE
