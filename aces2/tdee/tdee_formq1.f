      ...
      SUBROUTINE TDEE_FORMQ1(IRREPX,ICORE,MAXCOR,IUHF,ISIDE)
C
...
C
      ...
      MXCOR=MAXCOR-I000+1
      IF (ISIDE .EQ. 1) THEN
         CALL GETLST(ICORE(I0CA),1,1,1,1,390)
      ELSEIF (ISIDE .EQ. 2) THEN
         CALL GETLST(ICORE(I0CA),1,1,1,1,392)
      ENDIF
      IF(IUHF.NE.0)THEN
       I0CB=I0CA+IINTFP*LENCA
       IF (ISIDE .EQ. 1) THEN
          CALL GETLST(ICORE(I0CB),1,1,1,2,390)
       ELSEIF (ISIDE .EQ. 2) THEN
          CALL GETLST(ICORE(I0CB),1,1,1,2,392)
       ENDIF 
      ENDIF
C
...
C
     ...
     &             MXCOR,IUHF,7)
C
      CALL PUTLST(ICORE(I0QABA),1,1,1,1,382)
      CALL PUTLST(ICORE(I0QIJA),1,1,1,1,381)
      IF(IUHF.NE.0)THEN
       I0QABB=I0QABA+IINTFP*LENQABA
       I0QIJB=I0QIJA+IINTFP*LENQIJA
        CALL PUTLST(ICORE(I0QABB),1,1,1,2,382)
        CALL PUTLST(ICORE(I0QIJB),1,1,1,2,381)
      ENDIF
C
      RETURN
      END
