      SUBROUTINE TPDABCI5(ICORE,MAXCOR,IUHF,LISTL2,LISTR1,LISTR1OFF)
...
C
c      CALL GFORMG(IRREPX,1,444,44,400,ICORE,MAXCOR,2,ONEM,IUHF)
      CALL GFORMG(IRREPX,1,LISTL2,44,400,ICORE,MAXCOR,2,ONEM,IUHF)
C
...
C
c      CALL GETLST(ICORE(ISTART),1,1,1,3,490)
      CALL GETLST(ICORE(ISTART),1,1,1,LISTR1OFF+1,LISTR1)
      CALL SYMTRA(IRREPX,VRT(1,1),POP(1,1),1,ICORE(ISTART),
     &            ICORE(I0R(1)))
      CALL GETLST(ICORE(I0G(1)),1,1,1,3,492)
      IF(IUHF.EQ.1) THEN
c       CALL GETLST(ICORE(ISTART),1,1,1,4,490)
       CALL GETLST(ICORE(ISTART),1,1,1,LISTR1OFF+2,LISTR1)
