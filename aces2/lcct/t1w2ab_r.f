      SUBROUTINE T1W2AB_R(Z,W,TA,TB,FA,FB,POP1,POP2,VRT1,VRT2,
...
C
c       CALL GETLST(W,1,NUMSYZ,1,IRREP,LISTZ)
c       IF(IUHF.EQ.0) THEN
c        CALL SYMRHF(IRREP,POP1,POP1,DISSYZ,Z,TMP,TMP(1+DISSYZ),
c     &              TMP(1+2*DISSYZ))
c       ENDIF
c       CALL VADD(W,W,Z,NUMSYZ*DISSYZ,ONE)
c       CALL PUTLST(W,1,NUMSYZ,1,IRREP,LISTZ)
      ENDIF
      IF(IUHF.EQ.1) THEN
C
