      SUBROUTINE L2INL1(ICORE,MAXCOR,IUHF)
C
...
C
       ...
       FACT=ONE
       CALL SAXPY(NWAA,FACT,ICORE,1,ICORE(ITAA),1)
C
C  FOR UHF ONLY, ADD F(a,i) TO Z(a,i)
C
       IF(IUHF.EQ.1) THEN
        CALL GETLST(ICORE,1,1,1,2,93)
        CALL SAXPY(NWBB,FACT,ICORE,1,ICORE(ITBB),1)
       ENDIF
      ENDIF
...
c      if (.false.) then
        ...
        CALL GETLST(ICORE(1),1,1,1,3,93)
        FACT=ONE
        CALL SAXPY(NWAA,FACT,ICORE,1,ICORE(ITAA),1)
C
C  FOR UHF ONLY, ADD F(a,i) TO Z(a,i)
C
        ...
        call brueck_effh(icore,mxcor,iuhf)

        LSTOFF=200
C Add new hermitian contributions that only depend on T2 amps
C  WITHIN THE SPIN ADAPTED RHF CODE NO CALL TO T2T1AA2 IS NECESSARY
C
      IF(IUHF.EQ.1) THEN
        CALL T2T1AA2(ICORE(ITAA),ICORE,MXCOR,POP1,VRT1,1,.FALSE.,LSTOFF)
C
        CALL T2T1AA2(ICORE(ITBB),ICORE,MXCOR,POP2,VRT2,2,.FALSE.,LSTOFF)
      ENDIF
C
      CALL T2T1AB2(ICORE(ITAA),ICORE,MXCOR,POP1,POP2,VRT1,VRT2,1,
     &             IUHF,.FALSE.,LSTOFF)
...
C
      IF(IUHF.EQ.1) THEN
       CALL T2T1AA1(ICORE(ITAA),ICORE,MXCOR,POP1,VRT1,1,.FALSE.,LSTOFF)
C
       CALL T2T1AA1(ICORE(ITBB),ICORE,MXCOR,POP2,VRT2,2,.FALSE.,LSTOFF)
      ENDIF
C
      CALL T2T1AB1(ICORE(ITAA),ICORE,MXCOR,POP1,POP2,VRT1,VRT2,1,
     &             IUHF,.FALSE.,LSTOFF)
...
C
      CALL NEWL1(ICORE(ITAA),ICORE,MXCOR,1)
      CALL PUTLST(ICORE(ITAA),1,1,1,3,90)
      IF(IUHF.NE.0) THEN
       CALL NEWL1(ICORE(ITBB),ICORE,MXCOR,2)
