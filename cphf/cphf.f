...
C THIS PROGRAM DRIVES THE SOLUTION OF THE CPHF EQUATIONS.
C SYMMETRY IS USED EXPLICITLY.
C
C POSSIBLE PERTURBATIONS:
C
...
C VARIOUS UPDATES, J. GAUSS, UNIVERSITY KARLSRUHE, 1992, 1993
      ...
      SUBROUTINE CPHF(ICORE,ICRSIZ)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)

      INTEGER DIRPRD,POP,VRT,XYZSYM(3),XYZSYM2(3)
      LOGICAL SCF,NONHF,GEOM,FIELD,THIRD,MAGN,ROHF,SEMI,LAST,SPIN,
     &        TRAINV,SWITCH,JSO,JFC,JSD,PMSZ,GEERTSEN,NO_ZERO_AI,
     &        NO_ZERO_BI, TERMINATE 

cYAU 20010316
c   Instead of screwing around with I0, this subroutine is now passed
c   the entire useable space of icore(*). We still need IUSE as a
c   replacement for I0 since some programmers thought it necessary to
c   start remapping things without planning ahead.
      INTEGER IUSE, ICRSIZ
...
C
      IF (TRAINV) THEN
C      o READ INFORMATION ABOUT TRANSLATIONAL INVARIANCE RELATIONS
         ITRANS = 1
         IMAP   = ITRANS + 3*NUCIND-3 + MOD(3*(NUCIND-1),2)
         IUSE   = IMAP   + NTPERT     + MOD(NTPERT,2)
         MXCOR  = MXCOR  + 3-3*NUCIND - MOD(3*(NUCIND-1),2)
     &                   - NTPERT     - MOD(NTPERT,2)
         CALL GETREC(20,'JOBARC','TRANSINV',3*(NUCIND-1),ICORE(ITRANS))
         ICOUNT = 0
         IOFF = 0
         DO IRREP = 1, NIRREP
            N = NPERT(IRREP)
...
C
      ...
      DO IRREPX = 1, NIRREP
         LENAI = IRPDPD(IRREPX,ISYTYP(1,18))
         IF (LENAI .NE. 0) NO_ZERO_AI = .TRUE.
         IF (IUHF .NE. 0) THEN
             LENBI = IRPDPD(IRREPX,ISYTYP(1,17))
             IF (LENBI .NE. 0)  NO_ZERO_BI = .TRUE.
         ENDIF
      END DO
      IF (IUHF .EQ. 0 .AND. .NOT. NO_ZERO_AI) TERMINATE = .TRUE.
      IF (IUHF .EQ. 1 .AND. .NOT. (NO_ZERO_AI .AND.
     &    NO_ZERO_BI)) TERMINATE = .TRUE.
       
       IF (TERMINATE) THEN
          Write(6,*)
          Write(6,"(a,a)")" There are no possible single excitations",
     &                    ": Increase the basis set size and retry!"
          Write(6,*)
          Call Errex
       ENDIF 
...
C
       ...
       MXCOR=MXCOR-IINTFP*(IUHF*LENGAA+LENGBB)
...
C CALCULATE STATIC SCF HYPERPOLARIZABILITY AND PRINT RESULTS
      IF(GEOM.OR.FIELD) THEN
       CALL SCFHYP(ICORE(ISTART),MXCOR/IINTFP,XYZSYM)
       CALL POLSUM(SCF)
      ENDIF

C FOR ROHF COMPLETE HERE CALCULATION OF FOCK MATRIX
C DERIVATIVES (ONLY FOR CORRELATED CALCULATIONS)
      IF(ROHF.AND.(.NOT.SCF)) CALL ROHFDF(ICORE(IUSE),MAXCOR)
