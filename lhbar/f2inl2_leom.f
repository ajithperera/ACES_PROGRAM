      ...
      SUBROUTINE F2INL2_LEOM(ICORE,MAXCOR,IUHF)
C
...
C
C The singles contribution with standard F(MI) intermediate.
C
      NFAA=NF1AA
      NFBB=NF1BB
...
C
     ...
     &                        NOCC)
      ...
      ENDIF
C
C The doubles contribution is with the scalled F(MI) intermediate. 
C
...
C    IN CORE VERSION
C
          CALL F2L2AA_LEOM(ICORE(I001),ICORE(I002),ICORE(I000),
...
C       IN CORE ALGORITHM
C
         CALL F2L2AB_LEOM(ICORE(I001),ICORE(I002),ICORE(I0AA),
     &               ICORE(I0BB),
