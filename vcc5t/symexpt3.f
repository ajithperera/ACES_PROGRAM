      SUBROUTINE SYMEXPT3(T3,W,IADW,ISPIN,IRPIJK)
...
C
      INDEX(I) = I*(I-1)/2
C
C     SUBROUTINE THAT IS SUPPOSED TO GENERATE W SUCH THAT
C
C     W(A<B,C) =  T3(A<B<C)
C     W(A<C,B) = -T3(A<B<C)
C     W(B<C,A) =  T3(A<B<C)
...
C
      W(IADW(IRPC) + (C-1)*IRPDPD(IRPAB,ISPIN)
     1             + IOFFVV(IRPB,IRPAB,ISPIN) 
     1             + (B-1)*VRT(IRPA,ISPIN)
     1             + A - 1) =  T3(ABC)
C
      W(IADW(IRPB) + (B-1)*IRPDPD(IRPAC,ISPIN)
     1             + IOFFVV(IRPC,IRPAC,ISPIN) 
     1             + (C-1)*VRT(IRPA,ISPIN)
     1             + A - 1) = -T3(ABC)
C
      W(IADW(IRPA) + (A-1)*IRPDPD(IRPBC,ISPIN)
