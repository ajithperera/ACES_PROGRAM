      SUBROUTINE S1S223N(S1A,S1B,
     1                   CORE,T3D,W,IADT3,IADW,ISPIN1,ISPIN2,I,J,K,
     1                  IRPI,IRPJ,IRPK,IRPIJ,IRPIK,IRPJK,IRPIJK,IUHF,
     1                  IMODE)
      IMPLICIT INTEGER (A-Z)
      DOUBLE PRECISION S1A(1),S1B(1),CORE(1),T3D(1),W(1)
...
C
      ...
      IF(IMODE.EQ.3)THEN
c YAU : old
c      IF(ISPIN1.EQ.1) CALL ICOPY(IINTFP*NS1B,S1A,1,CORE(I030),1)
c      IF(ISPIN1.EQ.2) CALL ICOPY(IINTFP*NS1B,S1B,1,CORE(I030),1)
c YAU : new
       IF(ISPIN1.EQ.1) CALL DCOPY(NS1B,S1A,1,CORE(I030),1)
       IF(ISPIN1.EQ.2) CALL DCOPY(NS1B,S1B,1,CORE(I030),1)
c YAU : end
      ENDIF
C
      IRPABC = IRPIJK
...
C
      IF(IMODE.EQ.3)THEN
c YAU : old
c      IF(ISPIN1.EQ.1) CALL ICOPY(IINTFP*NS1A,S1B,1,CORE(I030),1)
c      IF(ISPIN1.EQ.2) CALL ICOPY(IINTFP*NS1A,S1A,1,CORE(I030),1)
c YAU : new
       IF(ISPIN1.EQ.1) CALL DCOPY(NS1A,S1B,1,CORE(I030),1)
       IF(ISPIN1.EQ.2) CALL DCOPY(NS1A,S1A,1,CORE(I030),1)
c YAU : end
      ENDIF
C
