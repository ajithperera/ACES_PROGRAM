C     Local copy of vcc5t/tspabci2.f, renamed to avoid collision with the
C     differently-shaped TSPABCI2 in trp/librt3/lambda (4 and 5-arg
C     versions). Same rationale as tspabci_lcct.f in this directory --
C     lcct is not linked against vcc5t, so without this local copy the
C     linker would resolve TSPABCI2 from trp's 4-arg version instead,
C     which ltrps.f's 3-argument calls would feed garbage into.
      SUBROUTINE TSPABCI2(CORE,MAXCOR,ICODE)
