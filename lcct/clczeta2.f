      SUBROUTINE CLCZETA2(SCR,MAXCOR,IUHF,ASMALL,KMAX,NSIZEC,
     &                    ICUT,ITMAX,LISTZ1,LISTZ1OFF,LISTZ2)
C
...
CEND
      IMPLICIT NONE
C Common blocks
      integer ndimr,ioldest
      COMMON/EXTINF/NDIMR,IOLDEST
      integer IINTLN,IFLTLN,IINTFP,IALONE,IBITWD
      COMMON/MACHSP/IINTLN,IFLTLN,IINTFP,IALONE,IBITWD
C Input variables
      integer iuhf,maxcor,kmax,nsizec,icut,itmax,listz1,listz1off,listz2
C Pre-allocated local variables
      double precision scr(maxcor),asmall(kmax*kmax)
C External functions
      integer idamax
      double precision ddot,dnrm2
      external ddot,dnrm2,idamax
C Local variables
      integer i,j,n,i000,i010,i020,i030,ioff,k,niter,indxf
      double precision onem,cutoff,emaxdiis,emaxjac,errdiis,errjacob
      DATA ONEM /-1.0D0/
      INDXF(I,J,N)=I+(J-1)*N
C
      CUTOFF=10.0D0**(-ICUT)
...
C
      ...
      I030=I020+NSIZEC
      IF (I030 .GT. MAXCOR) CALL INSMEM('CLCZETA2',i030,MAXCOR)
C
...
C
      ...
      CALL DAXPY (NSIZEC,ONEM,SCR(I010),1,SCR(I000),1)
      ERRJACOB=DNRM2(NSIZEC,SCR(I000),1)
      EMAXJAC=ABS(SCR(IDAMAX(NSIZEC,SCR,1)))
...
C
      DO 110 I=1,NDIMR
       DO 111 J=1,NDIMR
        ...
        ASMALL(INDXF(I,J,KMAX))=DDOT(NSIZEC,SCR(I010),1,SCR(I000),1)
111    CONTINUE
110   CONTINUE
C
...
C
      ...
      DO 210 I=1,NDIMR
       ...
       CALL DAXPY (NSIZEC,SCR(IOFF),SCR(I020),1,SCR(I000),1)
       IOFF=IOFF+1
210   CONTINUE
      ERRDIIS=DNRM2(NSIZEC,SCR,1)
      EMAXDIIS=ABS(SCR(IDAMAX(NSIZEC,SCR,1)))
...
C
      IF(NITER.EQ.1)THEN
       ...
       CALL DAXPY (NSIZEC,ONEM,SCR(I000),1,SCR(I010),1)
...
C
       ...
       CALL DSCAL (NSIZEC,-0.75D0,SCR(I000),1)
       DO 220 I=1,NDIMR
        CALL GETLST(SCR(I020),I,1,1,1,470)
        CALL DAXPY (NSIZEC,SCR(IOFF),SCR(I020),1,SCR(I000),1)
        IOFF=IOFF+1
...
1002  FORMAT(T12,'Subspace',T30,'Current',T64,'DIIS',/,
     &       T1,'Iteration',T12,'Dimension',T30,'Iterate',
     &       T61,'Extrapolant')
1004  FORMAT(T24,'RMS',T39,'Max',T56,'RMS',T71,'Max')
1003  FORMAT(T3,I4,T13,I4,T20,E20.14,T42,E20.14,T66,E20.14,
     &       T88,E20.14)
