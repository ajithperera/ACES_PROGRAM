...
C
c      IF(IFLAGS(93).EQ.2.OR.ISYTYP(1,233).EQ.5)THEN
      if (.false.) then
       IF(ISIDE.EQ.1)CALL MODT2(ICORE,MAXCOR,IUHF,IRREPX,ONE)
       IF(IFLAGS(93).EQ.2)THEN
        CALL DRAOLAD(ICORE,MAXCOR,IUHF,.TRUE.,IRREPX,0,
     &               443,460,413,463)
       ELSE
        CALL DLADDER2(ICORE,MAXCOR,IUHF,IRREPX,6,LISTT2,
     &                LISTABCD,LISTT2IN,ISIDE)
       ENDIF
       IF(ISIDE.EQ.1)CALL MODT2(ICORE,MAXCOR,IUHF,IRREPX,ONEM)
