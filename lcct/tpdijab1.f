      SUBROUTINE TPDIJAB1(ICORE,MAXCOR,IUHF,LISTL1,LISTL1OFF,LISTL2NRS,
     &   LISTL2RS,LISTR1,LISTR1OFF,LISTR2NRS,LISTR2RS)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      INTEGER DIRPRD,DISSYR,DISSYL,DISSYG,DISSYR1,DISSYR2,POP,VRT
      INTEGER DISSYT,DISSYT1,DISSYT2,R1LIST,R2LIST
...
C
C FORM FIRST RL(M,E)
C SET RL Zero, because it involves fully contracted R
...
C
     ...
     &             ICORE(I000),DISSYL,ONE,ICORE(I0RL(ISPIN)),1)
          ...
          call zero(icore(i0rl(ispin)),IRPDPD(1,8+ispin))
        endif
C
3      CONTINUE
C
...
c       L2LIST=439
       L2LIST=LISTL2RS+5
C
       NUMSYL=IRPDPD(1,ISYTYP(2,L1LIST))
       DISSYL=IRPDPD(IRREPX,ISYTYP(1,L1LIST))
C
       I000=ISTART
       I010=I000+IINTFP*NUMSYL*DISSYL
       IEND=I010+IINTFP*NUMSYL*DISSYL
C
       CALL GETLST(ICORE(I000),1,NUMSYL,1,1,L1LIST)
       CALL GETLST(ICORE(I010),1,NUMSYL,1,1,L2LIST)
C
...
C
     ...
     &            ICORE(I000),DISSYL,AZERO,ICORE(I0RL(1)),1)
          ...
          call zero(icore(i0rl(1)),IRPDPD(1,9))
        endif
C
      ENDIF
C
...
C
c       R1LIST=458-ISPIN
       R1LIST=LISTR2RS+4-ISPIN
c       R2LIST=453+ISPIN
       R2LIST=LISTR2RS-1+ISPIN
       IF(IUHF.EQ.0) THEN
c        R1LIST=457
        R1LIST=LISTR2RS+3
c        R2LIST=459
       ...
       ENDIF
C
       NUMSYR1=IRPDPD(IRREPX,ISYTYP(2,R1LIST))
       DISSYR1=IRPDPD(1,ISYTYP(1,R1LIST))
       NUMSYR2=IRPDPD(IRREPX,ISYTYP(2,R2LIST))
       DISSYR2=IRPDPD(1,ISYTYP(1,R2LIST))
C
C ALLOCATE MEMORY
C
       I000=ISTART
...
C GET R AMPLITUDES
C
       CALL GETLST(ICORE(I000),1,NUMSYR1,1,IRREPX,R1LIST)
C
C SPIN ADAPT FOR RHF
C
       IF(IUHF.EQ.0) THEN
        CALL SSCAL(NUMSYR1*DISSYR1,TWO,ICORE(I000),1)
        CALL GETLST(ICORE(I010),1,NUMSYR2,1,IRREPX,R2LIST)
...
C
        CALL GETLST(ICORE(I000),1,NUMSYR2,1,IRREPX,R2LIST)
C
...
C
c         LISTL=443+ISPIN
         LISTL=LISTL2NRS-1+ISPIN
C         LISTR=460+ISPIN
         LISTR=LISTR2NRS-1+ISPIN
C
