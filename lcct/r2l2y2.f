      SUBROUTINE R2L2Y2(ICORE,MAXCOR,IUHF,LISTL2,LISTL2RS,LISTR2,
     &   LISTR2RS)
C
...
C
      I000=1
c      IF(.NOT.MBPT2.AND.IRREPX.EQ.1)THEN
C R and W fully contracted, need open line on W
      if (.false.) then
       X=ZILCH
...
C
       DO 11 ISPIN=3,3-2*IUHF,-1
        ...
        CALL GETALL(ICORE(I000),ISIZE,1,LISTL2-1+ISPIN)
...
C
       CALL R2L2Y2A(ICORE,MAXCOR,IUHF,LISTL2RS,LISTR2RS)
       CALL R2L2Y2B(ICORE,MAXCOR,IUHF,LISTL2,LISTR2)
C
       CALL R2L2Y2C(ICORE,MAXCOR,IUHF,LISTL2,LISTR2)
C
C
C
...
C
      CALL GFORMG(1,IRREPX,14,LISTR2,400,ICORE(I000),MAXCOR,0,ONE,IUHF)
      CALL GFORMG(IRREPX,IRREPX,LISTL2,LISTR2,100,ICORE(I000),
     &            MAXCOR,0,ONE,IUHF)

       CALL R2L2Y2D(ICORE,MAXCOR,IUHF,LISTL2)
c      IOFF=1
...
c1113    CONTINUE
c
       CALL R2L2Y2E(ICORE,MAXCOR,IUHF,LISTL2)
c      IOFF=1
