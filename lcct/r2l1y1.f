      SUBROUTINE R2L1Y1(ICORE,MAXCOR,IUHF,LISTL1,LISTL1OFF,LISTR2,
     &   LISTR2RS)
C
...
C
      I0L(1)=1
      CALL GETLST(ICORE(I0L(1)),1,1,1,1+LISTL1OFF,LISTL1)
      IF(IUHF.NE.0)THEN
       I0L(2)=I0L(1)+IINTFP*IRPDPD(IRREPX,9)
       CALL GETLST(ICORE(I0L(2)),1,1,1,2+LISTL1OFF,LISTL1)
      ELSE
       I0L(2)=I0L(1)
      ENDIF
...
C W and R fully contracted, need W to have an open line
      if (.false.) then
       X=ZILCH
...
10     CONTINUE
       DO 11 ISPIN=1,1+IUHF
        CALL GETLST(ICORE(I0Y(ISPIN)),1,1,1,ISPIN+LISTL1OFF,LISTL1)
...
C
      CALL GFORMG(IRREPX,1,LISTR2,14,400,ICORE(I000),MXCOR,0,ONE,IUHF)
C
