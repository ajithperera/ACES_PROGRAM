     ...
     &                 LISTL1,LISTT1,LISTL2,LISTT2,
     &                 LZERO,ICORE,MAXCOR,IUHF,SCFCNT)
C
...
C
      DO 10 ISPIN=1,1+IUHF
       ...
       IOFFVO=(ISPIN-1)*NT(1)
       CALL GETLST(DENVO(1+IOFFVO),1,1,1,ISPIN,LISTT1)
       CALL GETLST(DENOV(1+IOFFVO),1,1,1,ISPIN,LISTL1)
...
C
      IF(SCFCNT)THEN
       IOFF0=0
       DO 120 ISPIN=1,1+IUHF
        DO 121 IRREP=1,NIRREP
         NOCC=POP(IRREP,ISPIN)
         DO 122 I=1,NOCC
          IOFF=IOFF0+I+(I-1)*NOCC
          DENOO(IOFF)=DENOO(IOFF)+ONE
122      CONTINUE
         IOFF0=IOFF0+NOCC*NOCC
121     CONTINUE
120    CONTINUE
      ENDIF
      RETURN
