...
c
 ...
 11   continue
      ...
      call engy1(no,nu,a,a(n1),a(n2),'noprint ')
 ...
 12   continue
      call dr22b(nirrep,a)
c prepare files for the third part of dgr22
      do 13 ia=1,nu
         call rpakv(36,nu,ia,a(n2),a(n3),a)
         call trant3(a,nu,3)
...
c o2:nu,no,no,nu
 ...
 10   continue
      ...
      call timer(1)
      if(print)write(6,2348)timenew
 2348 format('Diagram nr.22 required', f15.2,'  seconds')
      return
      end
 ...
 1000 continue
     ...
     *icllvl.eq.21.or.icllvl.eq.22
      do 1000 irp=1,nirrep
         ...
         ntotsiz=num233*dis233
         if(maxcor.gt.3*ntotsiz)then
            if(print)write(6,*)'fulmat2 selected'
            nt1=ntotsiz+1
            nt2=ntotsiz+nt1
            call fulmat2(irp,num233,num233,dis233,ntotsiz,nt1,nt2,a)
         else
            if (maxcor.gt.ntotsiz+2*no2u2)then
               if(print)write(6,*)'partmatb selected'
               nt1=ntotsiz+1
               nt2=dis233+nt1
 ...
 1000 continue
     ...
     *icllvl.eq.21.or.icllvl.eq.22
      do 1000 irp=1,nirrep
         ...
         dis233=irpdpd(irp,isytyp(1,233))
         if(smlv)dis233=num233
         if(print)write(6,*)'num233,dis233:',num233,dis233
         ntotsiz=num233*dis233
         if(maxcor.gt.3*ntotsiz)then
            if(print)write(6,*)'fulmat3 selected'
            nt1=ntotsiz+1
            nt2=ntotsiz+nt1
            call fulmat3(irp,num233,num233,dis233,ntotsiz,nt1,nt2,a)
         else
            if (maxcor.gt.ntotsiz+2*no2u2)then
               if(print)write(6,*)'partmatc selected'
               nt1=ntotsiz+1
               nt2=dis233+nt1
