...
C  in the file COPYRIGHT.
         SUBROUTINE  OED__PVP_PCGTO_BLOCK
     +
     +                    ( NBATCH,
...
C
     ...
     +
     +                    ( SHELLP+XCOMP+XCOMP,
     +                      MIN (SHELLA+XCOMP,SHELLB+XCOMP),
     +                      SHELLA+XCOMP,SHELLB+XCOMP,
     +                      MGIJCEN,
     +                      WTS,
     +                      R1X,R2,
     +                      ABX,
...
C
     ...
     +
     +                    ( SHELLP+YCOMP+YCOMP,
     +                      MIN (SHELLA+YCOMP,SHELLB+YCOMP),
     +                      SHELLA+YCOMP,SHELLB+YCOMP,
     +                      MGIJCEN,
     +                      WTS,
     +                      R1Y,R2,
     +                      ABY,
...
C
C
             NINT = MGIJCEN * (SHELLA+YCOMP+1) * (SHELLB+YCOMP+1)
             DO M = 1,NINT
                BATCH (M) = INT1DY (M)
             END DO
     ...
     +
     +
C             WRITE (*,*) "NAI Y integrals A,B"
C             CALL  OED__PRINT_2IND_BATCH
C     +                  (MGIJCEN,SHELLA,SHELLB,6,INT1DY)
C     +
         END IF
C
C
...
C
     ...
     +
     +                    ( SHELLP+ZCOMP+ZCOMP,
     +                      MIN (SHELLA+ZCOMP,SHELLB+ZCOMP),
     +                      SHELLA+ZCOMP,SHELLB+ZCOMP,
     +                      MGIJCEN,
     +                      WTS,
     +                      R1Z,R2,
     +                      ABZ,
     ...
     +
     +
C             WRITE (*,*) "NAI Z integrals A+1,B+1"
C             CALL  OED__PRINT_2IND_BATCH
C     +                  (MGIJCEN,SHELLA+1,SHELLB+1,6,INT1DZ)
C     +
C
C
C             ...form the pVp integrals!
C
C
             NINT = MGIJCEN * (SHELLA+ZCOMP+1) * (SHELLB+ZCOMP+1)
             DO M = 1,NINT
                BATCH (M) = INT1DZ (M)
             END DO
     ...
     +
     +
C             WRITE (*,*) "NAI Z integrals A,B"
C             CALL  OED__PRINT_2IND_BATCH
C     +                  (MGIJCEN,SHELLA,SHELLB,6,INT1DZ)
C     +
         END IF
C
C
