      ...
      subroutine compute_4cent_dshield_integrals
...
c end PV
     ...
     *                 charge(*)
      double precision out_4dsh_xx(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_xy(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_xz(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_yx(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_yy(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_yz(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_zx(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_zy(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4dsh_zz(a1:a2,b1:b2,c1:c2,d1:d2),
     *                 out_4eri(a1:a2,b1:b2,c1:c2,d1:d2),
...
c----------------------------------------------------------------------------
      ...
      Write(6,*)
      write(6,*) "Printing from compute_4cent_dshield_integrals"
...
C
      ...
      do a = 1, Nbasis
         do b = 1, Nbasis
            do c = 1, Nbasis 
               do d = 1, Nbasis 
                  out_4dsh_xx(a,b,c,d) = 0.0d0
                  out_4dsh_xy(a,b,c,d) = 0.0d0
                  out_4dsh_xz(a,b,c,d) = 0.0d0
                  out_4dsh_yx(a,b,c,d) = 0.0d0
                  out_4dsh_yy(a,b,c,d) = 0.0d0
                  out_4dsh_yz(a,b,c,d) = 0.0d0
                  out_4dsh_zx(a,b,c,d) = 0.0d0
                  out_4dsh_zy(a,b,c,d) = 0.0d0
                  out_4dsh_zz(a,b,c,d) = 0.0d0
                  out_4eri(a,b,c,d)    = 0.0d0
               enddo
            enddo 
         end do
      end do
...
C
CSSS      if (m .eq.1 .and. n .eq. 1 .and. r .eq. 2 .and.s.eq.1) then
CSSS           Write(6,*)  "Entering ERD__GENER_DSHIELD_BATCH"
     ...
     *                nfirst, scr)
           ...
           if (nints .gt. 0) then
              ...
              if (XX .eq. 1) call add_4cent_integrals(out_4dsh_XX, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  
              if (XY .eq. 1) call add_4cent_integrals(out_4dsh_XY, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  
              if (XZ .eq. 1) call add_4cent_integrals(out_4dsh_XZ, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (YX .eq. 1) call add_4cent_integrals(out_4dsh_YX, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (YY .eq. 1) call add_4cent_integrals(out_4dsh_YY, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (YZ .eq. 1) call add_4cent_integrals(out_4dsh_Yz, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (ZX .eq. 1) call add_4cent_integrals(out_4dsh_ZX, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (ZY .eq. 1) call add_4cent_integrals(out_4dsh_ZY, 
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)  

              if (ZZ .eq. 1) call add_4cent_integrals(out_4dsh_ZZ,
     *                           a1,a2,b1,b2,c1,
     *                           c2,d1,d2,scr(nfirst),
     *                           aa1,aa2,bb1,bb2,cc1,cc2,dd1,dd2,-1.0D0,
     *                           0)   
...
CSSS          Call rescale_ints(out_4dsh_ZZ, ACES_4DSH, Nbasis,
CSSS     &                      ERD_index,
CSSS     &                      ERD_scale)
CSSS
CSSS         if (d .le. c .and. b .le. a) then
CSSS          Write(6, "(4(1x,i3),1x,F15.10))")
Ca,b,c,d,out_4dsh_XX(a,b,c,d)
...
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_XX(a,b,c,d), a=1, nbasis)
      xx_s = xx_s + Out_4dsh_XX(a,b,c,d) * Out_4dsh_XX(a,b,c,d) 
      xx_l = xx_l +  Out_4dsh_XX(a,b,c,d) 
CSSS      Write(6, "(a,4(1x,i3),1x,F15.10))") "Ajith",a,b,c,d,
CSSS     &                                     ACES_4DSH(a,b,c,d)
C      write(*,*)

CSSS      Write(*,*) "XY integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_XY(a,b,c,d), a=1, nbasis)
      xy_s = xy_s + Out_4dsh_XY(a,b,c,d) * Out_4dsh_XY(a,b,c,d) 
      xy_l = xy_l + Out_4dsh_XY(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "XZ integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_XZ(a,b,c,d), a=1, nbasis)
      xz_s = xz_s + Out_4dsh_XZ(a,b,c,d) * Out_4dsh_XZ(a,b,c,d) 
      xz_l = xz_l + Out_4dsh_XZ(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "YX integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_YX(a,b,c,d), a=1, nbasis)
      yx_s = yx_s + Out_4dsh_YX(a,b,c,d) * Out_4dsh_YX(a,b,c,d) 
      yx_l = yx_l + Out_4dsh_YX(a,b,c,d) 
CSSS      Write(6, "(a,4(1x,i3),1x,F15.10))") "Ajith",a,b,c,d,
CSSS     &                                     ACES_4DSH(a,b,c,d)
...
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_YY(a,b,c,d), a=1, nbasis)
      yy_s = yy_s + Out_4dsh_YY(a,b,c,d) * Out_4dsh_YY(a,b,c,d) 
      yy_l = yy_l + Out_4dsh_YY(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "YZ integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_YZ(a,b,c,d), a=1, nbasis)
      yz_s = yz_s + Out_4dsh_YZ(a,b,c,d) * Out_4dsh_YZ(a,b,c,d) 
      yz_l = yz_l + Out_4dsh_YZ(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "ZX integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_ZX(a,b,c,d), a=1, nbasis)
      zx_s = zx_s + Out_4dsh_ZX(a,b,c,d) * Out_4dsh_ZX(a,b,c,d) 
      zx_l = zx_l + Out_4dsh_ZX(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "ZY integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_ZY(a,b,c,d), a=1, nbasis)
      zy_s = zy_s + Out_4dsh_ZY(a,b,c,d) * Out_4dsh_ZY(a,b,c,d) 
      zy_l = zy_l + Out_4dsh_ZY(a,b,c,d) 
C      write(*,*)

CSSS      Write(*,*) "ZZ integrals"
CSSS      Write(6, "(5(1x,F15.10))") (Out_4dsh_ZZ(a,b,c,d), a=1, nbasis)
      zz_s = zz_s + Out_4dsh_ZZ(a,b,c,d) * Out_4dsh_ZZ(a,b,c,d) 
      zz_l = zz_l + Out_4dsh_ZZ(a,b,c,d) 
CSSS      Write(6, "(a,4(1x,i3),1x,F15.10))") "Ajith",a,b,c,d,
CSSS     &                                     ACES_4DSH(a,b,c,d)
CSSS      endif 
      ...
      Write(*,*)
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the XX-DSHIELD",
     &                      "integrals = " ,xx_s,xx_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the XY-DSHIELD",
     &                      "integrals = " ,xy_s,xy_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the XZ-DSHIELD",
     &                      "integrals = " ,xz_s,xz_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the YX-DSHIELD",
     &                      "integrals = " ,YX_s,YX_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the YY-DSHIELD",
     &                      "integrals = " ,YY_s,YY_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the YZ-DSHIELD",
     &                      "integrals = " ,YZ_s,YZ_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the ZX-DSHIELD",
     &                      "integrals = " ,ZX_s,ZX_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the ZY-DSHIELD",
     &                      "integrals = " ,ZY_s,ZY_l
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of the ZZ-DSHIELD",
     &                      "integrals = " ,ZZ_s,ZZ_l
      Write(6,*) 
      Write(6,"(a,a,F20.10,1x,F20.10)")"Check sum of scaled DSHIELD",
     &                      "integrals = " ,s22
