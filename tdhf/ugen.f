      SUBROUTINE UGEN(U,AB,ABVEC,ABVEC1,IVRT,IOCC,IVRTS,IOCCS,DEN,FP,F
     X ,EVAL,EVEC,ENRG,UPDATE,ASMALL,ASQUARE,ASCALE,ICONV,IA,HMO,E,
     X MCOMP,XX,IX,NIREP,NSIZ1,NSIZ3,NSIZO,NSIZVO,NINTMX)
     ...
     X ,AB(1),ABVEC(NSIZVO,MCOMP),ABVEC1(NSIZVO,MCOMP)
     X ,E(NSIZO,NSIZO,MCOMP)
...
C ........................  (xx,yy,zz,xy,yz,zx,yx,zy,xz) ......
     ...
     X                       L2/1,2,3,2,3,1,1,2,3/
      WRITE(6,*) ' now in UGEN , MCOMP = ',MCOMP
...
C     write(6,*) ' mcoms,nvosym ',MCOMS,NVOSYM
      DO 1000 I=1,NVOSYM
      IF(NITER.NE.0) THEN        
      ABVEC(I,MCOM)=-U(IVRTS(I,MCOMS),IOCCS(I,MCOMS),MCOM)
C     write(6,*) I,IVRTS(I,MCOMS),IOCCS(I,MCOMS),ABVEC(I,MCOM)
 ...
 1000 CONTINUE
   29 CONTINUE
C     write(6,*) 'NSIZVO,NSIZIV= ',NSIZVO,NSIZIV
C     NI1 = NSIZMO + 1
C     NI2 = NSIZMO + MCOMP + 1
      IF(NITER.NE.0) THEN        
...
C     write(6,*) ' mcoms,nvosym ',MCOMS,NVOSYM
      DO 91 I=1,NVOSYM
      ABVEC(I,MCOM)= ABVEC(I,MCOM)/(EVAL(IVRTS(I,MCOMS))
     X -EVAL(IOCCS(I,MCOMS)))
C     write(6,*) I,IVRTS(I,MCOMS),IOCCS(I,MCOMS),ABVEC(I,MCOM)
   91 CONTINUE
      CALL TIMER(0)
      CALL LINES1(IVRT,IOCC,IVRTS,IOCCS,MCOMP,3,AB,ABVEC,
...
C    X NUMSCF,NOC,NIREP,FP,DEN,CONV,NVO,KMAX)
     X NUMSCF,NOC,NIREP,FP,DEN,CONV,NVO,KMAX,XX,IX,IA,NINTMX)
      CALL TIMER(1)
C     write(6,*) ' Timing for TLINEQ1 for MCOMP = ',MCOMP,TIMENEW
      DO 37 MCOM=1,MCOMP
      MCOMS=IPRSYM(MCOM+3)
c YAU : old
c     CALL ICOPY(IINTFP*NVOS(MCOMS),ABVEC(1,MCOM),1,ABVEC1(1,MCOM),1)
c YAU : new
      CALL DCOPY(NVOS(MCOMS),ABVEC(1,MCOM),1,ABVEC1(1,MCOM),1)
c YAU : end
   37 CONTINUE
      ELSE
      DO 38 MCOM=1,MCOMP
      MCOMS=IPRSYM(MCOM+3)
      CALL MXM(AB(NIJS2(MCOMS)+1),NVOS(MCOMS),ABVEC(1,MCOM),
     X NVOS(MCOMS),ABVEC1(1,MCOM),1)
   38 CONTINUE
C     CALL MATMUL(AB,ABVEC,ABVEC1,NSIZVO,1,NSIZVO,0,0)
      END IF
      ELSE
      ITER =0
...
C     N2=NSIZMO+MCOMP+MCOM
   ...
   16 U(IVT,IOC,MCOM)=ZERO
      ...
      IVT=IVRTS(I,MCOMS)
      IOC=IOCCS(I,MCOMS)
      U(IVT,IOC,MCOM)=ABVEC1(I,MCOM)
      IJD=(IVT-1)*IVT/2+IOC
      DEN(IJD)=TWO*U(IVT,IOC,MCOM)
C     write(6,*) I,IVT,IOC,ABVEC(I,MCOM),ABVEC1(I,MCOM)
   20 CONTINUE
      DO 18 M=1,3
      ENRGS= ZERO
      DO 19 I=1,NVO
...
C     WRITE(6,*) ' - Beta = ',(ENRG(M,MCOM),M=1,3)
  ...
  200 FORMAT(1H ,3A1,' = ',F20.7,'  ',3A1,' = ',F20.7,'  ',
     X3A1,' = ',F20.7)
      ...
      ELSE
      CALL DENSP(NBASIS,NVO,IVRT,IOCC,EVEC,ABVEC1(1,MCOM),DEN)
      IF(IINDO.EQ.0) THEN
