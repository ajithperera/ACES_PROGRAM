










      Subroutine Form_qij2(Qij_aa,Qij_bb,W_aa,W_bb,W_ab,T2aa,T2bb,
     +                    T2ab,Nocc_a,Nocc_b,Nvrt_a,Nvrt_b)

      Implicit Double Precision (A-H,O-Z)

      Dimension T2aa(Nvrt_a,Nvrt_a,Nocc_a,Nocc_a)
      Dimension T2bb(Nvrt_b,Nvrt_b,Nocc_b,Nocc_b)
      Dimension T2ab(Nvrt_a,Nvrt_B,Nocc_a,Nocc_b) 
     
      Dimension W_aa(Nocc_a,Nocc_a,Nvrt_a,Nvrt_b) 
      Dimension W_bb(Nocc_b,Nocc_b,Nvrt_b,Nvrt_b) 
      Dimension W_ab(Nocc_a,Nocc_b,Nvrt_a,Nvrt_b) 

      Dimension Qij_aa(Nocc_a,Nocc_a)
      Dimension Qij_bb(Nocc_b,Nocc_b)

      Integer M,N,E,F,I

c maxbasfn.par : begin

c MAXBASFN := the maximum number of (Cartesian) basis functions

c This parameter is the same as MXCBF. Do NOT change this without changing
c mxcbf.par as well.

      INTEGER MAXBASFN
      PARAMETER (MAXBASFN=1000)
c maxbasfn.par : end

      Integer cc_maxcyc
      Integer Act_min_a,Act_min_b,Act_max_a,Act_max_b
      Integer Lineq_mxcyc
      Logical Ring_cc,Brueck,Active_space,Regular
      Double Precision ocn_oa,Ocn_ob,Ocn_va,Ocn_vb
      Double Precision Denom_tol,Brueck_tol,Lineq_tol
      Double Precision Rfac
      Dimension E_corr(0:500)

      Common /ccsdlight_vars/Ring_cc,Brueck,cc_conv,cc_maxcyc,
     +                       ocn_oa(Maxbasfn),ocn_ob(Maxbasfn),
     +                       ocn_va(Maxbasfn),ocn_vb(Maxbasfn),
     +                       E_corr,Denom_tol,Brueck_tol,Lineq_tol,
     +                       Act_min_a,Act_min_b,Act_max_a,
     +                       Act_max_b,Active_space,Lineq_mxcyc,
     +                       Regular,Rfac
     +                       




C Qij_aa(M,I)= 1/2 T2(EF,IN)*W2(MN,EF) + T2(Ef,In)*W(Mn,Ef)
      
      Do I = 1, Nocc_a
      Do M = 1, Nocc_a
         T = 0.0D0 
      Do E = 1, Nvrt_a
      Do F = 1, Nvrt_a
      Do N = 1, Nocc_a
         T = T + 0.50D0*T2aa(E,F,I,N)*W_aa(M,N,E,F)
      Enddo
      Enddo
      Enddo 
      Qij_aa(M,I) = T 
      Enddo 
      Enddo 
 
      Do I = 1, Nocc_a
      Do M = 1, Nocc_a
         T = 0.0D0 
      Do E = 1, Nvrt_a
      Do f = 1, Nvrt_b
      Do n = 1, Nocc_b
         T = T + T2ab(E,f,I,n)*W_ab(M,n,E,f)
      Enddo
      Enddo
      Enddo 
      Qij_aa(M,I) = Qij_aa(M,I) + T
      Enddo 
      Enddo 

C Qij_bb(m,i)= 1/2 T2(ef,in)*W2(mn,ef) + T2(eF,iN)*W(mN,eF)

      Do i = 1, Nocc_b
      Do m = 1, Nocc_b
         T = 0.0D0
      Do e = 1, Nvrt_b
      Do f = 1, Nvrt_b
      Do n = 1, Nocc_b
         T = T + 0.50D0*T2bb(e,f,i,n)*W_bb(m,n,e,f)
      Enddo
      Enddo
      Enddo
      Qij_bb(m,i) = T 
      Enddo
      Enddo 

      Do i = 1, Nocc_b
      Do m = 1, Nocc_b
         T = 0.0D0
      Do e = 1, Nvrt_b
      Do F = 1, Nvrt_a
      Do N = 1, Nocc_a
         T = T + T2ab(F,e,N,i)*W_ab(N,m,F,e)
      Enddo
      Enddo
      Enddo
      Qij_bb(m,i) = Qij_bb(m,i) + T 
      Enddo
      Enddo 

      call checksum("Qij_aa    :",Qij_aa,Nocc_a*Nocc_a)
      call checksum("Qij_bb    :",Qij_bb,Nocc_b*Nocc_b)

      Return 
      End 
