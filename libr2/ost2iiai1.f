      SUBROUTINE OST2IIAI1(ICORE,MAXCOR,IUHF,W,LAMBDA,WL)
C
C
C     THIS ROUTINE CALCULATE INACTIVE,INACTIVE-ACTIVE,INACTIVE T2 CONTRIBUTION
C
C     T2(IJ,NA) <-- T2(AI,NJ)=-W * P(i,j) T2(iM,aN)*T1(m,j)
C     T2(ij,ma) <-- T2(ai,mj)=-W * P(I,J) T2(In,Am)*T1(N,J)
C     T2(Ji,Am) <-- T2(Ai,mJ)=-W * T2(In,Na)*T1(m,j)
C     T2(Ij,Na) <-- T2(Nj,aI)=-W * T2(Mi,Am)*T1(N,J)
C     T2(Ji,Am) <-- T2(AJ,mi)=-W * T2(jn,ma)*T1(N,I)
C     T2(Ij,Na) <-- T2(NI,aj)=-W * T2(MJ,AN)*T1(m,i)
C
CEND
C
...
C     AAAA   SPINCASE
C
      CALL OST2I1(ICORE,MAXCOR,IRREPVA,IRREPVA,IRREPOA,IRREPVB,90,2,46,
     $   POPI(1,2),POPI(1,2),VRTI(1,2),'AIAI','IIAI',
     $   40,ONEM*W,ZILCH,ZILCH,IEX)
      IF (IEX.EQ.0) GOTO 10
C
...
C     BBBB   SPINCASE
C
      CALL OST2I1(ICORE,MAXCOR,IRREPVB,IRREPVB,IRREPOB,IRREPVA,90,1,46,
     $   POPI(1,1),POPI(1,1),VRTI(1,1),'IAIA','IIAI',
     $   41,ONEM*W,ZILCH,ZILCH,IEX)
      IF(IEX.EQ.0) GOTO 20
C
C     RESORT AND ANTISYMMETRIZE T2 INCREMENT, AND THEN AUGMENT THE 
C     T2 INCREMENT LIST. 
...
C
      CALL OST2I1(ICORE,MAXCOR,IRREPVB,IRREPVA,IRREPOB,IRREPVB,90,2,46,
     $   POPI(1,1),POPI(1,2),VRTI(1,2),'AIIA','IIAI',
     $   43,ONEM*W,ZILCH,ZILCH,IEX1)
      IF(IEX1.NE.0) THEN
         FACT=ONE
      ELSE
         FACT=ZILCH
      ENDIF
      CALL OST2I1(ICORE,MAXCOR,IRREPVA,IRREPVB,IRREPOA,IRREPVA,90,1,
     $   46,POPI(1,2),POPI(1,1),VRTI(1,1),'IAAI','AIII',
     $   43,ONEM*W,FACT,ZILCH,IEX2)
C     
C
C     ABAB   SPINCASE LAST TWO TERMS
C
      CALL OST2I1(ICORE,MAXCOR,IRREPVB,IRREPVB,IRREPOB,IRREPVA,90,1,45,
     $   POPI(1,2),POPI(1,1),VRTI(1,2),'AIIA','IIAI',
     $   42,ONEM*W,ZILCH,ZILCH,IEX3)
      IF(IEX3.NE.0) THEN
         FACT=ONE
      ELSE
         FACT=ZILCH
      ENDIF
      CALL OST2I1(ICORE,MAXCOR,IRREPVA,IRREPVA,IRREPOA,IRREPVB,90,2,44,
     $   POPI(1,1),POPI(1,2),VRTI(1,1),'IAAI','AIII',
     $   42,ONEM*W,FACT,ZILCH,IEX4)
C
C     RESORT AND ANTISYMMETRIZE T2 INCREMENT, AND THEN AUGMENT THE 
C     T2 INCREMENT LIST. 
C
...
C
      ...
      I040=I030+ISCRSZ
      IF(I040.GT.MAXCOR)CALL INSMEM('OST2IIAI1',I040,MAXCOR)
C
