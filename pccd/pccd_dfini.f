      ...
      SUBROUTINE PCCD_DFINI(DOO,DVV,DVO,AIOO,AIVV,AIOV,XAI,
...
CJDW ISHIFT for KKB. Worry about 5 vs. 3.
...
CJDW ISHIFT for KKB. Worry about 6 vs. 4.
...
C
CSSS         Print*, "Dvo"
CSSS         Call output(Dvo(ioffd),1,nvrt,1,nocc,nvrt,nocc,1)
CSSS      Print*, "foo"
CSSS         Call output(icore(iofff),1,nocc,1,nocc,nocc,nocc,1)
...
C
        IF(MIN(NOCC,NVRT).NE.0) THEN
CSSS         Print*, "Dvv"
CSSS         Call output(Dvv(ioffd),1,nvrt,1,nvrt,nvrt,nvrt,1)
CSSS         Print*, "fvo"
CSSS         Call output(icore(iofff),1,nvrt,1,nvrt,nvrt,nvrt,1)
C
...
200    CONTINUE 
C
C    ADD HERE f(A,I) TO -I(A,I)
C
C       CALL SAXPY(NT(ISPIN),ONE,ICORE(I0FAI(ISPIN)),1,
C     &            XAI(I0VO(ISPIN)),1)
C       Print*,"Xai-1"
C       Call output(Xai(I0VO(ISPIN)),1,nvrt,1,nocc,nvrt,nocc,1)
C
